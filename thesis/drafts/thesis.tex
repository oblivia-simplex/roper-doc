...
\label{sec:org7b670c4}
\label{org4189442}
\section{What is the aim of this research?}
\label{sec:org2c39ea0}
\label{org2b28d7a}
...
here could, with minor adaptations,\footnote{Discussed in \S \ref{org55c2645}.} used
to test and train \glspl{ids}, or provide one more instrument in the penetration
tester's toolbox.

\section{Who is this for?}
\label{sec:org8de915b}
...
\section{Synopsis}
\label{sec:org9b68416}
\label{org69b6133}

In Chapter \ref{org8dc5fe8}, I set up some of the conceptual
...
design, in Chapter \ref{org1b629bf}.

Chapter \ref{orged9fb06} introduces my contribution to research in the field of
...
Chapter \ref{org7aafeca} goes over a handful of experimental studies with
\gls{roper}, and consequent modifications to the design. 

Chapter \ref{org55c2645} lays out some directions for future work and study on this
...
\label{sec:org8464190}
\label{orgcc50912}
\subsection{1969: Benford}
\label{sec:orgdb15f02}
...
somewhat, in \S \ref{org070c6c9}, but it remains a useful
simplification.

The main loop, outlined in Algorithm \ref{alg:evolve} and figure \ref{fig:orgb8137cf},
...
implementing the algorithms surveyed in \S \ref{org82fa823}.
...
harvested from the target executable. 
  \footnote{See \S \ref{org8a2c336} for a sustained explanation of how return-oriented
...
in figure \ref{fig:orgb77fbea}. Since we are
...
As explained in \S \ref{org2f373ce}, it is often helpful to think of
...
harvested from the target binary (see \S \ref{org282129b}), and a
...
these consequences in Chapter \ref{org7aafeca}.


\subsection{Crossover}
\label{sec:org75d01f4}
\label{org74c5e2e} 
...
\ref{org48125fa}. The details of the algorithm itself can be found in
\ref{alg:crossover}.
...
Section \ref{org6d387e7}. If we abstract away from the information loss
...
Section \ref{org6d387e7} commute with and distribute over crossover, if we consider
...
\label{sec:org74bc30f}
\label{org6e1ba9b}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/evaluation_model.png}
\caption{\label{fig:org3b95eb9}
High-level overview of the ontogenesis and evaluation process. The ``path diversity'' component will not be introduced until we reach \S \ref{org4652b79}.}
\end{figure}
...
penalty to fitness, the details of which will be discussed in \S \ref{org8f7fe24}.
...
detail in Chapter \ref{org7aafeca}. For the time being, the matter of \gls{cpu}
exceptions deserves closer comment.

\subsubsection{Failure modes and crash rates}
\label{sec:org012fa02}
...
of fitness function, listed in \S \ref{org8f7fe24} -- one way that diversity depletion often
...
binary's \texttt{.text} section are plotted in figure \ref{fig:org3b3a855}, to which
I will make frequent reference in the subsequent heatmap overlay images.

In the following subsections (\ref{orga27f8a4}, \ref{org7448e5a},
\ref{org83718e7}, and \ref{org360333e}), I will outline each task set that
\gls{roper} was given to perform, and specify the way in which each was
incorporated into \gls{roper}'s fitness function.

In \S \ref{orga91af63}, I will walk through some of the more interesting results
gleaned from each experiment, and in \S \ref{org84da35e}, I will lay out certain
...
\label{sec:org66c38e7}
\label{orga27f8a4}

As discussed in \S\S \ref{org6d387e7} and \ref{org74c5e2e}, when designing a genetic
...
as a component of the fitness function (see \S \ref{org8f7fe24} for details),
...
\gls{roper}, so long as the fitness sharing mechanism discussed in \ref{org93c0d7c}
...
In figure \ref{fig:org7da6ae7}, we see heatmaps of the the \emph{Xeqcyv} population's
...
\ref{fig:org7a1bd8c}, is not at all surprising. The longer they spend in
...
a surprising degree of ruggedness, as we can see in figure \ref{fig:org29e0667}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/wiwzuh_syscall_gaussian_3.pdf}
\caption{\label{fig:org29e0667}
...
usage shown in figure \ref{fig:org7055cd3}. If the output registers
...
fitness sharing (see \S \ref{org93c0d7c}) provides a means for the evolutionary
...
and two classes (see figure \ref{fig:org73a0a97}).
...
\ref{fig:org7e093a0}). Its best specimen clearly relied on a single
...
\caption{\label{fig:org7e093a0}
The classification on the data displayed in figure \ref{fig:org73a0a97} performed by the best specimen in the \emph{Kathot} population, 35\(^{\text{th}}\) generation (execution trace inset).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kathot_fitness.pdf}
\caption{\label{fig:org038c2df}
...
\caption{\label{fig:org1a120bd}
Address visitation heatmap of the \emph{Fizwej} population, at season 4 on the left, and season 124, on the right, showing a dramatic decline in phenotypic diversity. Compare with the \emph{xeqcyv} and \emph{kurlig} heatmaps in figures \ref{fig:org7da6ae7} and \ref{fig:org235a0d1}.}
\end{figure}
...
of future directions for this project, in \S \ref{org55c2645}. Incorporating
...
will look at more closely in \S \ref{org54cd4ed}) , which I stumbled
...
20,000\(^{\text{th}}\) tournament iteration (figure \ref{fig:org4205b86}). Due to the data that
was collected at the time being insufficiently coarse-grained, I cannot say
whether dynamics similar to those described in \S \ref{org50555a8} played
...
In view of the phenomena described in \S \ref{org50555a8}, however, this approach to regulating the crash rate may warrant revision. 

\paragraph{A proposed method for regulating malignant genes with a TTL field}
\label{sec:org0c7737c}
\label{org2554b2e}
...
Section \ref{orge4cd01a}. Its attribute-class mapping is plotted
in figure \ref{fig:org6405515}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_plot.pdf}
\caption{\label{fig:org6405515}
...
This is what we see happening in figure \ref{fig:org66b6b41}, for example,
...
depth in section \ref{org7c81268}.
...
mechanic detailed in \S \ref{org48125fa}, however: a sufficiently fragile gene
...
\ref{fig:org66b6b41} is a symptom of (premature) genetic convergence. 
...
\caption{\label{fig:org38fd12a}
A trial similar to the one documented in figure \ref{fig:org49fbe8c}, with per-class difficulty recorded, but with the fitness sharing mechanism suspended (\emph{Cazmud} population). The filled curve surrounding each mean difficulty class, here being of visible breadth only in the case of the \emph{iris virginia}, represents the standard deviation of difficulty for each exemplar class.}
\end{figure}

For this reason, I introduced into \gls{roper} the \emph{fitness sharing} mechanism 
that I have outlined in \S \ref{org93c0d7c}. 
...
phylogenic generation of 91.3. Figure \ref{fig:org49fbe8c} shows the course the
...
\caption{\label{fig:org49fbe8c}
A very good run on the Iris classification task, employing the fitness sharing algorithm documented in \S \ref{org93c0d7c} (\emph{Ragweb} population). The filled curve surrounding each mean difficulty line again represents the standard deviation of difficulty for that exemplar class.}
\end{figure}



This run can be informatively compared with the one illustrated in figure
\ref{fig:org38fd12a}. Note the tight interbraiding of problem difficulties in
figure \ref{fig:org49fbe8c}, as compared to their gaping -- but still, slowly,
fluctuating -- disparity in figure \ref{fig:org38fd12a}. The ballooning standard
deviation of difficulty by class in figure \ref{fig:org49fbe8c} also suggests
...
The specifications of the game are described above, in \S \ref{org360333e}.
...
in \S \ref{orgcc50912}), our ``in vitro'' populations have the advantage of a gently
...
exploited process.\footnote{See the semantic correction noted in \S \ref{org914fef0}, which 
...
discussed in \S\S \ref{org7448e5a} and \ref{org914fef0}, but
...
The results, presented in figures \ref{fig:org8259b69} and
\ref{fig:org7c8a573}, were found to corroborate the intron hypothesis,
...
appearing in figure \ref{fig:org8259b69}, suggest that this \emph{may} be the
...
\label{sec:orge38ef07}
\label{org55c2645}

\section{The devil in the details}
\label{sec:org73dc24d}
...
spend the majority of Chapter \ref{org7aafeca} labouring over peculiarities
...
\label{sec:orgd54f032}

The reader will have noticed that Chapter \ref{org7aafeca} raised at least
...
\label{sec:org07d5679}

This was touched on in \S \ref{org2554b2e}, in response to an interesting case
...
\label{sec:org64fdabc}

Frequently, in my analysis of various peculiar behaviours in \gls{roper}'s
populations, in Chapter \ref{org7aafeca}, my enquiry was cut short by a simple
...
\label{sec:org091c4ab}

The experiment touched on in \ref{orgd5e4bc1} remained largely tangential,
...
explains why we see it being given so great a preference in figure \ref{fig:org7055cd3}. 
...
\caption{\label{fig:org067fa2c}
Performance of the \emph{Xysxim} population on the simple, linearly separable dataset first encountered in \S \ref{orge4cd01a}.}
\end{figure}
...
\label{sec:orgfebe815}
\label{orgfeabeb1}
\subsection{The trouble with crossover}
\label{sec:orged81df9}
...
used in \S \ref{org93c0d7c}, and demonstrate its impact on classification tasks in 
Section \ref{org5d49832}. Its applicability, as we have seen, relies
...
neither the syscall preparation task (cf. \S \ref{org914fef0}), which uses a
singleton problem set, nor the
Snake experiment (cf. \S \ref{orgd5e4bc1}), which uses a continuously
...
throughout Chapter \ref{org7aafeca}, in the analysis of heatmap data. 
...
figure \ref{fig:orgc4d3817} -- whose evolutionary run bore the same configuration as the population
in which the malignant gene problem was first diagnosed (see \S \ref{org50555a8}).  

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/xilvyb.pdf}
\caption{\label{fig:orgc4d3817}
...
and introducing a selective incentive for behavioural diversity (see \S \ref{org4652b79}) appears to yield
...
completing the syscall preparation task, outlined in \S \ref{org7448e5a}.
...
\ref{fig:org49fbe8c} was never equaled in subsequent trials. 
...
\ref{fig:org49fbe8c}, but the results so far have shown an extraordinary improvement over
their predecessors. The performance curves for this run are shown in figure \ref{fig:org4e15200}. 
...
As we can see in figure \ref{fig:org5d56d15}, \emph{even the linearly inseparable} 
