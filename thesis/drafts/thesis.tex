...
\label{sec:org1f93620}
\label{orga8c5858}
\section{What is the aim of this research?}
\label{sec:orgeda57fd}
\label{org2e1a69d}
...
here could, with minor adaptations,\footnote{Discussed in \S \ref{orgf86fd91}.} used
to test and train \glspl{ids}, or provide one more instrument in the penetration
tester's toolbox.

\section{Who is this for?}
\label{sec:org6bd156c}
...
\section{Synopsis}
\label{sec:org444bce0}
\label{orga20f42c}

In Chapter \ref{orgacd6922}, I set up some of the conceptual
...
design, in Chapter \ref{org31fcb5e}.

Chapter \ref{orgd120b91} introduces my contribution to research in the field of
...
Chapter \ref{org8141f9a} goes over a handful of experimental studies with
\gls{roper}, and consequent modifications to the design. 

Chapter \ref{orgf86fd91} lays out some directions for future work and study on this
...
This will become clear as we work through Chapter \ref{org8141f9a}.
...
\label{sec:org7e83f82}
\label{orgb85df15}
\subsection{1969: Benford}
\label{sec:org52cf609}
...
somewhat, in \S \ref{orgfeb8768}, but it remains a useful
...
The main loop, outlined in Algorithm \ref{alg:evolve} and figure \ref{fig:orge325c01},
...
implementing the algorithms surveyed in \S \ref{org8f485c2}.
...
harvested from the target executable. 
  \footnote{See \S \ref{orgc83c3a6} for a sustained explanation of how return-oriented
...
in figure \ref{fig:org181c9d1}. Since we are
...
As explained in \S \ref{orgec390a8}, it is often helpful to think of
...
harvested from the target binary (see \S \ref{org0619972}), and a
...
these consequences in Chapter \ref{org8141f9a}.


\subsection{Crossover}
\label{sec:org879afcb}
\label{org7cecdc1} 
...
\ref{org9b572c7}. The details of the algorithm itself can be found in
\ref{alg:crossover}.
...
Section \ref{orga9cd8de}. If we abstract away from the information loss
...
Section \ref{orga9cd8de} commute with and distribute over crossover, if we consider
...
\label{sec:org4df5547}
\label{org5f88bb3}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/evaluation_model.png}
\caption{\label{fig:orgc96bb50}
High-level overview of the ontogenesis and evaluation process. The ``path diversity'' component will not be introduced until we reach \S \ref{orge7f4d14}.}
\end{figure}
...
penalty to fitness, the details of which will be discussed in \S \ref{org24dd41a}.
...
detail in Chapter \ref{org8141f9a}. For the time being, the matter of \gls{cpu}
exceptions deserves closer comment.

\subsubsection{Failure modes and crash rates}
\label{sec:orgbf2a2fa}
...
of fitness function, listed in \S \ref{org24dd41a} -- one way that diversity depletion often
...
binary. (In all of the experiments documented in Chapter \ref{org8141f9a},
...
binary's \texttt{.text} section are plotted in figure \ref{fig:orgb056286}, to which
I will make frequent reference in the subsequent heatmap overlay images.

In the following subsections (\ref{orgb3a60c7}, \ref{org76f14fd},
\ref{org6840d76}, and \ref{org15db0bd}), I will outline each task set that
\gls{roper} was given to perform, and specify the way in which each was
incorporated into \gls{roper}'s fitness function.

In \S \ref{orga1769d3}, I will walk through some of the more interesting results
gleaned from each experiment, and in \S \ref{org06aa58c}, I will lay out certain
...
\label{sec:org157f892}
\label{orgb3a60c7}

As discussed in \S\S \ref{orga9cd8de} and \ref{org7cecdc1}, when designing a genetic
...
as a component of the fitness function (see \S \ref{org24dd41a} for details),
...
\gls{roper}, so long as the fitness sharing mechanism discussed in \ref{org873a2f5}
...
In figure \ref{fig:org0bef0c7}, we see heatmaps of the the \emph{Xeqcyv}\index{Xeqcyv} population's
...
\ref{fig:org2da1b55}, is not at all surprising. The longer they spend in
...
a surprising degree of ruggedness, as we can see in figure \ref{fig:orga65aed3}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/wiwzuh_syscall_gaussian_3.pdf}
\caption{\label{fig:orga65aed3}
...
usage shown in figure \ref{fig:org1b4f088}. If the output registers
...
fitness sharing (see \S \ref{org873a2f5}) provides a means for the evolutionary
...
and two classes (see figure \ref{fig:orge7b1d81}).
...
\ref{fig:org443b7d0}). Its best specimen clearly relied on a single
...
\caption{\label{fig:org443b7d0}
The classification on the data displayed in figure \ref{fig:orge7b1d81} performed by the best specimen in the \emph{Kathot}\index{Kathot} population, 35\(^{\text{th}}\) generation (execution trace inset).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kathot_fitness.pdf}
\caption{\label{fig:org3e2fce5}
...
\caption{\label{fig:org6b0497b}
Address visitation heatmap of the \emph{Fizwej}\index{Fizwej} population, at season 4 on the left, and season 124, on the right, showing a dramatic decline in phenotypic diversity. Compare with the \emph{xeqcyv} and \emph{kurlig} heatmaps in figures \ref{fig:org0bef0c7} and \ref{fig:orgd09270a}.}
\end{figure}
...
of future directions for this project, in \S \ref{orgf86fd91}. Incorporating
...
will look at more closely in \S \ref{org5a56b7a}) , which I stumbled
...
20,000\(^{\text{th}}\) tournament iteration (figure \ref{fig:org14d9835}). Due to the data that
was collected at the time being insufficiently coarse-grained, I cannot say
whether dynamics similar to those described in \S \ref{org4a09ea9} played
...
In view of the phenomena described in \S \ref{org4a09ea9}, however, this approach to regulating the crash rate may warrant revision. 

\paragraph{A proposed method for regulating malignant genes with a TTL field}
\label{sec:org881c5e3}
\label{org73366bd}
...
Section \ref{org7772db3}. Its attribute-class mapping is plotted
in figure \ref{fig:org8d7878f}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_plot.pdf}
\caption{\label{fig:org8d7878f}
...
This is what we see happening in figure \ref{fig:orge846c81}, for example,
...
depth in section \ref{orgc9217f9}.
...
mechanic detailed in \S \ref{org9b572c7}, however: a sufficiently fragile gene
...
\ref{fig:orge846c81} is a symptom of (premature) genetic convergence. 
...
\caption{\label{fig:org9070f1f}
A trial similar to the one documented in figure \ref{fig:org4116c0a}, with per-class difficulty recorded, but with the fitness sharing mechanism suspended (\emph{Cazmud}\index{Cazmud} population). The filled curve surrounding each mean difficulty class, here being of visible breadth only in the case of the \emph{iris virginia}, represents the standard deviation of difficulty for each exemplar class.}
\end{figure}

For this reason, I introduced into \gls{roper} the \emph{fitness sharing} mechanism 
that I have outlined in \S \ref{org873a2f5}. 
...
phylogenic generation of 91.3. Figure \ref{fig:org4116c0a} shows the course the
...
\caption{\label{fig:org4116c0a}
A very good run on the Iris classification task, employing the fitness sharing algorithm documented in \S \ref{org873a2f5} (\emph{Ragweb}\index{Ragweb} population). The filled curve surrounding each mean difficulty line again represents the standard deviation of difficulty for that exemplar class.}
\end{figure}



This run can be informatively compared with the one illustrated in figure
\ref{fig:org9070f1f}. Note the tight interbraiding of problem difficulties in
figure \ref{fig:org4116c0a}, as compared to their gaping -- but still, slowly,
fluctuating -- disparity in figure \ref{fig:org9070f1f}. The ballooning standard
deviation of difficulty by class in figure \ref{fig:org4116c0a} also suggests
...
The specifications of the game are described above, in \S \ref{org15db0bd}.
...
in \S \ref{orgb85df15}), our ``in vitro'' populations have the advantage of a gently
...
exploited process.\footnote{See the semantic correction noted in \S \ref{org286f47c}, which 
...
discussed in \S\S \ref{org76f14fd} and \ref{org286f47c}, but
...
The results, presented in figures \ref{fig:orgdccafeb} and
\ref{fig:org5b032f6}, were found to corroborate the intron hypothesis,
...
appearing in figure \ref{fig:orgdccafeb}, suggest that this \emph{may} be the
...
\label{sec:org7081d60}
\label{orgf86fd91}

\section{The devil in the details}
\label{sec:orgce6dc7b}
...
spend the majority of Chapter \ref{org8141f9a} labouring over peculiarities
...
\label{sec:org6fd3ef4}

The reader will have noticed that Chapter \ref{org8141f9a} raised at least
...
\label{sec:org9216208}

This was touched on in \S \ref{org73366bd}, in response to an interesting case
...
\label{sec:org56e6b9a}

Frequently, in my analysis of various peculiar behaviours in \gls{roper}'s
populations, in Chapter \ref{org8141f9a}, my enquiry was cut short by a simple
...
\label{sec:org8a7ffcf}

The experiment touched on in \ref{org5a85e87} remained largely tangential,
...
explains why we see it being given so great a preference in figure \ref{fig:org1b4f088}. 
...
\caption{\label{fig:orgde8ad16}
Performance of the \emph{Xysxim}\index{Xysxim} population on the simple, linearly separable dataset first encountered in \S \ref{org7772db3}.}
\end{figure}
...
\label{sec:orgc02b764}
\label{org79d680c}
\subsection{The trouble with crossover}
\label{sec:org9149d5c}
...
used in \S \ref{org873a2f5}, and demonstrate its impact on classification tasks in 
Section \ref{org977259b}. Its applicability, as we have seen, relies
...
neither the syscall preparation task (cf. \S \ref{org286f47c}), which uses a
singleton problem set, nor the
Snake experiment (cf. \S \ref{org5a85e87}), which uses a continuously
...
throughout Chapter \ref{org8141f9a}, in the analysis of heatmap data. 
...
figure \ref{fig:org8dfec6f} -- whose evolutionary run bore the same configuration as the population
in which the malignant gene problem was first diagnosed (see \S \ref{org4a09ea9}).  

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/xilvyb.pdf}
\caption{\label{fig:org8dfec6f}
...
and introducing a selective incentive for behavioural diversity (see \S \ref{orge7f4d14}) appears to yield
...
completing the syscall preparation task, outlined in \S \ref{org76f14fd}.
...
\ref{fig:org4116c0a} was never equaled in subsequent trials. 
...
\ref{fig:org4116c0a}, but the results so far have shown an extraordinary improvement over
their predecessors. The performance curves for this run are shown in figure \ref{fig:org6133d02}. 
...
As we can see in figure \ref{fig:orge0782c6}, \emph{even the linearly inseparable} 
...
\label{sec:org72c0c2b}
\listofalgorithms

\chapter{Population Parameters}
\label{sec:org975eac9}

\section{Note}
\label{sec:org7f22cca}
