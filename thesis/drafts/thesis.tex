...
\label{sec:org13ab31f}
\label{orgae22500}
\section{What is the aim of this research?}
\label{sec:org46296cb}
\label{orgdf778dc}
...
here could, with minor adaptations,\footnote{Discussed in \S \ref{org1283011}.} used
to test and train \glspl{ids}, or provide one more instrument in the penetration
tester's toolbox.

\section{Who is this for?}
\label{sec:orga349c4f}
...
\section{Synopsis}
\label{sec:org1c4a39a}
\label{orga8384e5}

In Chapter \ref{orga752c58}, I set up some of the conceptual
...
design, in Chapter \ref{org071f63c}.

Chapter \ref{org56e973f} introduces my contribution to research in the field of
...
Chapter \ref{org5ee2bea} goes over a handful of experimental studies with
\gls{roper}, and consequent modifications to the design. 

Chapter \ref{org1283011} lays out some directions for future work and study on this
...
\label{sec:org9843b5b}
\label{org49e0918}
\subsection{1969: Benford}
\label{sec:org3428d18}
...
somewhat, in \S \ref{org6d9f884}, but it remains a useful
simplification.

The main loop, outlined in Algorithm \ref{alg:evolve} and figure \ref{fig:org3fc756f},
...
implementing the algorithms surveyed in \S \ref{orgd435a80}.
...
harvested from the target executable. 
  \footnote{See \S \ref{org43ae0bd} for a sustained explanation of how return-oriented
...
in figure \ref{fig:org8f4e543}. Since we are
...
As explained in \S \ref{org1aafff9}, it is often helpful to think of
...
harvested from the target binary (see \S \ref{orgeee738b}), and a
...
these consequences in Chapter \ref{org5ee2bea}.


\subsection{Crossover}
\label{sec:org1049842}
\label{orgc3645de} 
...
\ref{org9f59d3f}. The details of the algorithm itself can be found in
\ref{alg:crossover}.
...
Section \ref{org27383fb}. If we abstract away from the information loss
...
Section \ref{org27383fb} commute with and distribute over crossover, if we consider
...
\label{sec:org6c20c52}
\label{orgf5349eb}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/evaluation_model.png}
\caption{\label{fig:org40c25c9}
High-level overview of the ontogenesis and evaluation process. The ``path diversity'' component will not be introduced until we reach \S \ref{orgcc39c0f}.}
\end{figure}
...
penalty to fitness, the details of which will be discussed in \S \ref{org7170358}.
...
detail in Chapter \ref{org5ee2bea}. For the time being, the matter of \gls{cpu}
exceptions deserves closer comment.

\subsubsection{Failure modes and crash rates}
\label{sec:org9c63828}
...
of fitness function, listed in \S \ref{org7170358} -- one way that diversity depletion often
...
binary's \texttt{.text} section are plotted in figure \ref{fig:orge6e44b0}, to which
I will make frequent reference in the subsequent heatmap overlay images.

In the following subsections (\ref{org190a98a}, \ref{org97637af},
\ref{orgd466cbb}, and \ref{org83d1c6d}), I will outline each task set that
\gls{roper} was given to perform, and specify the way in which each was
incorporated into \gls{roper}'s fitness function.

In \S \ref{orgfc3cb5b}, I will walk through some of the more interesting results
gleaned from each experiment, and in \S \ref{org3c2810a}, I will lay out certain
...
\label{sec:orgf3e0ee0}
\label{org190a98a}

As discussed in \S\S \ref{org27383fb} and \ref{orgc3645de}, when designing a genetic
...
as a component of the fitness function (see \S \ref{org7170358} for details),
...
\gls{roper}, so long as the fitness sharing mechanism discussed in \ref{org6de6bad}
...
In figure \ref{fig:org4dbf45c}, we see heatmaps of the the \emph{Xeqcyv} population's
...
\ref{fig:org8eca0e3}, is not at all surprising. The longer they spend in
...
a surprising degree of ruggedness, as we can see in figure \ref{fig:orgfe28838}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/wiwzuh_syscall_gaussian_3.pdf}
\caption{\label{fig:orgfe28838}
...
usage shown in figure \ref{fig:org68ed952}. If the output registers
...
fitness sharing (see \S \ref{org6de6bad}) provides a means for the evolutionary
...
and two classes (see figure \ref{fig:org3bc1553}).
...
\ref{fig:orga5d780a}). Its best specimen clearly relied on a single
...
\caption{\label{fig:orga5d780a}
The classification on the data displayed in figure \ref{fig:org3bc1553} performed by the best specimen in the \emph{Kathot} population, 35\(^{\text{th}}\) generation (execution trace inset).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kathot_fitness.pdf}
\caption{\label{fig:orgbc80a44}
...
\caption{\label{fig:org8d5e29e}
Address visitation heatmap of the \emph{Fizwej} population, at season 4 on the left, and season 124, on the right, showing a dramatic decline in phenotypic diversity. Compare with the \emph{xeqcyv} and \emph{kurlig} heatmaps in figures \ref{fig:org4dbf45c} and \ref{fig:orgef5b92a}.}
\end{figure}
...
of future directions for this project, in \S \ref{org1283011}. Incorporating
...
will look at more closely in \S \ref{org938c3b8}) , which I stumbled
...
20,000\(^{\text{th}}\) tournament iteration (figure \ref{fig:orgfd11afb}). Due to the data that
was collected at the time being insufficiently coarse-grained, I cannot say
whether dynamics similar to those described in \S \ref{org4f10d10} played
...
In view of the phenomena described in \S \ref{org4f10d10}, however, this approach to regulating the crash rate may warrant revision. 

\paragraph{A proposed method for regulating malignant genes with a TTL field}
\label{sec:org2541e6e}
\label{org338807c}
...
Section \ref{org5e80a3e}. Its attribute-class mapping is plotted
in figure \ref{fig:org71201ff}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_plot.pdf}
\caption{\label{fig:org71201ff}
...
This is what we see happening in figure \ref{fig:orgb32be70}, for example,
...
depth in section \ref{org8aa03d6}.
...
mechanic detailed in \S \ref{org9f59d3f}, however: a sufficiently fragile gene
...
\ref{fig:orgb32be70} is a symptom of (premature) genetic convergence. 
...
\caption{\label{fig:orgb2a9efc}
A trial similar to the one documented in figure \ref{fig:org33e535c}, with per-class difficulty recorded, but with the fitness sharing mechanism suspended (\emph{Cazmud} population). The filled curve surrounding each mean difficulty class, here being of visible breadth only in the case of the \emph{iris virginia}, represents the standard deviation of difficulty for each exemplar class.}
\end{figure}

For this reason, I introduced into \gls{roper} the \emph{fitness sharing} mechanism 
that I have outlined in \S \ref{org6de6bad}. 
...
phylogenic generation of 91.3. Figure \ref{fig:org33e535c} shows the course the
...
\caption{\label{fig:org33e535c}
A very good run on the Iris classification task, employing the fitness sharing algorithm documented in \S \ref{org6de6bad} (\emph{Ragweb} population). The filled curve surrounding each mean difficulty line again represents the standard deviation of difficulty for that exemplar class.}
\end{figure}



This run can be informatively compared with the one illustrated in figure
\ref{fig:orgb2a9efc}. Note the tight interbraiding of problem difficulties in
figure \ref{fig:org33e535c}, as compared to their gaping -- but still, slowly,
fluctuating -- disparity in figure \ref{fig:orgb2a9efc}. The ballooning standard
deviation of difficulty by class in figure \ref{fig:org33e535c} also suggests
...
The specifications of the game are described above, in \S \ref{org83d1c6d}.
...
in \S \ref{org49e0918}), our ``in vitro'' populations have the advantage of a gently
...
discussed in \S\S \ref{org97637af} and \ref{org7fe81b2}, but
...
The results, presented in figures \ref{fig:orgd21aa59} and
\ref{fig:org05fc82b}, were found to corroborate the intron hypothesis,
...
appearing in figure \ref{fig:orgd21aa59}, suggest that this \emph{may} be the
...
\label{sec:orga03fece}
\label{org1283011}

\section{The devil in the details}
\label{sec:orge1b36ed}
...
spend the majority of Chapter \ref{org5ee2bea} labouring over peculiarities
...
\label{sec:orgac8c274}

The reader will have noticed that Chapter \ref{org5ee2bea} raised at least
...
\label{sec:org9551637}

This was touched on in \S \ref{org338807c}, in response to an interesting case
...
\label{sec:org54efc50}

Frequently, in my analysis of various peculiar behaviours in \gls{roper}'s
populations, in Chapter \ref{org5ee2bea}, my enquiry was cut short by a simple
...
\label{sec:orgd8339d1}

The experiment touched on in \ref{org003e37c} remained largely tangential,
...
explains why we see it being given so great a preference in figure \ref{fig:org68ed952}. 
...
\caption{\label{fig:org97238b2}
Performance of the \emph{Xysxim} population on the simple, linearly separable dataset first encountered in \S \ref{org5e80a3e}.}
\end{figure}
...
\label{sec:orga2feab3}
\label{orgbc9ffd9}
\subsection{The trouble with crossover}
\label{sec:org151529a}
...
used in \S \ref{org6de6bad}, and demonstrate its impact on classification tasks in 
Section \ref{org3a21edd}. Its applicability, as we have seen, relies
...
neither the syscall preparation task (cf. \S \ref{org7fe81b2}), which uses a
singleton problem set, nor the
Snake experiment (cf. \S \ref{org003e37c}), which uses a continuously
...
throughout Chapter \ref{org5ee2bea}, in the analysis of heatmap data. 
...
figure \ref{fig:org8a7705e} -- whose evolutionary run bore the same configuration as the population
in which the malignant gene problem was first diagnosed (see \S \ref{org4f10d10}).  

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/xilvyb.pdf}
\caption{\label{fig:org8a7705e}
...
and introducing a selective incentive for behavioural diversity (see \S \ref{orgcc39c0f}) appears to yield
...
completing the syscall preparation task, outlined in \S \ref{org97637af}.
