...
\label{sec:org2c24465}
\label{orgdab7e81}
\section{What is the aim of this research?}
\label{sec:orgb7130d9}
\label{org33148c9}
...
here could, with minor adaptations,\footnote{Discussed in \S \ref{org007310d}.} used
to test and train \glspl{ids}, or provide one more instrument in the penetration
tester's toolbox.

\section{Who is this for?}
\label{sec:orgbb0e6d2}
...
\section{Synopsis}
\label{sec:orgfe84079}
\label{org2b9a7fc}

In Chapter \ref{org6657218}, I set up some of the conceptual
...
design, in Chapter \ref{org43c71b6}.

Chapter \ref{org3f736c1} introduces my contribution to research in the field of
...
Chapter \ref{orgcdf8d25} goes over a handful of experimental studies with
\gls{roper}, and consequent modifications to the design. 

Chapter \ref{org007310d} lays out some directions for future work and study on this
...
\label{sec:orgf0899ad}
\label{orgc78aaba}
\subsection{1969: Benford}
\label{sec:org8438060}
...
somewhat, in \S \ref{orgdee5bc5}, but it remains a useful
simplification.

The main loop, outlined in Algorithm \ref{alg:evolve} and figure \ref{fig:orgb2f9b7d},
...
implementing the algorithms surveyed in \S \ref{org7d6c015}.
...
harvested from the target executable. 
  \footnote{See \S \ref{orgb502966} for a sustained explanation of how return-oriented
...
in figure \ref{fig:orga5ad2fb}. Since we are
...
As explained in \S \ref{orgc4f6fa8}, it is often helpful to think of
...
harvested from the target binary (see \S \ref{orgca7f630}), and a
...
these consequences in Chapter \ref{orgcdf8d25}.


\subsection{Crossover}
\label{sec:org17b894f}
\label{orgdeefff9} 
...
\ref{orgebcec72}. The details of the algorithm itself can be found in
\ref{alg:crossover}.
...
Section \ref{org54247dc}. If we abstract away from the information loss
...
Section \ref{org54247dc} commute with and distribute over crossover, if we consider
...
\label{sec:org8966df6}
\label{org199bce9}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/evaluation_model.png}
\caption{\label{fig:org2ada580}
High-level overview of the ontogenesis and evaluation process. The ``path diversity'' component will not be introduced until we reach \S \ref{orgd16c905}.}
\end{figure}
...
penalty to fitness, the details of which will be discussed in \S \ref{org6ca95bb}.
...
detail in Chapter \ref{orgcdf8d25}. For the time being, the matter of \gls{cpu}
exceptions deserves closer comment.

\subsubsection{Failure modes and crash rates}
\label{sec:orga3ff723}
...
of fitness function, listed in \S \ref{org6ca95bb} -- one way that diversity depletion often
...
binary's \texttt{.text} section are plotted in figure \ref{fig:orgca127ea}, to which
I will make frequent reference in the subsequent heatmap overlay images.

In the following subsections (\ref{orgcb688a6}, \ref{orgb8cfcdd},
\ref{org77b7516}, and \ref{org34bcff0}), I will outline each task set that
\gls{roper} was given to perform, and specify the way in which each was
incorporated into \gls{roper}'s fitness function.

In \S \ref{org4b36135}, I will walk through some of the more interesting results
gleaned from each experiment, and in \S \ref{org7d2d887}, I will lay out certain
...
\label{sec:org6b41772}
\label{orgcb688a6}

As discussed in \S\S \ref{org54247dc} and \ref{orgdeefff9}, when designing a genetic
...
as a component of the fitness function (see \S \ref{org6ca95bb} for details),
...
\gls{roper}, so long as the fitness sharing mechanism discussed in \ref{org406738a}
...
In figure \ref{fig:org6f8a03e}, we see heatmaps of the the \emph{Xeqcyv} population's
...
\ref{fig:org58cb104}, is not at all surprising. The longer they spend in
...
a surprising degree of ruggedness, as we can see in figure \ref{fig:org177be33}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/wiwzuh_syscall_gaussian_3.pdf}
\caption{\label{fig:org177be33}
...
usage shown in figure \ref{fig:org9c33166}. If the output registers
...
fitness sharing (see \S \ref{org406738a}) provides a means for the evolutionary
...
and two classes (see figure \ref{fig:org7555e5b}).
...
\ref{fig:orge7bff61}). Its best specimen clearly relied on a single
...
\caption{\label{fig:orge7bff61}
The classification on the data displayed in figure \ref{fig:org7555e5b} performed by the best specimen in the \emph{Kathot} population, 35\(^{\text{th}}\) generation (execution trace inset).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kathot_fitness.pdf}
\caption{\label{fig:org2ce881a}
...
\caption{\label{fig:org9d36c1a}
Address visitation heatmap of the \emph{Fizwej} population, at season 4 on the left, and season 124, on the right, showing a dramatic decline in phenotypic diversity. Compare with the \emph{xeqcyv} and \emph{kurlig} heatmaps in figures \ref{fig:org6f8a03e} and \ref{fig:orgeb820ee}.}
\end{figure}
...
of future directions for this project, in \S \ref{org007310d}. Incorporating
...
will look at more closely in \S \ref{org7cc1227}) , which I stumbled
...
20,000\(^{\text{th}}\) tournament iteration (figure \ref{fig:org35ad406}). Due to the data that
was collected at the time being insufficiently coarse-grained, I cannot say
whether dynamics similar to those described in \S \ref{org7074f54} played
...
In view of the phenomena described in \S \ref{org7074f54}, however, this approach to regulating the crash rate may warrant revision. 

\paragraph{A proposed method for regulating malignant genes with a TTL field}
\label{sec:org86a9545}
\label{org32d5268}
...
Section \ref{orgff79d0f}. Its attribute-class mapping is plotted
in figure \ref{fig:org81182ce}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_plot.pdf}
\caption{\label{fig:org81182ce}
...
This is what we see happening in figure \ref{fig:orgf7b0371}, for example,
...
depth in section \ref{orgb60a50c}.
...
mechanic detailed in \S \ref{orgebcec72}, however: a sufficiently fragile gene
...
\ref{fig:orgf7b0371} is a symptom of (premature) genetic convergence. 
...
\caption{\label{fig:org27d8116}
A trial similar to the one documented in figure \ref{fig:orgc3d407b}, with per-class difficulty recorded, but with the fitness sharing mechanism suspended (\emph{Cazmud} population). The filled curve surrounding each mean difficulty class, here being of visible breadth only in the case of the \emph{iris virginia}, represents the standard deviation of difficulty for each exemplar class.}
\end{figure}

For this reason, I introduced into \gls{roper} the \emph{fitness sharing} mechanism 
that I have outlined in \S \ref{org406738a}. 
...
phylogenic generation of 91.3. Figure \ref{fig:orgc3d407b} shows the course the
...
\caption{\label{fig:orgc3d407b}
A very good run on the Iris classification task, employing the fitness sharing algorithm documented in \S \ref{org406738a} (\emph{Ragweb} population). The filled curve surrounding each mean difficulty line again represents the standard deviation of difficulty for that exemplar class.}
\end{figure}



This run can be informatively compared with the one illustrated in figure
\ref{fig:org27d8116}. Note the tight interbraiding of problem difficulties in
figure \ref{fig:orgc3d407b}, as compared to their gaping -- but still, slowly,
fluctuating -- disparity in figure \ref{fig:org27d8116}. The ballooning standard
deviation of difficulty by class in figure \ref{fig:orgc3d407b} also suggests
...
The specifications of the game are described above, in \S \ref{org34bcff0}.
...
in \S \ref{orgc78aaba}), our ``in vitro'' populations have the advantage of a gently
...
discussed in \S\S \ref{orgb8cfcdd} and \ref{org83e7968}, but
...
The results, presented in figures \ref{fig:org924a03d} and
\ref{fig:org15c8ab6}, were found to corroborate the intron hypothesis,
...
appearing in figure \ref{fig:org924a03d}, suggest that this \emph{may} be the
...
\label{sec:org18dc2e4}
\label{org007310d}

\section{The devil in the details}
\label{sec:orgc9e51ec}
...
spend the majority of Chapter \ref{orgcdf8d25} labouring over peculiarities
...
\label{sec:orgb992127}

The reader will have noticed that Chapter \ref{orgcdf8d25} raised at least
...
\label{sec:orge8e5103}

This was touched on in \S \ref{org32d5268}, in response to an interesting case
...
\label{sec:orgb30a1cf}

Frequently, in my analysis of various peculiar behaviours in \gls{roper}'s
populations, in Chapter \ref{orgcdf8d25}, my enquiry was cut short by a simple
...
\label{sec:orga940c18}

The experiment touched on in \ref{org373aa15} remained largely tangential,
...
explains why we see it being given so great a preference in figure \ref{fig:org9c33166}. 
...
\caption{\label{fig:org94314c2}
Performance of the \emph{Xysxim} population on the simple, linearly separable dataset first encountered in \S \ref{orgff79d0f}.}
\end{figure}
...
\label{sec:org84d0cca}
\label{orgb56180f}
\subsection{The trouble with crossover}
\label{sec:orgbe74428}
...
used in \S \ref{org406738a}, and demonstrate its impact on classification tasks in 
Section \ref{orgfcac4a9}. Its applicability, as we have seen, relies
...
neither the syscall preparation task (cf. \S \ref{org83e7968}), which uses a
singleton problem set, nor the
Snake experiment (cf. \S \ref{org373aa15}), which uses a continuously
...
throughout Chapter \ref{orgcdf8d25}, in the analysis of heatmap data. 
...
figure \ref{fig:org221c93d} -- whose evolutionary run bore the same configuration as the population
in which the malignant gene problem was first diagnosed (see \S \ref{org7074f54}).  

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/xilvyb.pdf}
\caption{\label{fig:org221c93d}
...
and introducing a selective incentive for behavioural diversity (see \S \ref{orgd16c905}) appears to yield
...
completing the syscall preparation task, outlined in \S \ref{orgb8cfcdd}.
