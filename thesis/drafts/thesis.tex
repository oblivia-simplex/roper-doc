...
\label{sec:orgaa145f3}
\label{org4a6e972}
\section{What is the aim of this research?}
\label{sec:org830132b}
\label{org6ef5ef1}
...
here could, with minor adaptations,\footnote{Discussed in \S \ref{org0c75d14}.} used
to test and train \glspl{ids}, or provide one more instrument in the penetration
tester's toolbox.

\section{Who is this for?}
\label{sec:org002f3c5}
...
In Chapter \ref{orgfd7afc4}, I set up some of the conceptual
...
design, in Chapter \ref{org7619b21}.

Chapter \ref{orgd925150} introduces my contribution to research in the field of
...
Chapter \ref{org5f2b6d1} goes over a handful of experimental studies with
\gls{roper}, and consequent modifications to the design. 

Chapter \ref{org0c75d14} lays out some directions for future work and study on this
...
\label{sec:org98a7109}
\label{org7b272c7}
\subsection{1969: Benford}
\label{sec:org4175a6c}
...
somewhat, in \S \ref{orga92668e}, but it remains a useful
simplification.

The main loop, outlined in Algorithm \ref{alg:evolve} and figure \ref{fig:org129975a},
...
implementing the algorithms surveyed in \S \ref{orgab6b9ed}.
...
harvested from the target executable. 
  \footnote{See \S \ref{orgbd0c49b} for a sustained explanation of how return-oriented
...
in figure \ref{fig:org1fd1928}. Since we are
...
As explained in \S \ref{orgf16edd0}, it is often helpful to think of
...
harvested from the target binary (see \S \ref{org6e6f678}), and a
...
these consequences in Chapter \ref{org5f2b6d1}.


\subsection{Crossover}
\label{sec:org3c3fbf6}
\label{org12f1d17} 
...
\ref{org1f529d1}. The details of the algorithm itself can be found in
\ref{alg:crossover}.
...
Section \ref{org8c5413d}. If we abstract away from the information loss
...
Section \ref{org8c5413d} commute with and distribute over crossover, if we consider
...
\label{sec:org2ee6b85}
\label{orgdcc8ccb}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/evaluation_model.png}
\caption{\label{fig:org688faea}
High-level overview of the ontogenesis and evaluation process. The ``path diversity'' component will not be introduced until we reach \S \ref{orgf16f7af}.}
\end{figure}
...
penalty to fitness, the details of which will be discussed in \S \ref{org2045881}.
...
detail in Chapter \ref{org5f2b6d1}. For the time being, the matter of \gls{cpu}
exceptions deserves closer comment.

\subsubsection{Failure modes and crash rates}
\label{sec:org250b7cb}
...
of fitness function, listed in \S \ref{org2045881} -- one way that diversity depletion often
...
binary's \texttt{.text} section are plotted in figure \ref{fig:org5f32080}, to which
I will make frequent reference in the subsequent heatmap overlay images.

In the following subsections (\ref{org2527d98}, \ref{org52e6e8b},
\ref{org9d4c9a7}, and \ref{org1ee6dbb}), I will outline each task set that
\gls{roper} was given to perform, and specify the way in which each was
incorporated into \gls{roper}'s fitness function.

In \S \ref{orga02b558}, I will walk through some of the more interesting results
gleaned from each experiment, and in \S \ref{orgd33f56c}, I will lay out certain
...
\label{sec:org3c1f3d3}
\label{org2527d98}

As discussed in \S\S \ref{org8c5413d} and \ref{org12f1d17}, when designing a genetic
...
as a component of the fitness function (see \S \ref{org2045881} for details),
...
\gls{roper}, so long as the fitness sharing mechanism discussed in \ref{org3c3aa7f}
...
In figure \ref{fig:orgfb2d110}, we see heatmaps of the the \emph{Xeqcyv} population's
...
\ref{fig:org56d1d1b}, is not at all surprising. The longer they spend in
...
a surprising degree of ruggedness, as we can see in figure \ref{fig:org0a75575}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/wiwzuh_syscall_gaussian_3.pdf}
\caption{\label{fig:org0a75575}
...
usage shown in figure \ref{fig:org6c537c4}. If the output registers
...
fitness sharing (see \S \ref{org3c3aa7f}) provides a means for the evolutionary
...
and two classes (see figure \ref{fig:orgb7147a5}).
...
\ref{fig:orge47d765}). Its best specimen clearly relied on a single
...
\caption{\label{fig:orge47d765}
The classification on the data displayed in figure \ref{fig:orgb7147a5} performed by the best specimen in the \emph{Kathot} population, 35\(^{\text{th}}\) generation (execution trace inset).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kathot_fitness.pdf}
\caption{\label{fig:org709ad5a}
...
\caption{\label{fig:orgd91d57b}
Address visitation heatmap of the \emph{Fizwej} population, at season 4 on the left, and season 124, on the right, showing a dramatic decline in phenotypic diversity. Compare with the \emph{xeqcyv} and \emph{kurlig} heatmaps in figures \ref{fig:orgfb2d110} and \ref{fig:org0912751}.}
\end{figure}
...
of future directions for this project, in \S \ref{org0c75d14}. Incorporating
...
will look at more closely in \S \ref{org04968d6}) , which I stumbled
...
20,000\(^{\text{th}}\) tournament iteration (figure \ref{fig:org90e8640}). Due to the data that
was collected at the time being insufficiently coarse-grained, I cannot say
whether dynamics similar to those described in \S \ref{org3c0fde9} played
...
In view of the phenomena described in \S \ref{org3c0fde9}, however, this approach to regulating the crash rate may warrant revision. 

\paragraph{A proposed method for regulating malignant genes with a TTL field}
\label{sec:org41e4969}
\label{org1c93dcc}
...
Section \ref{org566ba5c}. Its attribute-class mapping is plotted
in figure \ref{fig:orgd150305}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_plot.pdf}
\caption{\label{fig:orgd150305}
...
This is what we see happening in figure \ref{fig:org751b6f0}, for example,
...
depth in section \ref{orgdbe57f0}.
...
mechanic detailed in \S \ref{org1f529d1}, however: a sufficiently fragile gene
...
\ref{fig:org751b6f0} is a symptom of (premature) genetic convergence. 
...
\caption{\label{fig:orgcbb515d}
A trial similar to the one documented in figure \ref{fig:org3eeaf46}, with per-class difficulty recorded, but with the fitness sharing mechanism suspended (\emph{Cazmud} population). The filled curve surrounding each mean difficulty class, here being of visible breadth only in the case of the \emph{iris virginia}, represents the standard deviation of difficulty for each exemplar class.}
\end{figure}

For this reason, I introduced into \gls{roper} the \emph{fitness sharing} mechanism 
that I have outlined in \S \ref{org3c3aa7f}. 
...
phylogenic generation of 91.3. Figure \ref{fig:org3eeaf46} shows the course the
...
\caption{\label{fig:org3eeaf46}
A very good run on the Iris classification task, employing the fitness sharing algorithm documented in \S \ref{org3c3aa7f} (\emph{Ragweb} population). The filled curve surrounding each mean difficulty line again represents the standard deviation of difficulty for that exemplar class.}
\end{figure}



This run can be informatively compared with the one illustrated in figure
\ref{fig:orgcbb515d}. Note the tight interbraiding of problem difficulties in
figure \ref{fig:org3eeaf46}, as compared to their gaping -- but still, slowly,
fluctuating -- disparity in figure \ref{fig:orgcbb515d}. The ballooning standard
deviation of difficulty by class in figure \ref{fig:org3eeaf46} also suggests
...
The specifications of the game are described above, in \S \ref{org1ee6dbb}.
...
in \S \ref{org7b272c7}), our ``in vitro'' populations have the advantage of a gently
...
exploited process.\footnote{See the semantic correction noted in \S \ref{orge84ebb6}, which 
...
discussed in \S\S \ref{org52e6e8b} and \ref{orge84ebb6}, but
...
The results, presented in figures \ref{fig:orgb3504aa} and
\ref{fig:org1fbf7ff}, were found to corroborate the intron hypothesis,
...
appearing in figure \ref{fig:orgb3504aa}, suggest that this \emph{may} be the
...
\label{sec:orgba8cdd8}
\label{org0c75d14}

\section{The devil in the details}
\label{sec:orgdebd3b6}
...
spend the majority of Chapter \ref{org5f2b6d1} labouring over peculiarities
...
\label{sec:org09403ff}

The reader will have noticed that Chapter \ref{org5f2b6d1} raised at least
...
\label{sec:orgab4ed94}

This was touched on in \S \ref{org1c93dcc}, in response to an interesting case
...
\label{sec:orgab8e865}

Frequently, in my analysis of various peculiar behaviours in \gls{roper}'s
populations, in Chapter \ref{org5f2b6d1}, my enquiry was cut short by a simple
...
\label{sec:org9ba165d}

The experiment touched on in \ref{orgdcf09e8} remained largely tangential,
...
explains why we see it being given so great a preference in figure \ref{fig:org6c537c4}. 
...
\caption{\label{fig:org43c314b}
Performance of the \emph{Xysxim} population on the simple, linearly separable dataset first encountered in \S \ref{org566ba5c}.}
\end{figure}
...
\label{sec:orge4b6445}
\label{org245b545}
\subsection{The trouble with crossover}
\label{sec:org52b4cf1}
...
used in \S \ref{org3c3aa7f}, and demonstrate its impact on classification tasks in 
Section \ref{orgc2aa15f}. Its applicability, as we have seen, relies
...
neither the syscall preparation task (cf. \S \ref{orge84ebb6}), which uses a
singleton problem set, nor the
Snake experiment (cf. \S \ref{orgdcf09e8}), which uses a continuously
...
throughout Chapter \ref{org5f2b6d1}, in the analysis of heatmap data. 
...
figure \ref{fig:org437c559} -- whose evolutionary run bore the same configuration as the population
in which the malignant gene problem was first diagnosed (see \S \ref{org3c0fde9}).  

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/xilvyb.pdf}
\caption{\label{fig:org437c559}
...
and introducing a selective incentive for behavioural diversity (see \S \ref{orgf16f7af}) appears to yield
...
completing the syscall preparation task, outlined in \S \ref{org52e6e8b}.
...
\ref{fig:org3eeaf46} was never equaled in subsequent trials. 
...
\ref{fig:org3eeaf46}, but the results so far have shown an extraordinary improvement over
their predecessors. The performance curves for this run are shown in figure \ref{fig:orgcdfc00e}. 
...
As we can see in figure \ref{fig:org940c413}, \emph{even the linearly inseparable} 
