...
\label{sec:orgbd1126c}
\label{org7ea68ca}
\section{What is the aim of this research?}
\label{sec:orgdeed48f}
\label{org780191e}
...
here could, with minor adaptations,\footnote{Discussed in \S \ref{orgd1abfc1}.} used
to test and train \glspl{ids}, or provide one more instrument in the penetration
tester's toolbox.

\section{Who is this for?}
\label{sec:orgf45c2ed}
...
\section{Synopsis}
\label{sec:orgdfa7123}
\label{orgc1c4462}

In Chapter \ref{orgb5716c3}, I set up some of the conceptual
...
design, in Chapter \ref{org8a19aba}.

Chapter \ref{org91cc339} introduces my contribution to research in the field of
...
Chapter \ref{orgf2bf7ff} goes over a handful of experimental studies with
\gls{roper}, and consequent modifications to the design. 

Chapter \ref{orgd1abfc1} lays out some directions for future work and study on this
...
\label{sec:org065b839}
\label{org92734a2}
\subsection{1969: Benford}
\label{sec:org72ff9dd}
...
somewhat, in \S \ref{org5b7bf72}, but it remains a useful
simplification.

The main loop, outlined in Algorithm \ref{alg:evolve} and figure \ref{fig:org399806b},
...
implementing the algorithms surveyed in \S \ref{org2c8344e}.
...
harvested from the target executable. 
  \footnote{See \S \ref{orgace0491} for a sustained explanation of how return-oriented
...
in figure \ref{fig:org0c42f18}. Since we are
...
As explained in \S \ref{org5c88d7c}, it is often helpful to think of
...
harvested from the target binary (see \S \ref{org6503e1a}), and a
...
these consequences in Chapter \ref{orgf2bf7ff}.


\subsection{Crossover}
\label{sec:orgeda583a}
\label{orgf8a3e5c} 
...
\ref{org627c672}. The details of the algorithm itself can be found in
\ref{alg:crossover}.
...
Section \ref{org5076e40}. If we abstract away from the information loss
...
Section \ref{org5076e40} commute with and distribute over crossover, if we consider
...
\label{sec:orgb58f7a1}
\label{orgd730df4}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/evaluation_model.png}
\caption{\label{fig:org6793770}
High-level overview of the ontogenesis and evaluation process. The ``path diversity'' component will not be introduced until we reach \S \ref{orge29fecc}.}
\end{figure}
...
penalty to fitness, the details of which will be discussed in \S \ref{org2e50976}.
...
detail in Chapter \ref{orgf2bf7ff}. For the time being, the matter of \gls{cpu}
exceptions deserves closer comment.

\subsubsection{Failure modes and crash rates}
\label{sec:org6fc5a51}
...
of fitness function, listed in \S \ref{org2e50976} -- one way that diversity depletion often
...
binary's \texttt{.text} section are plotted in figure \ref{fig:orgdbe6a21}, to which
I will make frequent reference in the subsequent heatmap overlay images.

In the following subsections (\ref{orgb574866}, \ref{org081d870},
\ref{org4ac164a}, and \ref{orga5f7387}), I will outline each task set that
\gls{roper} was given to perform, and specify the way in which each was
incorporated into \gls{roper}'s fitness function.

In \S \ref{org6667651}, I will walk through some of the more interesting results
gleaned from each experiment, and in \S \ref{org3c23482}, I will lay out certain
...
\label{sec:org07519e6}
\label{orgb574866}

As discussed in \S\S \ref{org5076e40} and \ref{orgf8a3e5c}, when designing a genetic
...
as a component of the fitness function (see \S \ref{org2e50976} for details),
...
\gls{roper}, so long as the fitness sharing mechanism discussed in \ref{org2b9c2ee}
...
In figure \ref{fig:orgb5ad573}, we see heatmaps of the the \emph{Xeqcyv} population's
...
\ref{fig:orgff6ab2f}, is not at all surprising. The longer they spend in
...
a surprising degree of ruggedness, as we can see in figure \ref{fig:orge846604}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/wiwzuh_syscall_gaussian_3.pdf}
\caption{\label{fig:orge846604}
...
usage shown in figure \ref{fig:org71491c5}. If the output registers
...
fitness sharing (see \S \ref{org2b9c2ee}) provides a means for the evolutionary
...
and two classes (see figure \ref{fig:orgd4ed3c3}).
...
\ref{fig:org4173b75}). Its best specimen clearly relied on a single
...
\caption{\label{fig:org4173b75}
The classification on the data displayed in figure \ref{fig:orgd4ed3c3} performed by the best specimen in the \emph{Kathot} population, 35\(^{\text{th}}\) generation (execution trace inset).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kathot_fitness.pdf}
\caption{\label{fig:org68b6421}
...
\caption{\label{fig:orgfdf2c45}
Address visitation heatmap of the \emph{Fizwej} population, at season 4 on the left, and season 124, on the right, showing a dramatic decline in phenotypic diversity. Compare with the \emph{xeqcyv} and \emph{kurlig} heatmaps in figures \ref{fig:orgb5ad573} and \ref{fig:org3acb92c}.}
\end{figure}
...
of future directions for this project, in \S \ref{orgd1abfc1}. Incorporating
...
will look at more closely in \S \ref{org16752cb}) , which I stumbled
...
20,000\(^{\text{th}}\) tournament iteration (figure \ref{fig:org4ccae9e}). Due to the data that
was collected at the time being insufficiently coarse-grained, I cannot say
whether dynamics similar to those described in \S \ref{orgec2bac6} played
...
In view of the phenomena described in \S \ref{orgec2bac6}, however, this approach to regulating the crash rate may warrant revision. 

\paragraph{A proposed method for regulating malignant genes with a TTL field}
\label{sec:org695f78d}
\label{orgdc0e0aa}
...
Section \ref{org3fd7bdf}. Its attribute-class mapping is plotted
in figure \ref{fig:orgc154fd2}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_plot.pdf}
\caption{\label{fig:orgc154fd2}
...
This is what we see happening in figure \ref{fig:orga353078}, for example,
...
depth in section \ref{orgbf7d749}.
...
mechanic detailed in \S \ref{org627c672}, however: a sufficiently fragile gene
...
\ref{fig:orga353078} is a symptom of (premature) genetic convergence. 
...
\caption{\label{fig:org6e02ac7}
A trial similar to the one documented in figure \ref{fig:orgd4f6a30}, with per-class difficulty recorded, but with the fitness sharing mechanism suspended (\emph{Cazmud} population). The filled curve surrounding each mean difficulty class, here being of visible breadth only in the case of the \emph{iris virginia}, represents the standard deviation of difficulty for each exemplar class.}
\end{figure}

For this reason, I introduced into \gls{roper} the \emph{fitness sharing} mechanism 
that I have outlined in \S \ref{org2b9c2ee}. 
...
phylogenic generation of 91.3. Figure \ref{fig:orgd4f6a30} shows the course the
...
\caption{\label{fig:orgd4f6a30}
A very good run on the Iris classification task, employing the fitness sharing algorithm documented in \S \ref{org2b9c2ee} (\emph{Ragweb} population). The filled curve surrounding each mean difficulty line again represents the standard deviation of difficulty for that exemplar class.}
\end{figure}



This run can be informatively compared with the one illustrated in figure
\ref{fig:org6e02ac7}. Note the tight interbraiding of problem difficulties in
figure \ref{fig:orgd4f6a30}, as compared to their gaping -- but still, slowly,
fluctuating -- disparity in figure \ref{fig:org6e02ac7}. The ballooning standard
deviation of difficulty by class in figure \ref{fig:orgd4f6a30} also suggests
...
The specifications of the game are described above, in \S \ref{orga5f7387}.
...
in \S \ref{org92734a2}), our ``in vitro'' populations have the advantage of a gently
...
exploited process.\footnote{See the semantic correction noted in \S \ref{orgaf9cfdd}, which 
...
discussed in \S\S \ref{org081d870} and \ref{orgaf9cfdd}, but
...
The results, presented in figures \ref{fig:orgfdc4dcc} and
\ref{fig:orgde48d44}, were found to corroborate the intron hypothesis,
...
appearing in figure \ref{fig:orgfdc4dcc}, suggest that this \emph{may} be the
...
\label{sec:orgf73006d}
\label{orgd1abfc1}

\section{The devil in the details}
\label{sec:org20c767b}
...
spend the majority of Chapter \ref{orgf2bf7ff} labouring over peculiarities
...
\label{sec:org9a3d90a}

The reader will have noticed that Chapter \ref{orgf2bf7ff} raised at least
...
\label{sec:org061bf50}

This was touched on in \S \ref{orgdc0e0aa}, in response to an interesting case
...
\label{sec:org8b83809}

Frequently, in my analysis of various peculiar behaviours in \gls{roper}'s
populations, in Chapter \ref{orgf2bf7ff}, my enquiry was cut short by a simple
...
\label{sec:orga65bd61}

The experiment touched on in \ref{orgdb58742} remained largely tangential,
...
explains why we see it being given so great a preference in figure \ref{fig:org71491c5}. 
...
\caption{\label{fig:orgcf3906c}
Performance of the \emph{Xysxim} population on the simple, linearly separable dataset first encountered in \S \ref{org3fd7bdf}.}
\end{figure}
...
\label{sec:org305a13b}
\label{orgcab3bb1}
\subsection{The trouble with crossover}
\label{sec:org768aabf}
...
used in \S \ref{org2b9c2ee}, and demonstrate its impact on classification tasks in 
Section \ref{org6303ff8}. Its applicability, as we have seen, relies
...
neither the syscall preparation task (cf. \S \ref{orgaf9cfdd}), which uses a
singleton problem set, nor the
Snake experiment (cf. \S \ref{orgdb58742}), which uses a continuously
...
throughout Chapter \ref{orgf2bf7ff}, in the analysis of heatmap data. 
...
figure \ref{fig:org3bd93f8} -- whose evolutionary run bore the same configuration as the population
in which the malignant gene problem was first diagnosed (see \S \ref{orgec2bac6}).  

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/xilvyb.pdf}
\caption{\label{fig:org3bd93f8}
...
and introducing a selective incentive for behavioural diversity (see \S \ref{orge29fecc}) appears to yield
...
completing the syscall preparation task, outlined in \S \ref{org081d870}.
...
\ref{fig:orgd4f6a30} was never equaled in subsequent trials. 
...
\ref{fig:orgd4f6a30}, but the results so far have shown an extraordinary improvement over
their predecessors. The performance curves for this run are shown in figure \ref{fig:orgf1069e5}. 
...
As we can see in figure \ref{fig:org0274343}, \emph{even the linearly inseparable} 
