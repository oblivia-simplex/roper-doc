...
\label{sec:org0bb450a}
\label{orgf313159}
\section{What is the aim of this research?}
\label{sec:org1464522}
\label{orgf46bcf4}
...
here could, with minor adaptations,\footnote{Discussed in \S \ref{org1ede2ac}.} used
to test and train \glspl{ids}, or provide one more instrument in the penetration
tester's toolbox.

\section{Who is this for?}
\label{sec:orgafd1a59}
...
\section{Synopsis}
\label{sec:orgd72816f}
\label{org60e28ed}

In Chapter \ref{org2b27518}, I set up some of the conceptual
...
design, in Chapter \ref{org7abfccf}.

Chapter \ref{orge99e35c} introduces my contribution to research in the field of
...
Chapter \ref{org3ac6397} goes over a handful of experimental studies with
\gls{roper}, and consequent modifications to the design. 

Chapter \ref{org1ede2ac} lays out some directions for future work and study on this
...
the one developed by PSHAPE's author, ROPocop.\footnote{Though this goes beyond the scope of this thesis, I'm willing to bet that
  ...
  as a sheer evolutionary spandrel effect, without any explicit adaptive advantage.}

\section{Prospects for Genetic ROP-chain Crafting}
\label{sec:orgad08353}
...
This will become clear as we work through Chapter \ref{org3ac6397}.
...
\label{sec:org98107b7}
\label{org60f7efb}
\subsection{1969: Benford}
\label{sec:orgcbb6dea}
...
somewhat, in \S \ref{orgd6f970f}, but it remains a useful
...
The main loop, outlined in Algorithm \ref{alg:evolve} and figure \ref{fig:org71991d8},
...
implementing the algorithms surveyed in \S \ref{org07b1081}.
...
harvested from the target executable. 
  \footnote{See \S \ref{org9911aaf} for a sustained explanation of how return-oriented
...
in figure \ref{fig:orgcbb4c4f}. Since we are
...
As explained in \S \ref{orge15c3b4}, it is often helpful to think of
...
harvested from the target binary (see \S \ref{orga707952}), and a
...
these consequences in Chapter \ref{org3ac6397}.


\subsection{Crossover}
\label{sec:org6b6138d}
\label{orgfd7805f} 
...
\ref{org052fbed}. The details of the algorithm itself can be found in
\ref{alg:crossover}.
...
Section \ref{org62e9be1}. If we abstract away from the information loss
...
Section \ref{org62e9be1} commute with and distribute over crossover, if we consider
...
\label{sec:org89d2271}
\label{orgeaf1af2}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/evaluation_model.png}
\caption{\label{fig:org4229c22}
High-level overview of the ontogenesis and evaluation process. The ``path diversity'' component will not be introduced until we reach \S \ref{org15dd182}.}
\end{figure}
...
penalty to fitness, the details of which will be discussed in \S \ref{org3efe5d3}.
...
detail in Chapter \ref{org3ac6397}. For the time being, the matter of \gls{cpu}
exceptions deserves closer comment.

\subsubsection{Failure modes and crash rates}
\label{sec:orgfd0c52d}
...
of fitness function, listed in \S \ref{org3efe5d3} -- one way that diversity depletion often
...
binary. (In all of the experiments documented in Chapter \ref{org3ac6397},
...
binary's \texttt{.text} section are plotted in figure \ref{fig:org93a7107}, to which
I will make frequent reference in the subsequent heatmap overlay images.

In the following subsections (\ref{org1d86cfc}, \ref{org46a0ecb},
\ref{org0aecd6f}, and \ref{org440f975}), I will outline each task set that
\gls{roper} was given to perform, and specify the way in which each was
incorporated into \gls{roper}'s fitness function.

In \S \ref{org2ad1e86}, I will walk through some of the more interesting results
gleaned from each experiment, and in \S \ref{org6d52a6b}, I will lay out certain
...
\label{sec:org4305aad}
\label{org1d86cfc}

As discussed in \S\S \ref{org62e9be1} and \ref{orgfd7805f}, when designing a genetic
...
as a component of the fitness function (see \S \ref{org3efe5d3} for details),
...
\gls{roper}, so long as the fitness sharing mechanism discussed in \ref{org9115f32}
...
In figure \ref{fig:orge5b0343}, we see heatmaps of the the \emph{Xeqcyv} population's
...
\ref{fig:orgb442041}, is not at all surprising. The longer they spend in
...
a surprising degree of ruggedness, as we can see in figure \ref{fig:org12ecaea}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/wiwzuh_syscall_gaussian_3.pdf}
\caption{\label{fig:org12ecaea}
...
usage shown in figure \ref{fig:org10a126f}. If the output registers
...
fitness sharing (see \S \ref{org9115f32}) provides a means for the evolutionary
...
and two classes (see figure \ref{fig:org77939c5}).
...
\ref{fig:org66f819e}). Its best specimen clearly relied on a single
...
\caption{\label{fig:org66f819e}
The classification on the data displayed in figure \ref{fig:org77939c5} performed by the best specimen in the \emph{Kathot} population, 35\(^{\text{th}}\) generation (execution trace inset).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kathot_fitness.pdf}
\caption{\label{fig:orgb49d9af}
...
\caption{\label{fig:orgaeafac9}
Address visitation heatmap of the \emph{Fizwej} population, at season 4 on the left, and season 124, on the right, showing a dramatic decline in phenotypic diversity. Compare with the \emph{xeqcyv} and \emph{kurlig} heatmaps in figures \ref{fig:orge5b0343} and \ref{fig:orgabc6a29}.}
\end{figure}
...
of future directions for this project, in \S \ref{org1ede2ac}. Incorporating
...
will look at more closely in \S \ref{org495a6e4}) , which I stumbled
...
20,000\(^{\text{th}}\) tournament iteration (figure \ref{fig:org20dc6ed}). Due to the data that
was collected at the time being insufficiently coarse-grained, I cannot say
whether dynamics similar to those described in \S \ref{org2dd247d} played
...
In view of the phenomena described in \S \ref{org2dd247d}, however, this approach to regulating the crash rate may warrant revision. 

\paragraph{A proposed method for regulating malignant genes with a TTL field}
\label{sec:orgb8b38ac}
\label{org9bb4407}
...
Section \ref{org95e06ba}. Its attribute-class mapping is plotted
in figure \ref{fig:org7b965c2}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_plot.pdf}
\caption{\label{fig:org7b965c2}
...
This is what we see happening in figure \ref{fig:org1da3141}, for example,
...
depth in section \ref{orga30fc20}.
...
mechanic detailed in \S \ref{org052fbed}, however: a sufficiently fragile gene
...
\ref{fig:org1da3141} is a symptom of (premature) genetic convergence. 
...
\caption{\label{fig:orgfbe85da}
A trial similar to the one documented in figure \ref{fig:org7b4a180}, with per-class difficulty recorded, but with the fitness sharing mechanism suspended (\emph{Cazmud} population). The filled curve surrounding each mean difficulty class, here being of visible breadth only in the case of the \emph{iris virginia}, represents the standard deviation of difficulty for each exemplar class.}
\end{figure}

For this reason, I introduced into \gls{roper} the \emph{fitness sharing} mechanism 
that I have outlined in \S \ref{org9115f32}. 
...
phylogenic generation of 91.3. Figure \ref{fig:org7b4a180} shows the course the
...
\caption{\label{fig:org7b4a180}
A very good run on the Iris classification task, employing the fitness sharing algorithm documented in \S \ref{org9115f32} (\emph{Ragweb} population). The filled curve surrounding each mean difficulty line again represents the standard deviation of difficulty for that exemplar class.}
\end{figure}



This run can be informatively compared with the one illustrated in figure
\ref{fig:orgfbe85da}. Note the tight interbraiding of problem difficulties in
figure \ref{fig:org7b4a180}, as compared to their gaping -- but still, slowly,
fluctuating -- disparity in figure \ref{fig:orgfbe85da}. The ballooning standard
deviation of difficulty by class in figure \ref{fig:org7b4a180} also suggests
...
The specifications of the game are described above, in \S \ref{org440f975}.
...
in \S \ref{org60f7efb}), our ``in vitro'' populations have the advantage of a gently
...
exploited process.\footnote{See the semantic correction noted in \S \ref{orgb182f57}, which 
...
discussed in \S\S \ref{org46a0ecb} and \ref{orgb182f57}, but
...
The results, presented in figures \ref{fig:orgcaf6b55} and
\ref{fig:org08930e5}, were found to corroborate the intron hypothesis,
...
appearing in figure \ref{fig:orgcaf6b55}, suggest that this \emph{may} be the
...
\label{sec:orgadad4cf}
\label{org1ede2ac}

\section{The devil in the details}
\label{sec:orgdbdacb0}
...
spend the majority of Chapter \ref{org3ac6397} labouring over peculiarities
...
\label{sec:org2454b9c}

The reader will have noticed that Chapter \ref{org3ac6397} raised at least
...
\label{sec:org462457a}

This was touched on in \S \ref{org9bb4407}, in response to an interesting case
...
\label{sec:org5431672}

Frequently, in my analysis of various peculiar behaviours in \gls{roper}'s
populations, in Chapter \ref{org3ac6397}, my enquiry was cut short by a simple
...
\label{sec:org26a7a0e}

The experiment touched on in \ref{org25843d5} remained largely tangential,
...
explains why we see it being given so great a preference in figure \ref{fig:org10a126f}. 
...
\caption{\label{fig:orge905437}
Performance of the \emph{Xysxim} population on the simple, linearly separable dataset first encountered in \S \ref{org95e06ba}.}
\end{figure}
...
\label{sec:org887ffb2}
\label{org4333aea}
\subsection{The trouble with crossover}
\label{sec:orga7eff37}
...
used in \S \ref{org9115f32}, and demonstrate its impact on classification tasks in 
Section \ref{org5bfc16d}. Its applicability, as we have seen, relies
...
neither the syscall preparation task (cf. \S \ref{orgb182f57}), which uses a
singleton problem set, nor the
Snake experiment (cf. \S \ref{org25843d5}), which uses a continuously
...
throughout Chapter \ref{org3ac6397}, in the analysis of heatmap data. 
...
figure \ref{fig:org3c24018} -- whose evolutionary run bore the same configuration as the population
in which the malignant gene problem was first diagnosed (see \S \ref{org2dd247d}).  

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/xilvyb.pdf}
\caption{\label{fig:org3c24018}
...
and introducing a selective incentive for behavioural diversity (see \S \ref{org15dd182}) appears to yield
...
completing the syscall preparation task, outlined in \S \ref{org46a0ecb}.
...
\ref{fig:org7b4a180} was never equaled in subsequent trials. 
...
\ref{fig:org7b4a180}, but the results so far have shown an extraordinary improvement over
their predecessors. The performance curves for this run are shown in figure \ref{fig:org5d7ade5}. 
...
As we can see in figure \ref{fig:org6eb1067}, \emph{even the linearly inseparable} 
