% Created 2018-04-19 Thu 03:37
% Intended LaTeX compiler: pdflatex
\documentclass[12pt,glossary,twosided,openright]{dalthesis}
\usepackage{listings}
\input{header}
...
\label{sec:orgf136e3a}
\label{orga10d338}
\section{What is the aim of this research?}
\label{sec:org4893fa3}
\label{org8dce3a9}
...
here could, with minor adaptations,\footnote{Discussed in \S \ref{orge3399e8}.} used
to test and train \glspl{ids}, or provide one more instrument in the penetration
tester's toolbox.

\section{Who is this for?}
\label{sec:org225ea00}
...
\section{Synopsis}
\label{sec:org2fd4361}
\label{orgb142188}

In Chapter \ref{org5c70163}, I set up some of the conceptual
...
design, in Chapter \ref{org856fac4}.

Chapter \ref{orgd19d135} introduces my contribution to research in the field of
...
Chapter \ref{org8552b76} goes over a handful of experimental studies with
\gls{roper}, and consequent modifications to the design. 

Chapter \ref{orge3399e8} lays out some directions for future work and study on this
...
This will become clear as we work through Chapter \ref{org8552b76}.
...
\label{sec:org4574091}
\label{orgd0309cb}
\subsection{1969: Benford}
\label{sec:orgf7be991}
...
somewhat, in \S \ref{orgd718403}, but it remains a useful
...
The main loop, outlined in Algorithm \ref{alg:evolve} and figure \ref{fig:org895325f},
...
implementing the algorithms surveyed in \S \ref{org45faea9}.
...
harvested from the target executable. 
  \footnote{See \S \ref{orgd6e09f5} for a sustained explanation of how return-oriented
...
in figure \ref{fig:org47df947}. Since we are
...
As explained in \S \ref{org2e6611e}, it is often helpful to think of
...
harvested from the target binary (see \S \ref{orgb09dcd9}), and a
...
these consequences in Chapter \ref{org8552b76}.


\subsection{Crossover}
\label{sec:orge62221f}
\label{org477d331} 
...
\ref{orgbf5ac3e}. The details of the algorithm itself can be found in
\ref{alg:crossover}.
...
Section \ref{org261ec90}. If we abstract away from the information loss
...
Section \ref{org261ec90} commute with and distribute over crossover, if we consider
...
\label{sec:org4935e05}
\label{org2e57db4}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/evaluation_model.png}
\caption{\label{fig:orgaa9d2bc}
High-level overview of the ontogenesis and evaluation process. The ``path diversity'' component will not be introduced until we reach \S \ref{org2e08726}.}
\end{figure}
...
penalty to fitness, the details of which will be discussed in \S \ref{org0fd2911}.
...
detail in Chapter \ref{org8552b76}. For the time being, the matter of \gls{cpu}
exceptions deserves closer comment.

\subsubsection{Failure modes and crash rates}
\label{sec:orgdc6ab95}
...
of fitness function, listed in \S \ref{org0fd2911} -- one way that diversity depletion often
...
binary. (In all of the experiments documented in Chapter \ref{org8552b76},
...
binary's \texttt{.text} section are plotted in figure \ref{fig:org7939c1d}, to which
I will make frequent reference in the subsequent heatmap overlay images.

In the following subsections (\ref{org031c2b9}, \ref{org3c6e2d3},
\ref{org8ecac64}, and \ref{org4408696}), I will outline each task set that
\gls{roper} was given to perform, and specify the way in which each was
incorporated into \gls{roper}'s fitness function.

In \S \ref{orga351a62}, I will walk through some of the more interesting results
gleaned from each experiment, and in \S \ref{org4b96efe}, I will lay out certain
...
\label{sec:orged9162c}
\label{org031c2b9}

As discussed in \S\S \ref{org261ec90} and \ref{org477d331}, when designing a genetic
...
as a component of the fitness function (see \S \ref{org0fd2911} for details),
...
\gls{roper}, so long as the fitness sharing mechanism discussed in \ref{org7ae61d0}
...
In figure \ref{fig:org1a7fe6d}, we see heatmaps of the the \emph{Xeqcyv} population's
...
\ref{fig:org7ce2464}, is not at all surprising. The longer they spend in
...
a surprising degree of ruggedness, as we can see in figure \ref{fig:org0f245b0}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/wiwzuh_syscall_gaussian_3.pdf}
\caption{\label{fig:org0f245b0}
...
usage shown in figure \ref{fig:org1e2a4e1}. If the output registers
...
fitness sharing (see \S \ref{org7ae61d0}) provides a means for the evolutionary
...
and two classes (see figure \ref{fig:org5793590}).
...
\ref{fig:orge625076}). Its best specimen clearly relied on a single
...
\caption{\label{fig:orge625076}
The classification on the data displayed in figure \ref{fig:org5793590} performed by the best specimen in the \emph{Kathot} population, 35\(^{\text{th}}\) generation (execution trace inset).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kathot_fitness.pdf}
\caption{\label{fig:orge048c19}
...
\caption{\label{fig:orgc9bb886}
Address visitation heatmap of the \emph{Fizwej} population, at season 4 on the left, and season 124, on the right, showing a dramatic decline in phenotypic diversity. Compare with the \emph{xeqcyv} and \emph{kurlig} heatmaps in figures \ref{fig:org1a7fe6d} and \ref{fig:org20c76b5}.}
\end{figure}
...
of future directions for this project, in \S \ref{orge3399e8}. Incorporating
...
will look at more closely in \S \ref{orge89093d}) , which I stumbled
...
20,000\(^{\text{th}}\) tournament iteration (figure \ref{fig:orga36224a}). Due to the data that
was collected at the time being insufficiently coarse-grained, I cannot say
whether dynamics similar to those described in \S \ref{org4dff2ed} played
...
In view of the phenomena described in \S \ref{org4dff2ed}, however, this approach to regulating the crash rate may warrant revision. 

\paragraph{A proposed method for regulating malignant genes with a TTL field}
\label{sec:org3447ebc}
\label{org5776025}
...
Section \ref{orgad67b32}. Its attribute-class mapping is plotted
in figure \ref{fig:orgb5c985e}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_plot.pdf}
\caption{\label{fig:orgb5c985e}
...
This is what we see happening in figure \ref{fig:orgfcf7a24}, for example,
...
depth in section \ref{org3b29a43}.
...
mechanic detailed in \S \ref{orgbf5ac3e}, however: a sufficiently fragile gene
...
\ref{fig:orgfcf7a24} is a symptom of (premature) genetic convergence. 
...
\caption{\label{fig:org4fbf69f}
A trial similar to the one documented in figure \ref{fig:org8370681}, with per-class difficulty recorded, but with the fitness sharing mechanism suspended (\emph{Cazmud} population). The filled curve surrounding each mean difficulty class, here being of visible breadth only in the case of the \emph{iris virginia}, represents the standard deviation of difficulty for each exemplar class.}
\end{figure}

For this reason, I introduced into \gls{roper} the \emph{fitness sharing} mechanism 
that I have outlined in \S \ref{org7ae61d0}. 
...
phylogenic generation of 91.3. Figure \ref{fig:org8370681} shows the course the
...
\caption{\label{fig:org8370681}
A very good run on the Iris classification task, employing the fitness sharing algorithm documented in \S \ref{org7ae61d0} (\emph{Ragweb} population). The filled curve surrounding each mean difficulty line again represents the standard deviation of difficulty for that exemplar class.}
\end{figure}



This run can be informatively compared with the one illustrated in figure
\ref{fig:org4fbf69f}. Note the tight interbraiding of problem difficulties in
figure \ref{fig:org8370681}, as compared to their gaping -- but still, slowly,
fluctuating -- disparity in figure \ref{fig:org4fbf69f}. The ballooning standard
deviation of difficulty by class in figure \ref{fig:org8370681} also suggests
...
The specifications of the game are described above, in \S \ref{org4408696}.
...
in \S \ref{orgd0309cb}), our ``in vitro'' populations have the advantage of a gently
...
exploited process.\footnote{See the semantic correction noted in \S \ref{org72dd30d}, which 
...
discussed in \S\S \ref{org3c6e2d3} and \ref{org72dd30d}, but
...
The results, presented in figures \ref{fig:org08910b7} and
\ref{fig:org63a5124}, were found to corroborate the intron hypothesis,
...
appearing in figure \ref{fig:org08910b7}, suggest that this \emph{may} be the
...
\label{sec:orgd92c048}
\label{orge3399e8}

\section{The devil in the details}
\label{sec:org2f572b4}
...
spend the majority of Chapter \ref{org8552b76} labouring over peculiarities
...
\label{sec:org488bed1}

The reader will have noticed that Chapter \ref{org8552b76} raised at least
...
\label{sec:orgbef60f1}

This was touched on in \S \ref{org5776025}, in response to an interesting case
...
\label{sec:orgfc8cc7e}

Frequently, in my analysis of various peculiar behaviours in \gls{roper}'s
populations, in Chapter \ref{org8552b76}, my enquiry was cut short by a simple
...
\label{sec:org61a69f3}

The experiment touched on in \ref{orgf73c42e} remained largely tangential,
...
explains why we see it being given so great a preference in figure \ref{fig:org1e2a4e1}. 
...
\caption{\label{fig:orga8b6480}
Performance of the \emph{Xysxim} population on the simple, linearly separable dataset first encountered in \S \ref{orgad67b32}.}
\end{figure}
...
\label{sec:org21f2ba3}
\label{org3f8bdef}
\subsection{The trouble with crossover}
\label{sec:org1c01ad4}
...
used in \S \ref{org7ae61d0}, and demonstrate its impact on classification tasks in 
Section \ref{org6964670}. Its applicability, as we have seen, relies
...
neither the syscall preparation task (cf. \S \ref{org72dd30d}), which uses a
singleton problem set, nor the
Snake experiment (cf. \S \ref{orgf73c42e}), which uses a continuously
...
throughout Chapter \ref{org8552b76}, in the analysis of heatmap data. 
...
figure \ref{fig:org1df131c} -- whose evolutionary run bore the same configuration as the population
in which the malignant gene problem was first diagnosed (see \S \ref{org4dff2ed}).  

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/xilvyb.pdf}
\caption{\label{fig:org1df131c}
...
and introducing a selective incentive for behavioural diversity (see \S \ref{org2e08726}) appears to yield
...
completing the syscall preparation task, outlined in \S \ref{org3c6e2d3}.
...
\ref{fig:org8370681} was never equaled in subsequent trials. 
...
\ref{fig:org8370681}, but the results so far have shown an extraordinary improvement over
their predecessors. The performance curves for this run are shown in figure \ref{fig:org7fe5da6}. 
...
As we can see in figure \ref{fig:org9ea307a}, \emph{even the linearly inseparable} 
...
\label{sec:orga7999be}

\section{Note}
\label{sec:orgda2862f}
