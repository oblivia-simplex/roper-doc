% Created 2018-04-19 Thu 03:39
% Intended LaTeX compiler: pdflatex
\documentclass[12pt,glossary,twoside,openright]{dalthesis}
\usepackage{listings}
\input{header}
...
\label{sec:org2f0aabb}
\label{org27ba935}
\section{What is the aim of this research?}
\label{sec:org5080435}
\label{orgf2b4fde}
...
here could, with minor adaptations,\footnote{Discussed in \S \ref{orgc64eb8a}.} used
to test and train \glspl{ids}, or provide one more instrument in the penetration
tester's toolbox.

\section{Who is this for?}
\label{sec:org6d6394b}
...
\section{Synopsis}
\label{sec:orgf4dd924}
\label{org5df9bf7}

In Chapter \ref{orgc9ee090}, I set up some of the conceptual
...
design, in Chapter \ref{org65c523c}.

Chapter \ref{orgda0086e} introduces my contribution to research in the field of
...
Chapter \ref{org059f4b0} goes over a handful of experimental studies with
\gls{roper}, and consequent modifications to the design. 

Chapter \ref{orgc64eb8a} lays out some directions for future work and study on this
...
This will become clear as we work through Chapter \ref{org059f4b0}.
...
\label{sec:org0658798}
\label{org45f60fb}
\subsection{1969: Benford}
\label{sec:org8b30393}
...
somewhat, in \S \ref{org1d861d7}, but it remains a useful
...
The main loop, outlined in Algorithm \ref{alg:evolve} and figure \ref{fig:orgf1a32b3},
...
implementing the algorithms surveyed in \S \ref{orgca8bd4b}.
...
harvested from the target executable. 
  \footnote{See \S \ref{orga143df8} for a sustained explanation of how return-oriented
...
in figure \ref{fig:org8cb499a}. Since we are
...
As explained in \S \ref{org3996419}, it is often helpful to think of
...
harvested from the target binary (see \S \ref{org0bfcfef}), and a
...
these consequences in Chapter \ref{org059f4b0}.


\subsection{Crossover}
\label{sec:org3fd92ca}
\label{org52391a5} 
...
\ref{org554a37f}. The details of the algorithm itself can be found in
\ref{alg:crossover}.
...
Section \ref{org000b3e5}. If we abstract away from the information loss
...
Section \ref{org000b3e5} commute with and distribute over crossover, if we consider
...
\label{sec:org7d1af82}
\label{orgc2a3fe2}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/evaluation_model.png}
\caption{\label{fig:orgc432b4f}
High-level overview of the ontogenesis and evaluation process. The ``path diversity'' component will not be introduced until we reach \S \ref{org7cf8779}.}
\end{figure}
...
penalty to fitness, the details of which will be discussed in \S \ref{org8088af4}.
...
detail in Chapter \ref{org059f4b0}. For the time being, the matter of \gls{cpu}
exceptions deserves closer comment.

\subsubsection{Failure modes and crash rates}
\label{sec:orgfd6737b}
...
of fitness function, listed in \S \ref{org8088af4} -- one way that diversity depletion often
...
binary. (In all of the experiments documented in Chapter \ref{org059f4b0},
...
binary's \texttt{.text} section are plotted in figure \ref{fig:orgff3bc31}, to which
I will make frequent reference in the subsequent heatmap overlay images.

In the following subsections (\ref{org8caa9ab}, \ref{orgcf811cf},
\ref{orgb86fc46}, and \ref{org76ac16a}), I will outline each task set that
\gls{roper} was given to perform, and specify the way in which each was
incorporated into \gls{roper}'s fitness function.

In \S \ref{org35444d9}, I will walk through some of the more interesting results
gleaned from each experiment, and in \S \ref{org4d6569e}, I will lay out certain
...
\label{sec:org50f6a0b}
\label{org8caa9ab}

As discussed in \S\S \ref{org000b3e5} and \ref{org52391a5}, when designing a genetic
...
as a component of the fitness function (see \S \ref{org8088af4} for details),
...
\gls{roper}, so long as the fitness sharing mechanism discussed in \ref{orgbe2f4bc}
...
In figure \ref{fig:orgc45486d}, we see heatmaps of the the \emph{Xeqcyv} population's
...
\ref{fig:orgfb709dc}, is not at all surprising. The longer they spend in
...
a surprising degree of ruggedness, as we can see in figure \ref{fig:orgc842ea4}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/wiwzuh_syscall_gaussian_3.pdf}
\caption{\label{fig:orgc842ea4}
...
usage shown in figure \ref{fig:org42b2272}. If the output registers
...
fitness sharing (see \S \ref{orgbe2f4bc}) provides a means for the evolutionary
...
and two classes (see figure \ref{fig:orga22fc9b}).
...
\ref{fig:org0c136d5}). Its best specimen clearly relied on a single
...
\caption{\label{fig:org0c136d5}
The classification on the data displayed in figure \ref{fig:orga22fc9b} performed by the best specimen in the \emph{Kathot} population, 35\(^{\text{th}}\) generation (execution trace inset).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kathot_fitness.pdf}
\caption{\label{fig:org3c32626}
...
\caption{\label{fig:org9d0e472}
Address visitation heatmap of the \emph{Fizwej} population, at season 4 on the left, and season 124, on the right, showing a dramatic decline in phenotypic diversity. Compare with the \emph{xeqcyv} and \emph{kurlig} heatmaps in figures \ref{fig:orgc45486d} and \ref{fig:org63b7fb7}.}
\end{figure}
...
of future directions for this project, in \S \ref{orgc64eb8a}. Incorporating
...
will look at more closely in \S \ref{org367c18e}) , which I stumbled
...
20,000\(^{\text{th}}\) tournament iteration (figure \ref{fig:org27e21fe}). Due to the data that
was collected at the time being insufficiently coarse-grained, I cannot say
whether dynamics similar to those described in \S \ref{orgfc438e2} played
...
In view of the phenomena described in \S \ref{orgfc438e2}, however, this approach to regulating the crash rate may warrant revision. 

\paragraph{A proposed method for regulating malignant genes with a TTL field}
\label{sec:org0110e26}
\label{orgccc6837}
...
Section \ref{orge9903a1}. Its attribute-class mapping is plotted
in figure \ref{fig:orgc9cc5bd}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_plot.pdf}
\caption{\label{fig:orgc9cc5bd}
...
This is what we see happening in figure \ref{fig:org161cbfd}, for example,
...
depth in section \ref{org587f93b}.
...
mechanic detailed in \S \ref{org554a37f}, however: a sufficiently fragile gene
...
\ref{fig:org161cbfd} is a symptom of (premature) genetic convergence. 
...
\caption{\label{fig:org763713c}
A trial similar to the one documented in figure \ref{fig:org59b7fe8}, with per-class difficulty recorded, but with the fitness sharing mechanism suspended (\emph{Cazmud} population). The filled curve surrounding each mean difficulty class, here being of visible breadth only in the case of the \emph{iris virginia}, represents the standard deviation of difficulty for each exemplar class.}
\end{figure}

For this reason, I introduced into \gls{roper} the \emph{fitness sharing} mechanism 
that I have outlined in \S \ref{orgbe2f4bc}. 
...
phylogenic generation of 91.3. Figure \ref{fig:org59b7fe8} shows the course the
...
\caption{\label{fig:org59b7fe8}
A very good run on the Iris classification task, employing the fitness sharing algorithm documented in \S \ref{orgbe2f4bc} (\emph{Ragweb} population). The filled curve surrounding each mean difficulty line again represents the standard deviation of difficulty for that exemplar class.}
\end{figure}



This run can be informatively compared with the one illustrated in figure
\ref{fig:org763713c}. Note the tight interbraiding of problem difficulties in
figure \ref{fig:org59b7fe8}, as compared to their gaping -- but still, slowly,
fluctuating -- disparity in figure \ref{fig:org763713c}. The ballooning standard
deviation of difficulty by class in figure \ref{fig:org59b7fe8} also suggests
...
The specifications of the game are described above, in \S \ref{org76ac16a}.
...
in \S \ref{org45f60fb}), our ``in vitro'' populations have the advantage of a gently
...
exploited process.\footnote{See the semantic correction noted in \S \ref{orgd4654b1}, which 
...
discussed in \S\S \ref{orgcf811cf} and \ref{orgd4654b1}, but
...
The results, presented in figures \ref{fig:org9ee69e3} and
\ref{fig:org3a6674c}, were found to corroborate the intron hypothesis,
...
appearing in figure \ref{fig:org9ee69e3}, suggest that this \emph{may} be the
...
\label{sec:orgeef9319}
\label{orgc64eb8a}

\section{The devil in the details}
\label{sec:orgdc35413}
...
spend the majority of Chapter \ref{org059f4b0} labouring over peculiarities
...
\label{sec:org499d41e}

The reader will have noticed that Chapter \ref{org059f4b0} raised at least
...
\label{sec:orgc94749e}

This was touched on in \S \ref{orgccc6837}, in response to an interesting case
...
\label{sec:org0b499b9}

Frequently, in my analysis of various peculiar behaviours in \gls{roper}'s
populations, in Chapter \ref{org059f4b0}, my enquiry was cut short by a simple
...
\label{sec:org0ae5b04}

The experiment touched on in \ref{org3239796} remained largely tangential,
...
explains why we see it being given so great a preference in figure \ref{fig:org42b2272}. 
...
\caption{\label{fig:orgdd10d28}
Performance of the \emph{Xysxim} population on the simple, linearly separable dataset first encountered in \S \ref{orge9903a1}.}
\end{figure}
...
\label{sec:orgd0d73a4}
\label{org2b543a7}
\subsection{The trouble with crossover}
\label{sec:org7c2ae91}
...
used in \S \ref{orgbe2f4bc}, and demonstrate its impact on classification tasks in 
Section \ref{org43917b5}. Its applicability, as we have seen, relies
...
neither the syscall preparation task (cf. \S \ref{orgd4654b1}), which uses a
singleton problem set, nor the
Snake experiment (cf. \S \ref{org3239796}), which uses a continuously
...
throughout Chapter \ref{org059f4b0}, in the analysis of heatmap data. 
...
figure \ref{fig:orgf885b20} -- whose evolutionary run bore the same configuration as the population
in which the malignant gene problem was first diagnosed (see \S \ref{orgfc438e2}).  

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/xilvyb.pdf}
\caption{\label{fig:orgf885b20}
...
and introducing a selective incentive for behavioural diversity (see \S \ref{org7cf8779}) appears to yield
...
completing the syscall preparation task, outlined in \S \ref{orgcf811cf}.
...
\ref{fig:org59b7fe8} was never equaled in subsequent trials. 
...
\ref{fig:org59b7fe8}, but the results so far have shown an extraordinary improvement over
their predecessors. The performance curves for this run are shown in figure \ref{fig:org8852f64}. 
...
As we can see in figure \ref{fig:org41cb297}, \emph{even the linearly inseparable} 
...
\label{sec:orgc9a81f1}

\section{Note}
\label{sec:org7e4c8de}
