...
\label{sec:org0151008}
\label{org87c32aa}
\section{What is the aim of this research?}
\label{sec:orgb62e85e}
\label{org716940f}
...
here could, with minor adaptations,\footnote{Discussed in \S \ref{orgb5b6da2}.} used
to test and train \glspl{ids}, or provide one more instrument in the penetration
tester's toolbox.

\section{Who is this for?}
\label{sec:org0cfd1a4}
...
\section{Synopsis}
\label{sec:orga065475}
\label{org30b4227}

In Chapter \ref{org6dbbaf7}, I set up some of the conceptual
...
design, in Chapter \ref{org5cf84c3}.

Chapter \ref{org0e4fee0} introduces my contribution to research in the field of
...
Chapter \ref{org9322748} goes over a handful of experimental studies with
\gls{roper}, and consequent modifications to the design. 

Chapter \ref{orgb5b6da2} lays out some directions for future work and study on this
...
This will become clear as we work through Chapter \ref{org9322748}.
...
\label{sec:org308cedb}
\label{org32f2a2d}
\subsection{1969: Benford}
\label{sec:org19fc9ef}
...
somewhat, in \S \ref{org3e56a90}, but it remains a useful
...
The main loop, outlined in Algorithm \ref{alg:evolve} and figure \ref{fig:org662f3a2},
...
implementing the algorithms surveyed in \S \ref{org4e9f67c}.
...
harvested from the target executable. 
  \footnote{See \S \ref{org796c27d} for a sustained explanation of how return-oriented
...
in figure \ref{fig:org6d8de58}. Since we are
...
As explained in \S \ref{orgdc823fe}, it is often helpful to think of
...
harvested from the target binary (see \S \ref{orgb5cd9c2}), and a
...
these consequences in Chapter \ref{org9322748}.


\subsection{Crossover}
\label{sec:org7746c72}
\label{org247809f} 
...
\ref{orgd7c221f}. The details of the algorithm itself can be found in
\ref{alg:crossover}.
...
Section \ref{orgbf19d39}. If we abstract away from the information loss
...
Section \ref{orgbf19d39} commute with and distribute over crossover, if we consider
...
\label{sec:org982a01f}
\label{org86846a0}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/evaluation_model.png}
\caption{\label{fig:orga1c110c}
High-level overview of the ontogenesis and evaluation process. The ``path diversity'' component will not be introduced until we reach \S \ref{org97d65da}.}
\end{figure}
...
penalty to fitness, the details of which will be discussed in \S \ref{org086771a}.
...
detail in Chapter \ref{org9322748}. For the time being, the matter of \gls{cpu}
exceptions deserves closer comment.

\subsubsection{Failure modes and crash rates}
\label{sec:org839b857}
...
of fitness function, listed in \S \ref{org086771a} -- one way that diversity depletion often
...
binary. (In all of the experiments documented in Chapter \ref{org9322748},
...
binary's \texttt{.text} section are plotted in figure \ref{fig:org3f8eed8}, to which
I will make frequent reference in the subsequent heatmap overlay images.

In the following subsections (\ref{org5b55d8c}, \ref{orgb81f8e2},
\ref{orge865c03}, and \ref{org6523620}), I will outline each task set that
\gls{roper} was given to perform, and specify the way in which each was
incorporated into \gls{roper}'s fitness function.

In \S \ref{orgca30ead}, I will walk through some of the more interesting results
gleaned from each experiment, and in \S \ref{org8b83361}, I will lay out certain
...
\label{sec:orgfe09739}
\label{org5b55d8c}

As discussed in \S\S \ref{orgbf19d39} and \ref{org247809f}, when designing a genetic
...
as a component of the fitness function (see \S \ref{org086771a} for details),
...
\gls{roper}, so long as the fitness sharing mechanism discussed in \ref{org19a0441}
...
In figure \ref{fig:org430ce44}, we see heatmaps of the the \emph{Xeqcyv}\index{Xeqcyv} population's
...
\ref{fig:org84fec78}, is not at all surprising. The longer they spend in
...
a surprising degree of ruggedness, as we can see in figure \ref{fig:orgf4878b0}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/wiwzuh_syscall_gaussian_3.pdf}
\caption{\label{fig:orgf4878b0}
...
usage shown in figure \ref{fig:orgb5a5e48}. If the output registers
...
fitness sharing (see \S \ref{org19a0441}) provides a means for the evolutionary
...
and two classes (see figure \ref{fig:orgaccbde6}).
...
\ref{fig:org3189905}). Its best specimen clearly relied on a single
...
\caption{\label{fig:org3189905}
The classification on the data displayed in figure \ref{fig:orgaccbde6} performed by the best specimen in the \emph{Kathot}\index{Kathot} population, 35\(^{\text{th}}\) generation (execution trace inset).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kathot_fitness.pdf}
\caption{\label{fig:org118a240}
...
\caption{\label{fig:orgd2287c8}
Address visitation heatmap of the \emph{Fizwej}\index{Fizwej} population, at season 4 on the left, and season 124, on the right, showing a dramatic decline in phenotypic diversity. Compare with the \emph{xeqcyv} and \emph{kurlig} heatmaps in figures \ref{fig:org430ce44} and \ref{fig:org222b08c}.}
\end{figure}
...
of future directions for this project, in \S \ref{orgb5b6da2}. Incorporating
...
will look at more closely in \S \ref{org101bad1}) , which I stumbled
...
20,000\(^{\text{th}}\) tournament iteration (figure \ref{fig:org6f95404}). Due to the data that
was collected at the time being insufficiently coarse-grained, I cannot say
whether dynamics similar to those described in \S \ref{orgfac7d85} played
...
In view of the phenomena described in \S \ref{orgfac7d85}, however, this approach to regulating the crash rate may warrant revision. 

\paragraph{A proposed method for regulating malignant genes with a TTL field}
\label{sec:orgbde5bf2}
\label{orgbc5c6f4}
...
Section \ref{org65346de}. Its attribute-class mapping is plotted
in figure \ref{fig:org08eaeb7}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_plot.pdf}
\caption{\label{fig:org08eaeb7}
...
This is what we see happening in figure \ref{fig:org9fc1fc7}, for example,
...
depth in section \ref{org102363c}.
...
mechanic detailed in \S \ref{orgd7c221f}, however: a sufficiently fragile gene
...
\ref{fig:org9fc1fc7} is a symptom of (premature) genetic convergence. 
...
\caption{\label{fig:orgca9e180}
A trial similar to the one documented in figure \ref{fig:orgb0ed4d0}, with per-class difficulty recorded, but with the fitness sharing mechanism suspended (\emph{Cazmud}\index{Cazmud} population). The filled curve surrounding each mean difficulty class, here being of visible breadth only in the case of the \emph{iris virginia}, represents the standard deviation of difficulty for each exemplar class.}
\end{figure}

For this reason, I introduced into \gls{roper} the \emph{fitness sharing} mechanism 
that I have outlined in \S \ref{org19a0441}. 
...
phylogenic generation of 91.3. Figure \ref{fig:orgb0ed4d0} shows the course the
...
\caption{\label{fig:orgb0ed4d0}
A very good run on the Iris classification task, employing the fitness sharing algorithm documented in \S \ref{org19a0441} (\emph{Ragweb}\index{Ragweb} population). The filled curve surrounding each mean difficulty line again represents the standard deviation of difficulty for that exemplar class.}
\end{figure}



This run can be informatively compared with the one illustrated in figure
\ref{fig:orgca9e180}. Note the tight interbraiding of problem difficulties in
figure \ref{fig:orgb0ed4d0}, as compared to their gaping -- but still, slowly,
fluctuating -- disparity in figure \ref{fig:orgca9e180}. The ballooning standard
deviation of difficulty by class in figure \ref{fig:orgb0ed4d0} also suggests
...
The specifications of the game are described above, in \S \ref{org6523620}.
...
in \S \ref{org32f2a2d}), our ``in vitro'' populations have the advantage of a gently
...
exploited process.\footnote{See the semantic correction noted in \S \ref{org86b6d9e}, which 
...
discussed in \S\S \ref{orgb81f8e2} and \ref{org86b6d9e}, but
...
The results, presented in figures \ref{fig:org2ad3602} and
\ref{fig:orgfb207c4}, were found to corroborate the intron hypothesis,
...
appearing in figure \ref{fig:org2ad3602}, suggest that this \emph{may} be the
...
\label{sec:org7143f5f}
\label{orgb5b6da2}

\section{The devil in the details}
\label{sec:org2583700}
...
spend the majority of Chapter \ref{org9322748} labouring over peculiarities
...
\label{sec:orgfbd5a26}

The reader will have noticed that Chapter \ref{org9322748} raised at least
...
\label{sec:org28429c5}

This was touched on in \S \ref{orgbc5c6f4}, in response to an interesting case
...
\label{sec:org58f977e}

Frequently, in my analysis of various peculiar behaviours in \gls{roper}'s
populations, in Chapter \ref{org9322748}, my enquiry was cut short by a simple
...
\label{sec:org5149a0f}

The experiment touched on in \ref{org650d68b} remained largely tangential,
...
explains why we see it being given so great a preference in figure \ref{fig:orgb5a5e48}. 
...
\caption{\label{fig:org91e9907}
Performance of the \emph{Xysxim}\index{Xysxim} population on the simple, linearly separable dataset first encountered in \S \ref{org65346de}.}
\end{figure}
...
\label{sec:orgb9a5096}
\label{orgbf90c1e}
\subsection{The trouble with crossover}
\label{sec:org4c9af2f}
...
used in \S \ref{org19a0441}, and demonstrate its impact on classification tasks in 
Section \ref{org20284e8}. Its applicability, as we have seen, relies
...
neither the syscall preparation task (cf. \S \ref{org86b6d9e}), which uses a
singleton problem set, nor the
Snake experiment (cf. \S \ref{org650d68b}), which uses a continuously
...
throughout Chapter \ref{org9322748}, in the analysis of heatmap data. 
...
figure \ref{fig:org894e40e} -- whose evolutionary run bore the same configuration as the population
in which the malignant gene problem was first diagnosed (see \S \ref{orgfac7d85}).  

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/xilvyb.pdf}
\caption{\label{fig:org894e40e}
...
and introducing a selective incentive for behavioural diversity (see \S \ref{org97d65da}) appears to yield
...
completing the syscall preparation task, outlined in \S \ref{orgb81f8e2}.
...
\ref{fig:orgb0ed4d0} was never equaled in subsequent trials. 
...
\ref{fig:orgb0ed4d0}, but the results so far have shown an extraordinary improvement over
their predecessors. The performance curves for this run are shown in figure \ref{fig:org969ffd4}. 
...
As we can see in figure \ref{fig:org3c11388}, \emph{even the linearly inseparable} 
...
\label{sec:orge5bc259}
\listofalgorithms

\chapter{Population Parameters}
\label{sec:orge3bbf67}

\section{Note}
\label{sec:org480ec7e}
