...
\label{sec:org1a346c6}
\label{org772cd91}
\section{What is the aim of this research?}
\label{sec:org03a59eb}
\label{orge24ab9f}
...
here could, with minor adaptations,\footnote{Discussed in Section \ref{orgbca48cb}.} used
to test and train \glspl{ids}, or provide one more instrument in the penetration
tester's toolbox.

\section{Who is this for?}
\label{sec:org742bfae}
...
\section{Synopsis}
\label{sec:org46a26d6}
\label{org56971fd}

In Chapter \ref{orgeb1c21b}, I set up some of the conceptual
...
design, in Chapter \ref{org8059333}.

Chapter \ref{orgea335a7} introduces my contribution to research in the field of
...
Chapter \ref{org5835ba7} goes over a handful of experimental studies with
\gls{roper}, and consequent modifications to the design. 

Chapter \ref{orgbca48cb} lays out some directions for future work and study on this
...
\label{sec:orged34055}
\label{org4bf636b}
\subsection{1969: Benford}
\label{sec:orgd704844}
...
somewhat, in Section \ref{orgde7e5a6}, but it remains a useful
simplification.
...
the system moves through the space of possible solutions'' (\cite{banzhaf98}, 144).
...
operators are detailed in Section \ref{orgbc7949a}.
...
implementing the algorithms surveyed in Section \ref{org5cb189f}.
...
harvested from the target executable. 
  \footnote{See Section \ref{orgf346df2} for a sustained explanation of how return-oriented
...
As explained in Section \ref{org60e2070}, it is often helpful to think of
...
harvested from the target binary (see Section \ref{orgac62a01}), and a
...
these consequences in Chapter \ref{org5835ba7}.


\subsection{Crossover}
\label{sec:org9f0ca40}
\label{orgfdc6c68} 
...
\ref{orga87e7c5}. The details of the algorithm itself can be found in
\ref{alg:crossover}.
...
Section \ref{org53e0c19}. If we abstract away from the information loss
...
Section \ref{org53e0c19} commute with and distribute over crossover, if we consider
...
penalty to fitness, the details of which will be discussed in Section \ref{org8989480}.
...
detail in Chapter \ref{org5835ba7}. For the time being, the matter of \gls{cpu}
exceptions deserves closer comment.

\subsubsection{Failure modes and crash rates}
\label{sec:org924f230}
...
of fitness function, listed in Section \ref{org8989480} -- one way that diversity depletion often
...
binary's \texttt{.text} section are plotted in figure \ref{fig:org881a8fa}, to which
I will make frequent reference in the subsequent heatmap overlay images.

In the following subsections (\ref{org8897ba5}, \ref{org0c6e88c},
\ref{org8811c99}, and \ref{orgc13b46a}), I will outline each task set that
\gls{roper} was given to perform, and specify the way in which each was
incorporated into \gls{roper}'s fitness function.

In Section \ref{org2f9074a}, I will walk through some of the more interesting results
gleaned from each experiment, and in Section \ref{orgaa55d05}, I will lay out certain
...
\label{sec:orgbca40df}
\label{org8897ba5}

As discussed in Sections \ref{org53e0c19} and \ref{orgfdc6c68}, when designing a genetic
...
as a component of the fitness function (see Section \ref{org8989480} for details),
...
\gls{roper}, so long as the fitness sharing mechanism discussed in \ref{orgaaa97fd}
...
In figure \ref{fig:org9f6b303}, we see heatmaps of the the \emph{xeqcyv} population's
...
\ref{fig:org8f182aa}, is not at all surprising. The longer they spend in
...
string, ``/tmp/flashXXXX'', which, for its part, is known to reside at \texttt{0x1f62f}. 
...
a surprising degree of ruggedness, as we can see in figure \ref{fig:org48eca57}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/wiwzuh_syscall_gaussian_3.pdf}
\caption{\label{fig:org48eca57}
...
usage shown in figure \ref{fig:org6657781}. If the output registers
...
fitness sharing (see Section \ref{orgaaa97fd}) provides a means for the evolutionary
...
and two classes (see figure \ref{fig:orgd4de17d}).
...
\ref{fig:org7579644}). Its best specimen clearly relied on a single
...
\caption{\label{fig:org7579644}
The classification on the data displayed in figure \ref{fig:orgd4de17d} performed by the best specimen in the \emph{kathot} population, 35\(^{\text{th}}\) generation (execution trace inset).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kathot_fitness.pdf}
\caption{\label{fig:orgdd5501e}
...
\caption{\label{fig:orgc193361}
Address visitation heatmap of the \emph{fizwej} population, at season 4 on the left, and season 124, on the right, showing a dramatic decline in phenotypic diversity. Compare with the \emph{xeqcyv} and \emph{kurlig} heatmaps in figures \ref{fig:org9f6b303} and \ref{fig:orge9b43da}.}
\end{figure}
...
of future directions for this project, in Section \ref{orgbca48cb}. Incorporating
...
will look at more closely in Section \ref{org41a2dde}) , which I stumbled
...
20,000\(^{\text{th}}\) tournament iteration (figure \ref{fig:org3586e30}). Due to the data that
was collected at the time being insufficiently coarse-grained, I cannot say
whether dynamics similar to those described in Section \ref{orga7484ab} played
...
In view of the phenomena described in Section \ref{orga7484ab}, however, this approach to regulating the crash rate may warrant revision. 

\paragraph{A proposed method for regulating malignant genes with a TTL field}
\label{sec:org2629881}
\label{org9df365b}
...
Section \ref{org5400964}. Its attribute-class mapping is plotted
in figure \ref{fig:org9f84947}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_plot.pdf}
\caption{\label{fig:org9f84947}
...
This is what we see happening in figure \ref{fig:org7108937}, for example,
...
depth in section \ref{org70dd858}.
...
mechanic detailed in Section \ref{orga87e7c5}, however: a sufficiently fragile gene
...
\ref{fig:org7108937} is a symptom of (premature) genetic convergence. 
...
\caption{\label{fig:org63cfb2a}
A trial similar to the one documented in figure \ref{fig:org35953ab}, with per-class difficulty recorded, but with the fitness sharing mechanism suspended (\emph{cazmud} population). The filled curve surrounding each mean difficulty class, here being of visible breadth only in the case of the \emph{iris virginia}, represents the standard deviation of difficulty for each exemplar class.}
\end{figure}

For this reason, I introduced into \gls{roper} the \emph{fitness sharing} mechanism 
that I have outlined in Section \ref{orgaaa97fd}. 
...
phylogenic generation of 91.3. Figure \ref{fig:org35953ab} shows the course the
...
\caption{\label{fig:org35953ab}
A very good run on the Iris classification task, employing the fitness sharing algorithm documented in Section \ref{orgaaa97fd} (\emph{ragweb} population). The filled curve surrounding each mean difficulty line again represents the standard deviation of difficulty for that exemplar class.}
\end{figure}



This run can be informatively compared with the one illustrated in figure
\ref{fig:org63cfb2a}. Note the tight interbraiding of problem difficulties in
figure \ref{fig:org35953ab}, as compared to their gaping -- but still, slowly,
fluctuating -- disparity in figure \ref{fig:org63cfb2a}. The ballooning standard
deviation of difficulty by class in figure \ref{fig:org35953ab} also suggests
...
The specifications of the game are described above, in Section \ref{orgc13b46a}.
...
in Section \ref{org4bf636b}), our ``in vitro'' populations have the advantage of a gently
...
discussed in Sections \ref{org0c6e88c} and \ref{org2bf8177}, but
...
The results, presented in figures \ref{fig:orgf135eb4} and
\ref{fig:org3474f6f}, were found to corroborate the intron hypothesis,
...
appearing in figure \ref{fig:orgf135eb4}, suggest that this \emph{may} be the
...
\label{sec:orge5cae61}
\label{orgbca48cb}

\section{The devil in the details}
\label{sec:orgc02b443}
...
spend the majority of Chapter \ref{org5835ba7} labouring over peculiarities
...
\label{sec:org3b435b7}

The reader will have noticed that Chapter \ref{org5835ba7} raised at least
...
\label{sec:orgb8cb3e3}

This was touched on in Section \ref{org9df365b}, in response to an interesting case
...
\label{sec:orgbcd33a2}

Frequently, in my analysis of various peculiar behaviours in \gls{roper}'s
populations, in Chapter \ref{org5835ba7}, my enquiry was cut short by a simple
...
\label{sec:org7a52a33}

The experiment touched on in \ref{orgcca9f83} remained largely tangential,
...
\appendix
\chapter{Appendix: Adjustment to the Classifier Algorithm}
\label{sec:orgdb35710}
\section{Disadvantages of the bid-based algorithm in ROPER}
\label{sec:orgc75dacf}
As we can see in figure \ref{fig:org6657781}, the distribution of register use
...
\section{Using R0 to decide}
\label{sec:org612f949}
...
explains why we see it being given so great a preference in figure \ref{fig:org6657781}. 
