...
\label{sec:org22b0efa}
\label{org2c76518}
\section{What is the aim of this research?}
\label{sec:org4b37b8a}
\label{orgbfe6659}
...
here could, with minor adaptations,\footnote{Discussed in \S \ref{orge205e59}.} used
to test and train \glspl{ids}, or provide one more instrument in the penetration
tester's toolbox.

\section{Who is this for?}
\label{sec:orge0fc923}
...
\section{Synopsis}
\label{sec:orge9526c1}
\label{org8dbf3ad}

In Chapter \ref{orgc062003}, I set up some of the conceptual
...
design, in Chapter \ref{org724f983}.

Chapter \ref{orgaef96ca} introduces my contribution to research in the field of
...
Chapter \ref{org946e0dd} goes over a handful of experimental studies with
\gls{roper}, and consequent modifications to the design. 

Chapter \ref{orge205e59} lays out some directions for future work and study on this
...
This will become clear as we work through Chapter \ref{org946e0dd}.
...
\label{sec:org53d8dcb}
\label{org654aef7}
\subsection{1969: Benford}
\label{sec:org9178bf2}
...
somewhat, in \S \ref{org3ff836a}, but it remains a useful
...
The main loop, outlined in Algorithm \ref{alg:evolve} and figure \ref{fig:orge13591b},
...
implementing the algorithms surveyed in \S \ref{orgaff010b}.
...
harvested from the target executable. 
  \footnote{See \S \ref{org1baaf70} for a sustained explanation of how return-oriented
...
in figure \ref{fig:orgfe518a6}. Since we are
...
As explained in \S \ref{org35f0b54}, it is often helpful to think of
...
harvested from the target binary (see \S \ref{orgd3fc388}), and a
...
these consequences in Chapter \ref{org946e0dd}.


\subsection{Crossover}
\label{sec:org4977f84}
\label{org5f3ee33} 
...
\ref{orge6ff9cf}. The details of the algorithm itself can be found in
\ref{alg:crossover}.
...
Section \ref{org431469c}. If we abstract away from the information loss
...
Section \ref{org431469c} commute with and distribute over crossover, if we consider
...
\label{sec:org8953a70}
\label{org8fbffd4}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/evaluation_model.png}
\caption{\label{fig:orgc32cd7d}
High-level overview of the ontogenesis and evaluation process. The ``path diversity'' component will not be introduced until we reach \S \ref{orgda6e947}.}
\end{figure}
...
penalty to fitness, the details of which will be discussed in \S \ref{org08f1760}.
...
detail in Chapter \ref{org946e0dd}. For the time being, the matter of \gls{cpu}
exceptions deserves closer comment.

\subsubsection{Failure modes and crash rates}
\label{sec:org3fb07fb}
...
of fitness function, listed in \S \ref{org08f1760} -- one way that diversity depletion often
...
binary. (In all of the experiments documented in Chapter \ref{org946e0dd},
...
binary's \texttt{.text} section are plotted in figure \ref{fig:org5f1f230}, to which
I will make frequent reference in the subsequent heatmap overlay images.

In the following subsections (\ref{org506724d}, \ref{orgf1d9acd},
\ref{org9efed47}, and \ref{org5d3e783}), I will outline each task set that
\gls{roper} was given to perform, and specify the way in which each was
incorporated into \gls{roper}'s fitness function.

In \S \ref{orgfaa6dd4}, I will walk through some of the more interesting results
gleaned from each experiment, and in \S \ref{orgf367ac6}, I will lay out certain
...
\label{sec:orgbf71477}
\label{org506724d}

As discussed in \S\S \ref{org431469c} and \ref{org5f3ee33}, when designing a genetic
...
as a component of the fitness function (see \S \ref{org08f1760} for details),
...
\gls{roper}, so long as the fitness sharing mechanism discussed in \ref{orgd9b7a80}
...
In figure \ref{fig:org92bdd23}, we see heatmaps of the the \emph{Xeqcyv}\index{Xeqcyv} population's
...
\ref{fig:orgdb0c219}, is not at all surprising. The longer they spend in
...
a surprising degree of ruggedness, as we can see in figure \ref{fig:org3f7a327}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/wiwzuh_syscall_gaussian_3.pdf}
\caption{\label{fig:org3f7a327}
...
usage shown in figure \ref{fig:orge291c05}. If the output registers
...
fitness sharing (see \S \ref{orgd9b7a80}) provides a means for the evolutionary
...
and two classes (see figure \ref{fig:org141b3ae}).
...
\ref{fig:org8d788fd}). Its best specimen clearly relied on a single
...
\caption{\label{fig:org8d788fd}
The classification on the data displayed in figure \ref{fig:org141b3ae} performed by the best specimen in the \emph{Kathot}\index{Kathot} population, 35\(^{\text{th}}\) generation (execution trace inset).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kathot_fitness.pdf}
\caption{\label{fig:org38ce3d6}
...
\caption{\label{fig:org4de131d}
Address visitation heatmap of the \emph{Fizwej}\index{Fizwej} population, at season 4 on the left, and season 124, on the right, showing a dramatic decline in phenotypic diversity. Compare with the \emph{xeqcyv} and \emph{kurlig} heatmaps in figures \ref{fig:org92bdd23} and \ref{fig:org845db86}.}
\end{figure}
...
of future directions for this project, in \S \ref{orge205e59}. Incorporating
...
will look at more closely in \S \ref{org316867a}) , which I stumbled
...
20,000\(^{\text{th}}\) tournament iteration (figure \ref{fig:org73f294f}). Due to the data that
was collected at the time being insufficiently coarse-grained, I cannot say
whether dynamics similar to those described in \S \ref{orge6c0f73} played
...
In view of the phenomena described in \S \ref{orge6c0f73}, however, this approach to regulating the crash rate may warrant revision. 

\paragraph{A proposed method for regulating malignant genes with a TTL field}
\label{sec:org0fbefec}
\label{orga2f3e78}
...
Section \ref{org1a88e19}. Its attribute-class mapping is plotted
in figure \ref{fig:org4912634}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_plot.pdf}
\caption{\label{fig:org4912634}
...
This is what we see happening in figure \ref{fig:orgd71ad9f}, for example,
...
depth in section \ref{org7b1c424}.
...
mechanic detailed in \S \ref{orge6ff9cf}, however: a sufficiently fragile gene
...
\ref{fig:orgd71ad9f} is a symptom of (premature) genetic convergence. 
...
\caption{\label{fig:org51d7b09}
A trial similar to the one documented in figure \ref{fig:org3704904}, with per-class difficulty recorded, but with the fitness sharing mechanism suspended (\emph{Cazmud}\index{Cazmud} population). The filled curve surrounding each mean difficulty class, here being of visible breadth only in the case of the \emph{iris virginia}, represents the standard deviation of difficulty for each exemplar class.}
\end{figure}

For this reason, I introduced into \gls{roper} the \emph{fitness sharing} mechanism 
that I have outlined in \S \ref{orgd9b7a80}. 
...
phylogenic generation of 91.3. Figure \ref{fig:org3704904} shows the course the
...
\caption{\label{fig:org3704904}
A very good run on the Iris classification task, employing the fitness sharing algorithm documented in \S \ref{orgd9b7a80} (\emph{Ragweb}\index{Ragweb} population). The filled curve surrounding each mean difficulty line again represents the standard deviation of difficulty for that exemplar class.}
\end{figure}



This run can be informatively compared with the one illustrated in figure
\ref{fig:org51d7b09}. Note the tight interbraiding of problem difficulties in
figure \ref{fig:org3704904}, as compared to their gaping -- but still, slowly,
fluctuating -- disparity in figure \ref{fig:org51d7b09}. The ballooning standard
deviation of difficulty by class in figure \ref{fig:org3704904} also suggests
...
The specifications of the game are described above, in \S \ref{org5d3e783}.
...
in \S \ref{org654aef7}), our ``in vitro'' populations have the advantage of a gently
...
exploited process.\footnote{See the semantic correction noted in \S \ref{org1fbd371}, which 
...
discussed in \S\S \ref{orgf1d9acd} and \ref{org1fbd371}, but
...
The results, presented in figures \ref{fig:org8d3471a} and
\ref{fig:org35524e9}, were found to corroborate the intron hypothesis,
...
appearing in figure \ref{fig:org8d3471a}, suggest that this \emph{may} be the
...
\label{sec:org91d0dd5}
\label{orge205e59}

\section{The devil in the details}
\label{sec:org724f14d}
...
spend the majority of Chapter \ref{org946e0dd} labouring over peculiarities
...
\label{sec:orgad99e57}

The reader will have noticed that Chapter \ref{org946e0dd} raised at least
...
\label{sec:org65acee7}

This was touched on in \S \ref{orga2f3e78}, in response to an interesting case
...
\label{sec:org804542f}

Frequently, in my analysis of various peculiar behaviours in \gls{roper}'s
populations, in Chapter \ref{org946e0dd}, my enquiry was cut short by a simple
...
\label{sec:orgfd1493c}

The experiment touched on in \ref{org1b69035} remained largely tangential,
...
explains why we see it being given so great a preference in figure \ref{fig:orge291c05}. 
...
\caption{\label{fig:org6849e04}
Performance of the \emph{Xysxim}\index{Xysxim} population on the simple, linearly separable dataset first encountered in \S \ref{org1a88e19}.}
\end{figure}
...
\label{sec:org4195fb9}
\label{org346c0a9}
\subsection{The trouble with crossover}
\label{sec:org13d5d77}
...
used in \S \ref{orgd9b7a80}, and demonstrate its impact on classification tasks in 
Section \ref{org75aabc0}. Its applicability, as we have seen, relies
...
neither the syscall preparation task (cf. \S \ref{org1fbd371}), which uses a
singleton problem set, nor the
Snake experiment (cf. \S \ref{org1b69035}), which uses a continuously
...
throughout Chapter \ref{org946e0dd}, in the analysis of heatmap data. 
...
figure \ref{fig:org42fb08c} -- whose evolutionary run bore the same configuration as the population
in which the malignant gene problem was first diagnosed (see \S \ref{orge6c0f73}).  

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/xilvyb.pdf}
\caption{\label{fig:org42fb08c}
...
and introducing a selective incentive for behavioural diversity (see \S \ref{orgda6e947}) appears to yield
...
completing the syscall preparation task, outlined in \S \ref{orgf1d9acd}.
...
\ref{fig:org3704904} was never equaled in subsequent trials. 
...
\ref{fig:org3704904}, but the results so far have shown an extraordinary improvement over
their predecessors. The performance curves for this run are shown in figure \ref{fig:org44dcfc2}. 
...
As we can see in figure \ref{fig:org7b7422d}, \emph{even the linearly inseparable} 
...
\label{sec:orgdc565c2}

\section{Note}
\label{sec:org4465295}
