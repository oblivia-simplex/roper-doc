...
\label{sec:org12b6fcd}
\label{org506f7bd}
\section{What is the aim of this research?}
\label{sec:org1d36958}
\label{org0986993}
...
here could, with minor adaptations,\footnote{Discussed in \S \ref{org6bf21c6}.} used
to test and train \glspl{ids}, or provide one more instrument in the penetration
tester's toolbox.

\section{Who is this for?}
\label{sec:org82bdfbf}
...
\section{Synopsis}
\label{sec:org80ba4a8}
\label{orge97ec62}

In Chapter \ref{org59f19f2}, I set up some of the conceptual
...
design, in Chapter \ref{org0b2170b}.

Chapter \ref{orgcfe79fc} introduces my contribution to research in the field of
...
Chapter \ref{org15620f4} goes over a handful of experimental studies with
\gls{roper}, and consequent modifications to the design. 

Chapter \ref{org6bf21c6} lays out some directions for future work and study on this
...
This will become clear as we work through Chapter \ref{org15620f4}.
...
\label{sec:org01ead7b}
\label{org69b759a}
\subsection{1969: Benford}
\label{sec:org3b5d63a}
...
somewhat, in \S \ref{org625d252}, but it remains a useful
...
The main loop, outlined in Algorithm \ref{alg:evolve} and figure \ref{fig:org4dba183},
...
implementing the algorithms surveyed in \S \ref{orgbd77224}.
...
harvested from the target executable. 
  \footnote{See \S \ref{orgf38fffd} for a sustained explanation of how return-oriented
...
in figure \ref{fig:org6944b17}. Since we are
...
As explained in \S \ref{org2962183}, it is often helpful to think of
...
harvested from the target binary (see \S \ref{orge59452a}), and a
...
these consequences in Chapter \ref{org15620f4}.


\subsection{Crossover}
\label{sec:org1db4b0d}
\label{org0151edb} 
...
\ref{orgb169313}. The details of the algorithm itself can be found in
\ref{alg:crossover}.
...
Section \ref{orgb531d6b}. If we abstract away from the information loss
...
Section \ref{orgb531d6b} commute with and distribute over crossover, if we consider
...
\label{sec:org5c0c09b}
\label{org10e98d5}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/evaluation_model.png}
\caption{\label{fig:org6aff74a}
High-level overview of the ontogenesis and evaluation process. The ``path diversity'' component will not be introduced until we reach \S \ref{orge43e72e}.}
\end{figure}
...
penalty to fitness, the details of which will be discussed in \S \ref{org00609a4}.
...
detail in Chapter \ref{org15620f4}. For the time being, the matter of \gls{cpu}
exceptions deserves closer comment.

\subsubsection{Failure modes and crash rates}
\label{sec:org2fa82ca}
...
of fitness function, listed in \S \ref{org00609a4} -- one way that diversity depletion often
...
binary. (In all of the experiments documented in Chapter \ref{org15620f4},
...
binary's \texttt{.text} section are plotted in figure \ref{fig:orgecf36de}, to which
I will make frequent reference in the subsequent heatmap overlay images.

In the following subsections (\ref{org312466a}, \ref{org831e046},
\ref{org2de6e6c}, and \ref{org5abd386}), I will outline each task set that
\gls{roper} was given to perform, and specify the way in which each was
incorporated into \gls{roper}'s fitness function.

In \S \ref{orgb90790a}, I will walk through some of the more interesting results
gleaned from each experiment, and in \S \ref{orgad5a52a}, I will lay out certain
...
\label{sec:org99b85c9}
\label{org312466a}

As discussed in \S\S \ref{orgb531d6b} and \ref{org0151edb}, when designing a genetic
...
as a component of the fitness function (see \S \ref{org00609a4} for details),
...
\gls{roper}, so long as the fitness sharing mechanism discussed in \ref{org7e69859}
...
In figure \ref{fig:org8bdd1f7}, we see heatmaps of the the \emph{Xeqcyv} population's
...
\ref{fig:orged7c1f4}, is not at all surprising. The longer they spend in
...
a surprising degree of ruggedness, as we can see in figure \ref{fig:org4230aa2}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/wiwzuh_syscall_gaussian_3.pdf}
\caption{\label{fig:org4230aa2}
...
usage shown in figure \ref{fig:orgf428307}. If the output registers
...
fitness sharing (see \S \ref{org7e69859}) provides a means for the evolutionary
...
and two classes (see figure \ref{fig:orgb1cb8ad}).
...
\ref{fig:org62b8774}). Its best specimen clearly relied on a single
...
\caption{\label{fig:org62b8774}
The classification on the data displayed in figure \ref{fig:orgb1cb8ad} performed by the best specimen in the \emph{Kathot} population, 35\(^{\text{th}}\) generation (execution trace inset).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kathot_fitness.pdf}
\caption{\label{fig:org9714e38}
...
\caption{\label{fig:orgf687ba9}
Address visitation heatmap of the \emph{Fizwej} population, at season 4 on the left, and season 124, on the right, showing a dramatic decline in phenotypic diversity. Compare with the \emph{xeqcyv} and \emph{kurlig} heatmaps in figures \ref{fig:org8bdd1f7} and \ref{fig:org651f1d4}.}
\end{figure}
...
of future directions for this project, in \S \ref{org6bf21c6}. Incorporating
...
will look at more closely in \S \ref{org4e290d8}) , which I stumbled
...
20,000\(^{\text{th}}\) tournament iteration (figure \ref{fig:org71cd9e1}). Due to the data that
was collected at the time being insufficiently coarse-grained, I cannot say
whether dynamics similar to those described in \S \ref{org499ec70} played
...
In view of the phenomena described in \S \ref{org499ec70}, however, this approach to regulating the crash rate may warrant revision. 

\paragraph{A proposed method for regulating malignant genes with a TTL field}
\label{sec:org14e0b51}
\label{org3665edb}
...
Section \ref{orgdaf22af}. Its attribute-class mapping is plotted
in figure \ref{fig:orgb33a892}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_plot.pdf}
\caption{\label{fig:orgb33a892}
...
This is what we see happening in figure \ref{fig:org6af17cd}, for example,
...
depth in section \ref{org16e689a}.
...
mechanic detailed in \S \ref{orgb169313}, however: a sufficiently fragile gene
...
\ref{fig:org6af17cd} is a symptom of (premature) genetic convergence. 
...
\caption{\label{fig:org548793f}
A trial similar to the one documented in figure \ref{fig:orgf63c365}, with per-class difficulty recorded, but with the fitness sharing mechanism suspended (\emph{Cazmud} population). The filled curve surrounding each mean difficulty class, here being of visible breadth only in the case of the \emph{iris virginia}, represents the standard deviation of difficulty for each exemplar class.}
\end{figure}

For this reason, I introduced into \gls{roper} the \emph{fitness sharing} mechanism 
that I have outlined in \S \ref{org7e69859}. 
...
phylogenic generation of 91.3. Figure \ref{fig:orgf63c365} shows the course the
...
\caption{\label{fig:orgf63c365}
A very good run on the Iris classification task, employing the fitness sharing algorithm documented in \S \ref{org7e69859} (\emph{Ragweb} population). The filled curve surrounding each mean difficulty line again represents the standard deviation of difficulty for that exemplar class.}
\end{figure}



This run can be informatively compared with the one illustrated in figure
\ref{fig:org548793f}. Note the tight interbraiding of problem difficulties in
figure \ref{fig:orgf63c365}, as compared to their gaping -- but still, slowly,
fluctuating -- disparity in figure \ref{fig:org548793f}. The ballooning standard
deviation of difficulty by class in figure \ref{fig:orgf63c365} also suggests
...
The specifications of the game are described above, in \S \ref{org5abd386}.
...
in \S \ref{org69b759a}), our ``in vitro'' populations have the advantage of a gently
...
exploited process.\footnote{See the semantic correction noted in \S \ref{org45ecbb9}, which 
...
discussed in \S\S \ref{org831e046} and \ref{org45ecbb9}, but
...
The results, presented in figures \ref{fig:org3be123b} and
\ref{fig:orgcbde444}, were found to corroborate the intron hypothesis,
...
appearing in figure \ref{fig:org3be123b}, suggest that this \emph{may} be the
...
\label{sec:orgb38fc47}
\label{org6bf21c6}

\section{The devil in the details}
\label{sec:org5643fbb}
...
spend the majority of Chapter \ref{org15620f4} labouring over peculiarities
...
\label{sec:org912127f}

The reader will have noticed that Chapter \ref{org15620f4} raised at least
...
\label{sec:orgdf36dd5}

This was touched on in \S \ref{org3665edb}, in response to an interesting case
...
\label{sec:org17fa034}

Frequently, in my analysis of various peculiar behaviours in \gls{roper}'s
populations, in Chapter \ref{org15620f4}, my enquiry was cut short by a simple
...
\label{sec:org7743051}

The experiment touched on in \ref{org9732b22} remained largely tangential,
...
explains why we see it being given so great a preference in figure \ref{fig:orgf428307}. 
...
\caption{\label{fig:orgab54523}
Performance of the \emph{Xysxim} population on the simple, linearly separable dataset first encountered in \S \ref{orgdaf22af}.}
\end{figure}
...
\label{sec:orgcf738dc}
\label{orge37e23e}
\subsection{The trouble with crossover}
\label{sec:org556b06f}
...
used in \S \ref{org7e69859}, and demonstrate its impact on classification tasks in 
Section \ref{org7504f43}. Its applicability, as we have seen, relies
...
neither the syscall preparation task (cf. \S \ref{org45ecbb9}), which uses a
singleton problem set, nor the
Snake experiment (cf. \S \ref{org9732b22}), which uses a continuously
...
throughout Chapter \ref{org15620f4}, in the analysis of heatmap data. 
...
figure \ref{fig:org76375f4} -- whose evolutionary run bore the same configuration as the population
in which the malignant gene problem was first diagnosed (see \S \ref{org499ec70}).  

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/xilvyb.pdf}
\caption{\label{fig:org76375f4}
...
and introducing a selective incentive for behavioural diversity (see \S \ref{orge43e72e}) appears to yield
...
completing the syscall preparation task, outlined in \S \ref{org831e046}.
...
\ref{fig:orgf63c365} was never equaled in subsequent trials. 
...
\ref{fig:orgf63c365}, but the results so far have shown an extraordinary improvement over
their predecessors. The performance curves for this run are shown in figure \ref{fig:org8b85f3f}. 
...
As we can see in figure \ref{fig:orgc70e252}, \emph{even the linearly inseparable} 
...
\label{sec:org1f503d8}

\section{Note}
\label{sec:orgddd3877}
