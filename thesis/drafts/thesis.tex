...
\label{sec:org43fa98e}
\label{org031bf77}
\section{What is the aim of this research?}
\label{sec:org0222fe1}
\label{org08dfe9d}
...
here could, with minor adaptations,\footnote{Discussed in \S \ref{org377f38a}.} used
to test and train \glspl{ids}, or provide one more instrument in the penetration
tester's toolbox.

\section{Who is this for?}
\label{sec:org2bb5a5f}
...
\section{Synopsis}
\label{sec:org6ee8e49}
\label{orgbe295e3}

In Chapter \ref{org106f82c}, I set up some of the conceptual
...
design, in Chapter \ref{orgd50b460}.

Chapter \ref{orgc5988fd} introduces my contribution to research in the field of
...
Chapter \ref{orga28514e} goes over a handful of experimental studies with
\gls{roper}, and consequent modifications to the design. 

Chapter \ref{org377f38a} lays out some directions for future work and study on this
...
This will become clear as we work through Chapter \ref{orga28514e}.
...
\label{sec:orgeb5c748}
\label{orgad91293}
\subsection{1969: Benford}
\label{sec:orgfff9eb2}
...
somewhat, in \S \ref{org3eaffeb}, but it remains a useful
...
The main loop, outlined in Algorithm \ref{alg:evolve} and figure \ref{fig:org9b6cba2},
...
implementing the algorithms surveyed in \S \ref{org6ed30dc}.
...
harvested from the target executable. 
  \footnote{See \S \ref{org7d9eeb7} for a sustained explanation of how return-oriented
...
in figure \ref{fig:org59412f0}. Since we are
...
As explained in \S \ref{org74022f0}, it is often helpful to think of
...
harvested from the target binary (see \S \ref{orga39fc97}), and a
...
these consequences in Chapter \ref{orga28514e}.


\subsection{Crossover}
\label{sec:orge1557b2}
\label{org0319929} 
...
\ref{org7419686}. The details of the algorithm itself can be found in
\ref{alg:crossover}.
...
Section \ref{orga46d340}. If we abstract away from the information loss
...
Section \ref{orga46d340} commute with and distribute over crossover, if we consider
...
\label{sec:orgbed7024}
\label{org279844e}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/evaluation_model.png}
\caption{\label{fig:org8602466}
High-level overview of the ontogenesis and evaluation process. The ``path diversity'' component will not be introduced until we reach \S \ref{org3a28209}.}
\end{figure}
...
penalty to fitness, the details of which will be discussed in \S \ref{org7aec379}.
...
detail in Chapter \ref{orga28514e}. For the time being, the matter of \gls{cpu}
exceptions deserves closer comment.

\subsubsection{Failure modes and crash rates}
\label{sec:orgff89716}
...
of fitness function, listed in \S \ref{org7aec379} -- one way that diversity depletion often
...
binary. (In all of the experiments documented in Chapter \ref{orga28514e},
...
binary's \texttt{.text} section are plotted in figure \ref{fig:orge19e44f}, to which
I will make frequent reference in the subsequent heatmap overlay images.

In the following subsections (\ref{orgcf322dd}, \ref{org791f403},
\ref{org1782530}, and \ref{orgda823d7}), I will outline each task set that
\gls{roper} was given to perform, and specify the way in which each was
incorporated into \gls{roper}'s fitness function.

In \S \ref{orgda3a939}, I will walk through some of the more interesting results
gleaned from each experiment, and in \S \ref{orgfb1f86d}, I will lay out certain
...
\label{sec:org6f08b87}
\label{orgcf322dd}

As discussed in \S\S \ref{orga46d340} and \ref{org0319929}, when designing a genetic
...
as a component of the fitness function (see \S \ref{org7aec379} for details),
...
\gls{roper}, so long as the fitness sharing mechanism discussed in \ref{org0bc30bd}
...
In figure \ref{fig:org23c88a4}, we see heatmaps of the the \emph{Xeqcyv} population's
...
\ref{fig:orgd162bf3}, is not at all surprising. The longer they spend in
...
a surprising degree of ruggedness, as we can see in figure \ref{fig:org1491dcb}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/wiwzuh_syscall_gaussian_3.pdf}
\caption{\label{fig:org1491dcb}
...
usage shown in figure \ref{fig:orgd8e6a4e}. If the output registers
...
fitness sharing (see \S \ref{org0bc30bd}) provides a means for the evolutionary
...
and two classes (see figure \ref{fig:org9c5836d}).
...
\ref{fig:org0b4d055}). Its best specimen clearly relied on a single
...
\caption{\label{fig:org0b4d055}
The classification on the data displayed in figure \ref{fig:org9c5836d} performed by the best specimen in the \emph{Kathot} population, 35\(^{\text{th}}\) generation (execution trace inset).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kathot_fitness.pdf}
\caption{\label{fig:org2e0d9d9}
...
\caption{\label{fig:orgbeafb8f}
Address visitation heatmap of the \emph{Fizwej} population, at season 4 on the left, and season 124, on the right, showing a dramatic decline in phenotypic diversity. Compare with the \emph{xeqcyv} and \emph{kurlig} heatmaps in figures \ref{fig:org23c88a4} and \ref{fig:org9921f4f}.}
\end{figure}
...
of future directions for this project, in \S \ref{org377f38a}. Incorporating
...
will look at more closely in \S \ref{org9854e1c}) , which I stumbled
...
20,000\(^{\text{th}}\) tournament iteration (figure \ref{fig:orgff9341c}). Due to the data that
was collected at the time being insufficiently coarse-grained, I cannot say
whether dynamics similar to those described in \S \ref{orga884a78} played
...
In view of the phenomena described in \S \ref{orga884a78}, however, this approach to regulating the crash rate may warrant revision. 

\paragraph{A proposed method for regulating malignant genes with a TTL field}
\label{sec:org379f81d}
\label{org3fcf0c3}
...
Section \ref{org6a6c601}. Its attribute-class mapping is plotted
in figure \ref{fig:org89c1671}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_plot.pdf}
\caption{\label{fig:org89c1671}
...
This is what we see happening in figure \ref{fig:org470f5dd}, for example,
...
depth in section \ref{org101caf3}.
...
mechanic detailed in \S \ref{org7419686}, however: a sufficiently fragile gene
...
\ref{fig:org470f5dd} is a symptom of (premature) genetic convergence. 
...
\caption{\label{fig:org5e4d0db}
A trial similar to the one documented in figure \ref{fig:org2f1aaa3}, with per-class difficulty recorded, but with the fitness sharing mechanism suspended (\emph{Cazmud} population). The filled curve surrounding each mean difficulty class, here being of visible breadth only in the case of the \emph{iris virginia}, represents the standard deviation of difficulty for each exemplar class.}
\end{figure}

For this reason, I introduced into \gls{roper} the \emph{fitness sharing} mechanism 
that I have outlined in \S \ref{org0bc30bd}. 
...
phylogenic generation of 91.3. Figure \ref{fig:org2f1aaa3} shows the course the
...
\caption{\label{fig:org2f1aaa3}
A very good run on the Iris classification task, employing the fitness sharing algorithm documented in \S \ref{org0bc30bd} (\emph{Ragweb} population). The filled curve surrounding each mean difficulty line again represents the standard deviation of difficulty for that exemplar class.}
\end{figure}



This run can be informatively compared with the one illustrated in figure
\ref{fig:org5e4d0db}. Note the tight interbraiding of problem difficulties in
figure \ref{fig:org2f1aaa3}, as compared to their gaping -- but still, slowly,
fluctuating -- disparity in figure \ref{fig:org5e4d0db}. The ballooning standard
deviation of difficulty by class in figure \ref{fig:org2f1aaa3} also suggests
...
The specifications of the game are described above, in \S \ref{orgda823d7}.
...
in \S \ref{orgad91293}), our ``in vitro'' populations have the advantage of a gently
...
exploited process.\footnote{See the semantic correction noted in \S \ref{org8065759}, which 
...
discussed in \S\S \ref{org791f403} and \ref{org8065759}, but
...
The results, presented in figures \ref{fig:orgf12c80b} and
\ref{fig:org92b0fcb}, were found to corroborate the intron hypothesis,
...
appearing in figure \ref{fig:orgf12c80b}, suggest that this \emph{may} be the
...
\label{sec:orgec2015f}
\label{org377f38a}

\section{The devil in the details}
\label{sec:org0afc889}
...
spend the majority of Chapter \ref{orga28514e} labouring over peculiarities
...
\label{sec:org1ebae84}

The reader will have noticed that Chapter \ref{orga28514e} raised at least
...
\label{sec:org6f93dab}

This was touched on in \S \ref{org3fcf0c3}, in response to an interesting case
...
\label{sec:org8cb4639}

Frequently, in my analysis of various peculiar behaviours in \gls{roper}'s
populations, in Chapter \ref{orga28514e}, my enquiry was cut short by a simple
...
\label{sec:orgcae0f38}

The experiment touched on in \ref{orgd6c8c15} remained largely tangential,
...
explains why we see it being given so great a preference in figure \ref{fig:orgd8e6a4e}. 
...
\caption{\label{fig:orgea38baa}
Performance of the \emph{Xysxim} population on the simple, linearly separable dataset first encountered in \S \ref{org6a6c601}.}
\end{figure}
...
\label{sec:orgc4609c3}
\label{org823c88b}
\subsection{The trouble with crossover}
\label{sec:org468fc6e}
...
used in \S \ref{org0bc30bd}, and demonstrate its impact on classification tasks in 
Section \ref{orgb25e9d6}. Its applicability, as we have seen, relies
...
neither the syscall preparation task (cf. \S \ref{org8065759}), which uses a
singleton problem set, nor the
Snake experiment (cf. \S \ref{orgd6c8c15}), which uses a continuously
...
throughout Chapter \ref{orga28514e}, in the analysis of heatmap data. 
...
figure \ref{fig:org7325412} -- whose evolutionary run bore the same configuration as the population
in which the malignant gene problem was first diagnosed (see \S \ref{orga884a78}).  

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/xilvyb.pdf}
\caption{\label{fig:org7325412}
...
and introducing a selective incentive for behavioural diversity (see \S \ref{org3a28209}) appears to yield
...
completing the syscall preparation task, outlined in \S \ref{org791f403}.
...
\ref{fig:org2f1aaa3} was never equaled in subsequent trials. 
...
\ref{fig:org2f1aaa3}, but the results so far have shown an extraordinary improvement over
their predecessors. The performance curves for this run are shown in figure \ref{fig:org639f06b}. 
...
As we can see in figure \ref{fig:org6428d92}, \emph{even the linearly inseparable} 
