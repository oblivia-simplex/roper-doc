...
\label{sec:orgab0e232}
\label{orga3c35a6}
\section{What is the aim of this research?}
\label{sec:org50265f6}
\label{orgf039999}
...
here could, with minor adaptations,\footnote{Discussed in \S \ref{orgec3b507}.} used
to test and train \glspl{ids}, or provide one more instrument in the penetration
tester's toolbox.

\section{Who is this for?}
\label{sec:orgf9b2a3f}
...
\footnote{In the immortal words of Pastor Manul Laphroaig \cite{laphroaig13}:
\begin{quote}
...
\section{Synopsis}
\label{sec:org7ca14d1}
\label{orgb46fe31}

In Chapter \ref{org44f6f32}, I set up some of the conceptual
...
design, in Chapter \ref{org9d2183b}.

Chapter \ref{orgfd39d0b} introduces my contribution to research in the field of
...
Chapter \ref{org54d1f94} goes over a handful of experimental studies with
\gls{roper}, and consequent modifications to the design. 

Chapter \ref{orgec3b507} lays out some directions for future work and study on this
...
This will become clear as we work through Chapter \ref{org54d1f94}.
...
\label{sec:org6960f17}
\label{org45e28db}
\subsection{1969: Benford}
\label{sec:org6a02289}
...
\subsection{2002: MetaPHOR}
\label{sec:orgc455e1f}
...
``genetic''. MetaPHOR\footnote{CARO identification: W32/Simile, \{W32,Linux\}/Simile.D, Etap.D} is a highly sophisticated metamorphic virus, capable of
...
somewhat, in \S \ref{orgf9f94c1}, but it remains a useful
...
The main loop, outlined in Algorithm \ref{alg:evolve} and figure \ref{fig:org2b6add4},
...
implementing the algorithms surveyed in \S \ref{orge5985bd}.
...
harvested from the target executable. 
  \footnote{See \S \ref{orgbd0d04c} for a sustained explanation of how return-oriented
...
in figure \ref{fig:orgee8c60a}. Since we are
...
As explained in \S \ref{org1f2e280}, it is often helpful to think of
...
harvested from the target binary (see \S \ref{org1bd041c}), and a
...
these consequences in Chapter \ref{org54d1f94}.


\subsection{Crossover}
\label{sec:org805981c}
\label{org809a47a} 
...
genetic exploration and selective pressure, which \aimention{Lee Spector} Lee Spector calls
...
\ref{orgfe8d7c0}. The details of the algorithm itself can be found in
\ref{alg:crossover}.
...
Section \ref{orgd50c10c}. If we abstract away from the information loss
...
Section \ref{orgd50c10c} commute with and distribute over crossover, if we consider
...
\label{sec:orgca4d7b0}
\label{org30df863}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/evaluation_model.png}
\caption{\label{fig:org04bdd7c}
High-level overview of the ontogenesis and evaluation process. The ``path diversity'' component will not be introduced until we reach \S \ref{org4f8d491}.}
\end{figure}
...
penalty to fitness, the details of which will be discussed in \S \ref{orga79562c}.
...
detail in Chapter \ref{org54d1f94}. For the time being, the matter of \gls{cpu}
exceptions deserves closer comment.

\subsubsection{Failure modes and crash rates}
\label{sec:org029aba4}
...
of fitness function, listed in \S \ref{orga79562c} -- one way that diversity depletion often
...
multi-architecture CPU emulator framework'', exposes the \gls{cpu}
...
binary. (In all of the experiments documented in Chapter \ref{org54d1f94},
...
binary's \texttt{.text} section are plotted in figure \ref{fig:orgdc4fed9}, to which
I will make frequent reference in the subsequent heatmap overlay images.

In the following subsections (\ref{orgae12952}, \ref{org27fe059},
\ref{orgdbb4683}, and \ref{org19f8e5b}), I will outline each task set that
\gls{roper} was given to perform, and specify the way in which each was
incorporated into \gls{roper}'s fitness function.

In \S \ref{orgac73636}, I will walk through some of the more interesting results
gleaned from each experiment, and in \S \ref{org32456bf}, I will lay out certain
...
\label{sec:org73379f0}
\label{orgae12952}

As discussed in \S\S \ref{orgd50c10c} and \ref{org809a47a}, when designing a genetic
...
as a component of the fitness function (see \S \ref{orga79562c} for details),
...
\gls{roper}, so long as the fitness sharing mechanism discussed in \ref{org748e518}
...
I've borrowed a trick used by Curtis Yarvin \aimention{Curtis Yarvin} and 
Galen Wolff-Pauly \aimention{Galen Wolff-Pauly} in designing the \emph{Urbit} namespace,
 and used strings of six random
letters, following the pattern ``\texttt{consonant vowel consonant consonant vowel
consonant}''\footnote{This superficially resembles the namespace allocated to
``stars'' in \emph{Urbit}, but is a bit less constrained: Urbit's namespace has room for
\(2^{16}-1\) names, ours accommodates \((20 * 6 * 20)^2 \approx 2^{11.2}\).}. Using
these labels over the course of this chapter lets me avoid circumlocutions like
``as seen in the second population discussed in \S \$N'', and has the further
advantage of setting up a self-documenting correspondence between the analyses
in this chapter and my system of log files.


\section{Initial Findings}
\label{sec:org3bd67e2}
\label{orgac73636}

\subsection{Surveying the landscape with the null task}
\label{sec:org2972d23}
\label{orgf3d4b61}
...
\label{sec:org6c2f52c}
\label{org1ecf788}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kurlig_kafka_performance.pdf}
\caption{\label{fig:orgcd7288b}
Performance metrics over \emph{Kurlig}\index{Kurlig} population of 2048, evolving under the absurd fitness function. Crash rate and stray address rate map to left vertical axis, while mean genome length and mean instructions executed map to the right.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kurlig_heatmap_4_212_montage.pdf}
\caption{\label{fig:orga7eeaff}
Address visitation heatmap over the tomato-RT-N18U process memory, by the \emph{Kurlig}\index{Kurlig} population, evolving under the absurd fitness function with no crash penalty. Season 4 heatmap displayed on the left, season 212 on the right.}
\end{figure}
...
In figure \ref{fig:org62db289}, we see heatmaps of the the \emph{Xeqcyv}\index{Xeqcyv} population's
...
\label{sec:orgf03d047}
\label{org1618878}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/xeqcyv_kafka_performance.pdf}
\caption{\label{fig:org28bd9a6}
Performance metrics over \emph{Xeqcyv}\index{Xeqcyv} population, evolving under absurd fitness function with crash penalty. Crash rate and stray address rate map to the left vertical axis, while mean genome length and mean instructions executed map to the right.}
\end{figure}
...
\caption{\label{fig:org62db289}
Address-visitation heatmap over the tomato-RT-N18U-httpd process memory, by the \emph{Xeqcyv}\index{Xeqcyv} population, under the ``Kafkaesque'' fitness function: Season 4 on the left, Season 212 on the right. Blue swaths indicate where the harvested gadgets lie, red and magenta swaths indicate addresses visited by creatures in the population in execution.}
\end{figure}


The tendency of these populations to shrink from execution, as shown in figure
\ref{fig:org28bd9a6}, is not at all surprising. The longer they spend in
...
a surprising degree of ruggedness, as we can see in figure \ref{fig:org781f371}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/wiwzuh_syscall_gaussian_3.pdf}
\caption{\label{fig:org781f371}
Gaussian approximation of the fitness landscape across the population and over evolutionary time, for the \emph{Wiwzuh}\index{Wiwzuh} population, evolving \texttt{execv("/tmp/flashXXXXXX", ["/tmp/flashXXXXXX"], NULL)} syscall \gls{rop} chains in tomato-RT-N18U-httpd, yielding a specimen with perfect (0.0) fitness.}
\end{figure}
...
usage shown in figure \ref{fig:org90e8c2e}. If the output registers
...
fitness sharing (see \S \ref{org748e518}) provides a means for the evolutionary
...
and two classes (see figure \ref{fig:orgf55278e}).
...
\ref{fig:orgadb0fcb}). Its best specimen clearly relied on a single
...
\caption{\label{fig:orgadb0fcb}
The classification on the data displayed in figure \ref{fig:orgf55278e} performed by the best specimen in the \emph{Kathot}\index{Kathot} population, 35\(^{\text{th}}\) generation (execution trace inset).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kathot_fitness.pdf}
\caption{\label{fig:orgbe5f1fb}
Performance profile of \emph{Kathot}\index{Kathot} population, with gadgets in tomato-RT-N18U-httpd, on the ``two simple blobs'' classification problem.}
\end{figure}

\paragraph{A case study of malignancy in the \emph{Fizwej}\index{Fizwej} population}
\label{sec:orge4fe4ae}
\label{org9addff0}
...
from the \emph{Fizwej}\index{Fizwej} population, by the time of its 64\(^{\text{th}}\) season, that had this
...
\caption{\label{fig:orgf4ba2c4}
A genetic union of a harmful phenotypic trait with an advantageous one, in the \emph{Fizwej}\index{Fizwej} population. As before, gadgets are separated by blank lines.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/fizwej-badgenes.pdf}
\caption{\label{fig:org87d374b}
The evolutionary dead-end of the \emph{Fizwej}\index{Fizwej} population, with the spread of the ``\texttt{0000ad04 stmib r1, \{r3, sl\}}'' gene.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/fizwej_S124_heatmap.pdf}
\caption{\label{fig:orge33039e}
Address visitation heatmap of the \emph{Fizwej}\index{Fizwej} population, at season 4 on the left, and season 124, on the right, showing a dramatic decline in phenotypic diversity. Compare with the \emph{xeqcyv} and \emph{kurlig} heatmaps in figures \ref{fig:org62db289} and \ref{fig:orga7eeaff}.}
\end{figure}
...
of future directions for this project, in \S \ref{orgec3b507}. Incorporating
...
will look at more closely in \S \ref{org630bae9}) , which I stumbled
...
20,000\(^{\text{th}}\) tournament iteration (figure \ref{fig:org2c11e86}). Due to the data that
was collected at the time being insufficiently coarse-grained, I cannot say
whether dynamics similar to those described in \S \ref{org9addff0} played
...
In view of the phenomena described in \S \ref{org9addff0}, however, this approach to regulating the crash rate may warrant revision. 

\paragraph{A proposed method for regulating malignant genes with a TTL field}
\label{sec:org49ec599}
\label{org7a40002}
...
Section \ref{org86edf9f}. Its attribute-class mapping is plotted
in figure \ref{fig:org059392b}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_plot.pdf}
\caption{\label{fig:org059392b}
...
This is what we see happening in figure \ref{fig:orgfec5a4e}, for example,
...
depth in section \ref{org5c09869}.
...
mechanic detailed in \S \ref{orgfe8d7c0}, however: a sufficiently fragile gene
...
\ref{fig:orgfec5a4e} is a symptom of (premature) genetic convergence. 
...
\caption{\label{fig:org876cddc}
A trial similar to the one documented in figure \ref{fig:org6f0270e}, with per-class difficulty recorded, but with the fitness sharing mechanism suspended (\emph{Cazmud}\index{Cazmud} population). The filled curve surrounding each mean difficulty class, here being of visible breadth only in the case of the \emph{iris virginia}, represents the standard deviation of difficulty for each exemplar class.}
\end{figure}

For this reason, I introduced into \gls{roper} the \emph{fitness sharing} mechanism 
that I have outlined in \S \ref{org748e518}. 
...
phylogenic generation of 91.3. Figure \ref{fig:org6f0270e} shows the course the
...
\caption{\label{fig:org6f0270e}
A very good run on the Iris classification task, employing the fitness sharing algorithm documented in \S \ref{org748e518} (\emph{Ragweb}\index{Ragweb} population). The filled curve surrounding each mean difficulty line again represents the standard deviation of difficulty for that exemplar class.}
\end{figure}



This run can be informatively compared with the one illustrated in figure
\ref{fig:org876cddc}. Note the tight interbraiding of problem difficulties in
figure \ref{fig:org6f0270e}, as compared to their gaping -- but still, slowly,
fluctuating -- disparity in figure \ref{fig:org876cddc}. The ballooning standard
deviation of difficulty by class in figure \ref{fig:org6f0270e} also suggests
...
The specifications of the game are described above, in \S \ref{org19f8e5b}.
...
of Snake in figure \ref{fig:orgbbce6f5}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/snek_misjax_montage_on_white.png}
\caption{\label{fig:orgbbce6f5}
Time-lapse rendering of one of the fitter specimens encountered in the Snake trials (\emph{Misjax}\index{Misjax} population). Snake segments: \texttt{[]}, apples: \texttt{()}, cacti: \texttt{\#\#}.}
\end{figure}



\section{Intron Pressure, Self-Modifying Payloads, and Extended Gadgetry}
\label{sec:orga7a96dc}
\label{org32456bf}
...
in \S \ref{org45e28db}), our ``in vitro'' populations have the advantage of a gently
...
exploited process.\footnote{See the semantic correction noted in \S \ref{org5a73cdb}, which 
...
discussed in \S\S \ref{org27fe059} and \ref{org5a73cdb}, but
...
The results, presented in figures \ref{fig:org032193b} and
\ref{fig:org7cb40e7}, were found to corroborate the intron hypothesis,
...
appearing in figure \ref{fig:org032193b}, suggest that this \emph{may} be the
...
\caption{\label{fig:org032193b}
Plot of EDI frequency to stray rate, crash rate, and the ratio of gadgets run in the \emph{hyzqyp}, \emph{megkek}, \emph{qatjaq} and \emph{Rofted}\index{Rofted} populations, with non-EDI mutation disabled.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/noedi_stray_montage.pdf}
\caption{\label{fig:org7cb40e7}
Plot of stray rate, crash rate, and the ratio of gadgets run in the \emph{mycwil}, \emph{simtyn}, \emph{surjes} and \emph{Xufmoc}\index{Xufmoc} populations, with mutation and EDI toggling disabled, with a base EDI rate of zero.}
\end{figure}

\chapter{Conclusions and Future Work}
\label{sec:orgd102dd5}
\label{orgec3b507}

\section{The devil in the details}
\label{sec:org8757808}
...
spend the majority of Chapter \ref{org54d1f94} labouring over peculiarities
...
\label{sec:org9489e5b}

The reader will have noticed that Chapter \ref{org54d1f94} raised at least
...
\label{sec:orgf88544a}

This was touched on in \S \ref{org7a40002}, in response to an interesting case
...
\label{sec:orgc808a92}

Frequently, in my analysis of various peculiar behaviours in \gls{roper}'s
populations, in Chapter \ref{org54d1f94}, my enquiry was cut short by a simple
...
\label{sec:org0fd939d}

The experiment touched on in \ref{orgdeea3ff} remained largely tangential,
...
\label{sec:org84f203c}

A design idea for the second iteration of \gls{roper} was spurred by an
interesting suggestion made by \aimention{Lee Spector} Lee Spector, in a discussion of some of my
...
\input{footer}
\chapter{Experimental Postscript}
\label{sec:org8461bbc}
\section{Adjusting the classification task function for better environmental compatibility}
\label{sec:org51961b7}
\subsection{Disadvantages of the bid-based algorithm in ROPER}
\label{sec:org599fd37}
As we can see in figure \ref{fig:org90e8c2e}, the distribution of register use
...
explains why we see it being given so great a preference in figure \ref{fig:org90e8c2e}. 
...
\caption{\label{fig:org3d38671}
Performance of the \emph{Xysxim}\index{Xysxim} population on the simple, linearly separable dataset first encountered in \S \ref{org86edf9f}.}
\end{figure}
...
\label{sec:orgc2cfa49}
\label{org3fdaf29}
\subsection{The trouble with crossover}
\label{sec:org893e9d6}
...
through the \emph{fragility} mechanism discussed in \ref{orgfe8d7c0} -- combining and substituting
genes without any regard to their form or function.

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/carboat-montage.png}
\caption{\label{fig:org00ef948}
Unconstrained crossover in action}
\end{figure}

\subsection{An implementation of (quasi-)homologous crossover}
\label{sec:org775a9a9}
...
used in \S \ref{org748e518}, and demonstrate its impact on classification tasks in 
Section \ref{orgae96292}. Its applicability, as we have seen, relies
...
neither the syscall preparation task (cf. \S \ref{org5a73cdb}), which uses a
singleton problem set, nor the
Snake experiment (cf. \S \ref{orgdeea3ff}), which uses a continuously
...
throughout Chapter \ref{org54d1f94}, in the analysis of heatmap data. 
...
\section{The clump TTL mechanism, and the role of extinction events}
\label{sec:org4b1a037}
...
figure \ref{fig:org9f854ec} -- whose evolutionary run bore the same configuration as the population
in which the malignant gene problem was first diagnosed (see \S \ref{org9addff0}).  

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/xilvyb.pdf}
\caption{\label{fig:org9f854ec}
Ecologically disruptive effects on the \emph{Xilvyb}\index{Xilvyb} population caused by the \gls{ttl} mechanism, causing a mass extinction of clump lines during a simple linear classification task, using register bid-bin classification.}
\end{figure}
...
and introducing a selective incentive for behavioural diversity (see \S \ref{org4f8d491}) appears to yield
...
completing the syscall preparation task, outlined in \S \ref{org27fe059}.
...
\caption{\label{fig:orga396a98}
The effects of visitation novelty pressure on the \emph{Soghuf}\index{Soghuf} population, with the additional influence of a \gls{ttl} field set to 4 and decremented on crash-prone clumps during crossover. The homologous crossover mechanism was not used for this run.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/soghuf.pdf}
\caption{\label{fig:org28eb865}
Performance metrics over the \emph{Soghuf}\index{Soghuf} population. Note mass extinction events around seasons 190, 300, and 400-500.}
\end{figure}



\section{Iris, Revisited}
\label{sec:orged880c1}
...
\ref{fig:org6f0270e} was never equaled in subsequent trials. 
...
\ref{fig:org6f0270e}, but the results so far have shown an extraordinary improvement over
their predecessors. The performance curves for this run are shown in figure \ref{fig:org601e7be}. 
...
\caption{\label{fig:org601e7be}
Performance of the \emph{Luxxyn}\index{Luxxyn} population of 4096, over time.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_with_luxxyn.pdf}
\caption{\label{fig:orga9d682c}
Comparison of the Iris dataset map with the classification performed by the \emph{Luxxyn}\index{Luxxyn} population's champion, after only 30 seasons.}
\end{figure}

As we can see in figure \ref{fig:orga9d682c}, \emph{even the linearly inseparable} 
...
\label{sec:orgfc2aa3c}
\listofalgorithms

\chapter{Population Parameters}
\label{sec:orgd4ee375}

\section{Note}
\label{sec:org9e11bf0}
...
\chapter{Index of Populations}
\label{sec:orgaa7cc08}
The following index tracks the mention and discussion of each \gls{roper} population
studied in the course of this thesis. Please see \S \ref{org76987f4} for an explanation
of the naming convention.

\printindex

\chapter{Index of Authors}
\label{sec:org0f515c6}



\begin{multicols}{2}
\noindent\printauthorindex
\end{multicols}
\end{document}