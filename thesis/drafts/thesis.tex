...
\label{sec:org57b7b22}
\label{orgddaee40}
\section{What is the aim of this research?}
\label{sec:org451f520}
\label{org1d1d5fd}
...
here could, with minor adaptations,\footnote{Discussed in \S \ref{org50110ec}.} used
to test and train \glspl{ids}, or provide one more instrument in the penetration
tester's toolbox.

\section{Who is this for?}
\label{sec:org6d10d54}
...
\section{Synopsis}
\label{sec:org09c7681}
\label{orgcf2cbdc}

In Chapter \ref{org161aa69}, I set up some of the conceptual
...
design, in Chapter \ref{org8d6d972}.

Chapter \ref{orgf3cba5b} introduces my contribution to research in the field of
...
Chapter \ref{orgd869779} goes over a handful of experimental studies with
\gls{roper}, and consequent modifications to the design. 

Chapter \ref{org50110ec} lays out some directions for future work and study on this
...
\label{sec:orge37f164}
\label{org9569967}
\subsection{1969: Benford}
\label{sec:org67236bf}
...
somewhat, in \S \ref{org97f8141}, but it remains a useful
simplification.

The main loop, outlined in Algorithm \ref{alg:evolve} and figure \ref{fig:org473cdee},
...
implementing the algorithms surveyed in \S \ref{org24a4f34}.
...
harvested from the target executable. 
  \footnote{See \S \ref{orgd8773df} for a sustained explanation of how return-oriented
...
in figure \ref{fig:org8471bb0}. Since we are
...
As explained in \S \ref{org3b20c1f}, it is often helpful to think of
...
harvested from the target binary (see \S \ref{org345dba7}), and a
...
these consequences in Chapter \ref{orgd869779}.


\subsection{Crossover}
\label{sec:orge407f6f}
\label{org2827b43} 
...
\ref{org8407a6c}. The details of the algorithm itself can be found in
\ref{alg:crossover}.
...
Section \ref{org997bb64}. If we abstract away from the information loss
...
Section \ref{org997bb64} commute with and distribute over crossover, if we consider
...
\label{sec:orgf6ec9fe}
\label{org7e6fc67}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/evaluation_model.png}
\caption{\label{fig:org382786c}
High-level overview of the ontogenesis and evaluation process. The ``path diversity'' component will not be introduced until we reach \S \ref{org6a04fa3}.}
\end{figure}
...
penalty to fitness, the details of which will be discussed in \S \ref{orgab3a230}.
...
detail in Chapter \ref{orgd869779}. For the time being, the matter of \gls{cpu}
exceptions deserves closer comment.

\subsubsection{Failure modes and crash rates}
\label{sec:orgcdbef81}
...
of fitness function, listed in \S \ref{orgab3a230} -- one way that diversity depletion often
...
binary's \texttt{.text} section are plotted in figure \ref{fig:org210ac58}, to which
I will make frequent reference in the subsequent heatmap overlay images.

In the following subsections (\ref{orgec0682e}, \ref{org6af2000},
\ref{orgafba698}, and \ref{orge92a392}), I will outline each task set that
\gls{roper} was given to perform, and specify the way in which each was
incorporated into \gls{roper}'s fitness function.

In \S \ref{org6de642e}, I will walk through some of the more interesting results
gleaned from each experiment, and in \S \ref{orga8d7ade}, I will lay out certain
...
\label{sec:org26d9cf2}
\label{orgec0682e}

As discussed in \S\S \ref{org997bb64} and \ref{org2827b43}, when designing a genetic
...
as a component of the fitness function (see \S \ref{orgab3a230} for details),
...
\gls{roper}, so long as the fitness sharing mechanism discussed in \ref{orgb9b9dd0}
...
In figure \ref{fig:org8ad1944}, we see heatmaps of the the \emph{Xeqcyv} population's
...
\ref{fig:orgc6d756a}, is not at all surprising. The longer they spend in
...
a surprising degree of ruggedness, as we can see in figure \ref{fig:orge762eb7}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/wiwzuh_syscall_gaussian_3.pdf}
\caption{\label{fig:orge762eb7}
...
usage shown in figure \ref{fig:org96fbd9c}. If the output registers
...
fitness sharing (see \S \ref{orgb9b9dd0}) provides a means for the evolutionary
...
and two classes (see figure \ref{fig:org735fe11}).
...
\ref{fig:orgc994bc8}). Its best specimen clearly relied on a single
...
\caption{\label{fig:orgc994bc8}
The classification on the data displayed in figure \ref{fig:org735fe11} performed by the best specimen in the \emph{Kathot} population, 35\(^{\text{th}}\) generation (execution trace inset).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kathot_fitness.pdf}
\caption{\label{fig:org75669d8}
...
\caption{\label{fig:org2905f35}
Address visitation heatmap of the \emph{Fizwej} population, at season 4 on the left, and season 124, on the right, showing a dramatic decline in phenotypic diversity. Compare with the \emph{xeqcyv} and \emph{kurlig} heatmaps in figures \ref{fig:org8ad1944} and \ref{fig:org88f0f21}.}
\end{figure}
...
of future directions for this project, in \S \ref{org50110ec}. Incorporating
...
will look at more closely in \S \ref{orge98f5dd}) , which I stumbled
...
20,000\(^{\text{th}}\) tournament iteration (figure \ref{fig:orgd480bb3}). Due to the data that
was collected at the time being insufficiently coarse-grained, I cannot say
whether dynamics similar to those described in \S \ref{orgb150b84} played
...
In view of the phenomena described in \S \ref{orgb150b84}, however, this approach to regulating the crash rate may warrant revision. 

\paragraph{A proposed method for regulating malignant genes with a TTL field}
\label{sec:org630693a}
\label{orgafec1a5}
...
Section \ref{org6f70be1}. Its attribute-class mapping is plotted
in figure \ref{fig:org598cabd}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_plot.pdf}
\caption{\label{fig:org598cabd}
...
This is what we see happening in figure \ref{fig:orgdd72718}, for example,
...
depth in section \ref{org6d93f40}.
...
mechanic detailed in \S \ref{org8407a6c}, however: a sufficiently fragile gene
...
\ref{fig:orgdd72718} is a symptom of (premature) genetic convergence. 
...
\caption{\label{fig:org81e2c66}
A trial similar to the one documented in figure \ref{fig:orgc543be6}, with per-class difficulty recorded, but with the fitness sharing mechanism suspended (\emph{Cazmud} population). The filled curve surrounding each mean difficulty class, here being of visible breadth only in the case of the \emph{iris virginia}, represents the standard deviation of difficulty for each exemplar class.}
\end{figure}

For this reason, I introduced into \gls{roper} the \emph{fitness sharing} mechanism 
that I have outlined in \S \ref{orgb9b9dd0}. 
...
phylogenic generation of 91.3. Figure \ref{fig:orgc543be6} shows the course the
...
\caption{\label{fig:orgc543be6}
A very good run on the Iris classification task, employing the fitness sharing algorithm documented in \S \ref{orgb9b9dd0} (\emph{Ragweb} population). The filled curve surrounding each mean difficulty line again represents the standard deviation of difficulty for that exemplar class.}
\end{figure}



This run can be informatively compared with the one illustrated in figure
\ref{fig:org81e2c66}. Note the tight interbraiding of problem difficulties in
figure \ref{fig:orgc543be6}, as compared to their gaping -- but still, slowly,
fluctuating -- disparity in figure \ref{fig:org81e2c66}. The ballooning standard
deviation of difficulty by class in figure \ref{fig:orgc543be6} also suggests
...
The specifications of the game are described above, in \S \ref{orge92a392}.
...
in \S \ref{org9569967}), our ``in vitro'' populations have the advantage of a gently
...
discussed in \S\S \ref{org6af2000} and \ref{orgeb70ee6}, but
...
The results, presented in figures \ref{fig:org8343d9c} and
\ref{fig:org621a918}, were found to corroborate the intron hypothesis,
...
appearing in figure \ref{fig:org8343d9c}, suggest that this \emph{may} be the
...
\label{sec:org8d61652}
\label{org50110ec}

\section{The devil in the details}
\label{sec:org1d59aa3}
...
spend the majority of Chapter \ref{orgd869779} labouring over peculiarities
...
\label{sec:orgecbe441}

The reader will have noticed that Chapter \ref{orgd869779} raised at least
...
\label{sec:org8a2c8e6}

This was touched on in \S \ref{orgafec1a5}, in response to an interesting case
...
\label{sec:orgbfda84b}

Frequently, in my analysis of various peculiar behaviours in \gls{roper}'s
populations, in Chapter \ref{orgd869779}, my enquiry was cut short by a simple
...
\label{sec:orga7bcd56}

The experiment touched on in \ref{org24b5816} remained largely tangential,
...
explains why we see it being given so great a preference in figure \ref{fig:org96fbd9c}. 
...
\caption{\label{fig:org8c17fac}
Performance of the \emph{Xysxim} population on the simple, linearly separable dataset first encountered in \S \ref{org6f70be1}.}
\end{figure}
...
\label{sec:org98f8668}
\label{org71b0074}
\subsection{The trouble with crossover}
\label{sec:org3f963c7}
...
used in \S \ref{orgb9b9dd0}, and demonstrate its impact on classification tasks in 
Section \ref{org1dd6593}. Its applicability, as we have seen, relies
...
neither the syscall preparation task (cf. \S \ref{orgeb70ee6}), which uses a
singleton problem set, nor the
Snake experiment (cf. \S \ref{org24b5816}), which uses a continuously
...
throughout Chapter \ref{orgd869779}, in the analysis of heatmap data. 
...
figure \ref{fig:org6090196} -- whose evolutionary run bore the same configuration as the population
in which the malignant gene problem was first diagnosed (see \S \ref{orgb150b84}).  

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/xilvyb.pdf}
\caption{\label{fig:org6090196}
...
and introducing a selective incentive for behavioural diversity (see \S \ref{org6a04fa3}) appears to yield
...
completing the syscall preparation task, outlined in \S \ref{org6af2000}.
...
\ref{fig:orgc543be6} was never equaled in subsequent trials. 
...
\ref{fig:orgc543be6}, but the results so far have shown an extraordinary improvement over
their predecessors. The performance curves for this run are shown in figure \ref{fig:org9eecb9b}. 
...
As we can see in figure \ref{fig:orgf9f7377}, \emph{even the linearly inseparable} 
