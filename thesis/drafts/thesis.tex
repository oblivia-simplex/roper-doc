...
\label{sec:org494cb48}
\label{orgb900645}
\section{What is the aim of this research?}
\label{sec:orgb3de313}
\label{org8996c6a}
...
here could, with minor adaptations,\footnote{Discussed in \S \ref{orge36a132}.} used
to test and train \glspl{ids}, or provide one more instrument in the penetration
tester's toolbox.

\section{Who is this for?}
\label{sec:orge43e559}
...
\section{Synopsis}
\label{sec:orgee009af}
\label{org67237a9}

In Chapter \ref{org309ff25}, I set up some of the conceptual
...
design, in Chapter \ref{org8380e2d}.

Chapter \ref{org870220c} introduces my contribution to research in the field of
...
Chapter \ref{org9b35957} goes over a handful of experimental studies with
\gls{roper}, and consequent modifications to the design. 

Chapter \ref{orge36a132} lays out some directions for future work and study on this
...
This will become clear as we work through Chapter \ref{org9b35957}.
...
\label{sec:org16ff0e3}
\label{orgfd1b72d}
\subsection{1969: Benford}
\label{sec:org033f8d3}
...
somewhat, in \S \ref{org933df10}, but it remains a useful
...
The main loop, outlined in Algorithm \ref{alg:evolve} and figure \ref{fig:orgd924d3e},
...
implementing the algorithms surveyed in \S \ref{orgb1f8956}.
...
harvested from the target executable. 
  \footnote{See \S \ref{org73b767a} for a sustained explanation of how return-oriented
...
in figure \ref{fig:orgd6d2ee8}. Since we are
...
As explained in \S \ref{org235c2d2}, it is often helpful to think of
...
harvested from the target binary (see \S \ref{org428d51c}), and a
...
these consequences in Chapter \ref{org9b35957}.


\subsection{Crossover}
\label{sec:org8a48c84}
\label{orgb90fe4c} 
...
\ref{org31d59e3}. The details of the algorithm itself can be found in
\ref{alg:crossover}.
...
Section \ref{org937640b}. If we abstract away from the information loss
...
Section \ref{org937640b} commute with and distribute over crossover, if we consider
...
\label{sec:orgb412c1c}
\label{org1128603}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/evaluation_model.png}
\caption{\label{fig:orgaa229f5}
High-level overview of the ontogenesis and evaluation process. The ``path diversity'' component will not be introduced until we reach \S \ref{orga523243}.}
\end{figure}
...
penalty to fitness, the details of which will be discussed in \S \ref{org0f04626}.
...
detail in Chapter \ref{org9b35957}. For the time being, the matter of \gls{cpu}
exceptions deserves closer comment.

\subsubsection{Failure modes and crash rates}
\label{sec:orgd7c1c95}
...
of fitness function, listed in \S \ref{org0f04626} -- one way that diversity depletion often
...
binary. (In all of the experiments documented in Chapter \ref{org9b35957},
...
binary's \texttt{.text} section are plotted in figure \ref{fig:orgd7cfd8d}, to which
I will make frequent reference in the subsequent heatmap overlay images.

In the following subsections (\ref{orgc7bfec6}, \ref{orgd2cc7e2},
\ref{org1020571}, and \ref{org8daced4}), I will outline each task set that
\gls{roper} was given to perform, and specify the way in which each was
incorporated into \gls{roper}'s fitness function.

In \S \ref{orgc621003}, I will walk through some of the more interesting results
gleaned from each experiment, and in \S \ref{org9a7a5ea}, I will lay out certain
...
\label{sec:org442bd32}
\label{orgc7bfec6}

As discussed in \S\S \ref{org937640b} and \ref{orgb90fe4c}, when designing a genetic
...
as a component of the fitness function (see \S \ref{org0f04626} for details),
...
\gls{roper}, so long as the fitness sharing mechanism discussed in \ref{org384619c}
...
In figure \ref{fig:org15a86b1}, we see heatmaps of the the \emph{Xeqcyv}\index{Xeqcyv} population's
...
\ref{fig:org2df0475}, is not at all surprising. The longer they spend in
...
a surprising degree of ruggedness, as we can see in figure \ref{fig:orga0f54df}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/wiwzuh_syscall_gaussian_3.pdf}
\caption{\label{fig:orga0f54df}
...
usage shown in figure \ref{fig:orge29fb21}. If the output registers
...
fitness sharing (see \S \ref{org384619c}) provides a means for the evolutionary
...
and two classes (see figure \ref{fig:org195538f}).
...
\ref{fig:org4d24eee}). Its best specimen clearly relied on a single
...
\caption{\label{fig:org4d24eee}
The classification on the data displayed in figure \ref{fig:org195538f} performed by the best specimen in the \emph{Kathot}\index{Kathot} population, 35\(^{\text{th}}\) generation (execution trace inset).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kathot_fitness.pdf}
\caption{\label{fig:org1bf4dca}
...
\caption{\label{fig:orgdfee888}
Address visitation heatmap of the \emph{Fizwej}\index{Fizwej} population, at season 4 on the left, and season 124, on the right, showing a dramatic decline in phenotypic diversity. Compare with the \emph{xeqcyv} and \emph{kurlig} heatmaps in figures \ref{fig:org15a86b1} and \ref{fig:org9411869}.}
\end{figure}
...
of future directions for this project, in \S \ref{orge36a132}. Incorporating
...
will look at more closely in \S \ref{org98240c8}) , which I stumbled
...
20,000\(^{\text{th}}\) tournament iteration (figure \ref{fig:orgc06c2d4}). Due to the data that
was collected at the time being insufficiently coarse-grained, I cannot say
whether dynamics similar to those described in \S \ref{org2e8aa57} played
...
In view of the phenomena described in \S \ref{org2e8aa57}, however, this approach to regulating the crash rate may warrant revision. 

\paragraph{A proposed method for regulating malignant genes with a TTL field}
\label{sec:org03e913a}
\label{org6c31d6c}
...
Section \ref{orgf9dc0ae}. Its attribute-class mapping is plotted
in figure \ref{fig:orgf3b14a7}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_plot.pdf}
\caption{\label{fig:orgf3b14a7}
...
This is what we see happening in figure \ref{fig:orge55f086}, for example,
...
depth in section \ref{orge924677}.
...
mechanic detailed in \S \ref{org31d59e3}, however: a sufficiently fragile gene
...
\ref{fig:orge55f086} is a symptom of (premature) genetic convergence. 
...
\caption{\label{fig:orgd18cc61}
A trial similar to the one documented in figure \ref{fig:org658bfb2}, with per-class difficulty recorded, but with the fitness sharing mechanism suspended (\emph{Cazmud}\index{Cazmud} population). The filled curve surrounding each mean difficulty class, here being of visible breadth only in the case of the \emph{iris virginia}, represents the standard deviation of difficulty for each exemplar class.}
\end{figure}

For this reason, I introduced into \gls{roper} the \emph{fitness sharing} mechanism 
that I have outlined in \S \ref{org384619c}. 
...
phylogenic generation of 91.3. Figure \ref{fig:org658bfb2} shows the course the
...
\caption{\label{fig:org658bfb2}
A very good run on the Iris classification task, employing the fitness sharing algorithm documented in \S \ref{org384619c} (\emph{Ragweb}\index{Ragweb} population). The filled curve surrounding each mean difficulty line again represents the standard deviation of difficulty for that exemplar class.}
\end{figure}



This run can be informatively compared with the one illustrated in figure
\ref{fig:orgd18cc61}. Note the tight interbraiding of problem difficulties in
figure \ref{fig:org658bfb2}, as compared to their gaping -- but still, slowly,
fluctuating -- disparity in figure \ref{fig:orgd18cc61}. The ballooning standard
deviation of difficulty by class in figure \ref{fig:org658bfb2} also suggests
...
The specifications of the game are described above, in \S \ref{org8daced4}.
...
in \S \ref{orgfd1b72d}), our ``in vitro'' populations have the advantage of a gently
...
exploited process.\footnote{See the semantic correction noted in \S \ref{org5057ca2}, which 
...
discussed in \S\S \ref{orgd2cc7e2} and \ref{org5057ca2}, but
...
The results, presented in figures \ref{fig:org2606973} and
\ref{fig:org10d00e4}, were found to corroborate the intron hypothesis,
...
appearing in figure \ref{fig:org2606973}, suggest that this \emph{may} be the
...
\label{sec:orge841133}
\label{orge36a132}

\section{The devil in the details}
\label{sec:orge07d809}
...
spend the majority of Chapter \ref{org9b35957} labouring over peculiarities
...
\label{sec:org583ceb6}

The reader will have noticed that Chapter \ref{org9b35957} raised at least
...
\label{sec:org07a6fd9}

This was touched on in \S \ref{org6c31d6c}, in response to an interesting case
...
\label{sec:orgfc1f0fa}

Frequently, in my analysis of various peculiar behaviours in \gls{roper}'s
populations, in Chapter \ref{org9b35957}, my enquiry was cut short by a simple
...
\label{sec:orga32187e}

The experiment touched on in \ref{org0e02493} remained largely tangential,
...
explains why we see it being given so great a preference in figure \ref{fig:orge29fb21}. 
...
\caption{\label{fig:org643fc28}
Performance of the \emph{Xysxim}\index{Xysxim} population on the simple, linearly separable dataset first encountered in \S \ref{orgf9dc0ae}.}
\end{figure}
...
\label{sec:org834250f}
\label{org8ee38a8}
\subsection{The trouble with crossover}
\label{sec:org8e1593a}
...
used in \S \ref{org384619c}, and demonstrate its impact on classification tasks in 
Section \ref{orge4b76e1}. Its applicability, as we have seen, relies
...
neither the syscall preparation task (cf. \S \ref{org5057ca2}), which uses a
singleton problem set, nor the
Snake experiment (cf. \S \ref{org0e02493}), which uses a continuously
...
throughout Chapter \ref{org9b35957}, in the analysis of heatmap data. 
...
figure \ref{fig:orgb43a9ac} -- whose evolutionary run bore the same configuration as the population
in which the malignant gene problem was first diagnosed (see \S \ref{org2e8aa57}).  

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/xilvyb.pdf}
\caption{\label{fig:orgb43a9ac}
...
and introducing a selective incentive for behavioural diversity (see \S \ref{orga523243}) appears to yield
...
completing the syscall preparation task, outlined in \S \ref{orgd2cc7e2}.
...
\ref{fig:org658bfb2} was never equaled in subsequent trials. 
...
\ref{fig:org658bfb2}, but the results so far have shown an extraordinary improvement over
their predecessors. The performance curves for this run are shown in figure \ref{fig:orgd225c5e}. 
...
As we can see in figure \ref{fig:orga800b19}, \emph{even the linearly inseparable} 
...
\label{sec:org9177082}
\listofalgorithms

\chapter{Population Parameters}
\label{sec:org5dad268}

\section{Note}
\label{sec:org554e4f1}
