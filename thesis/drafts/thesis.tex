...
\label{sec:org068247e}
\label{orgc9cd3aa}
\section{What is the aim of this research?}
\label{sec:org25cc32b}
\label{orgf4b0702}
...
here could, with minor adaptations,\footnote{Discussed in \S \ref{org0de89b6}.} used
to test and train \glspl{ids}, or provide one more instrument in the penetration
tester's toolbox.

\section{Who is this for?}
\label{sec:org72d5185}
...
\section{Synopsis}
\label{sec:org5a9a8ca}
\label{org7c239a8}

In Chapter \ref{org1c11ff7}, I set up some of the conceptual
...
design, in Chapter \ref{org6364b61}.

Chapter \ref{orga2a9006} introduces my contribution to research in the field of
...
Chapter \ref{orgf534caa} goes over a handful of experimental studies with
\gls{roper}, and consequent modifications to the design. 

Chapter \ref{org0de89b6} lays out some directions for future work and study on this
...
\label{sec:org80a8e56}
\label{org65ff6cf}
\subsection{1969: Benford}
\label{sec:orga88db38}
...
somewhat, in \S \ref{org43f71ad}, but it remains a useful
simplification.

The main loop, outlined in Algorithm \ref{alg:evolve} and figure \ref{fig:org9ee8a6a},
...
implementing the algorithms surveyed in \S \ref{org788ae63}.
...
harvested from the target executable. 
  \footnote{See \S \ref{org8566d01} for a sustained explanation of how return-oriented
...
in figure \ref{fig:org01d80b6}. Since we are
...
As explained in \S \ref{orge6ef5c2}, it is often helpful to think of
...
harvested from the target binary (see \S \ref{org738dc65}), and a
...
these consequences in Chapter \ref{orgf534caa}.


\subsection{Crossover}
\label{sec:org0385e04}
\label{org821fa9b} 
...
\ref{org4dacc08}. The details of the algorithm itself can be found in
\ref{alg:crossover}.
...
Section \ref{orgb584858}. If we abstract away from the information loss
...
Section \ref{orgb584858} commute with and distribute over crossover, if we consider
...
\label{sec:orgcc518d3}
\label{org84dd4cd}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/evaluation_model.png}
\caption{\label{fig:org5b2473b}
High-level overview of the ontogenesis and evaluation process. The ``path diversity'' component will not be introduced until we reach \S \ref{orgca4cce0}.}
\end{figure}
...
penalty to fitness, the details of which will be discussed in \S \ref{org8e31a7d}.
...
detail in Chapter \ref{orgf534caa}. For the time being, the matter of \gls{cpu}
exceptions deserves closer comment.

\subsubsection{Failure modes and crash rates}
\label{sec:org7dbea16}
...
of fitness function, listed in \S \ref{org8e31a7d} -- one way that diversity depletion often
...
binary's \texttt{.text} section are plotted in figure \ref{fig:orge347be6}, to which
I will make frequent reference in the subsequent heatmap overlay images.

In the following subsections (\ref{orgba2f373}, \ref{org2452348},
\ref{orgc1ba00c}, and \ref{org7356ffa}), I will outline each task set that
\gls{roper} was given to perform, and specify the way in which each was
incorporated into \gls{roper}'s fitness function.

In \S \ref{org06a7edb}, I will walk through some of the more interesting results
gleaned from each experiment, and in \S \ref{orgef53660}, I will lay out certain
...
\label{sec:org3d30c8a}
\label{orgba2f373}

As discussed in \S\S \ref{orgb584858} and \ref{org821fa9b}, when designing a genetic
...
as a component of the fitness function (see \S \ref{org8e31a7d} for details),
...
\gls{roper}, so long as the fitness sharing mechanism discussed in \ref{org433ba44}
...
In figure \ref{fig:org2a6fd2c}, we see heatmaps of the the \emph{Xeqcyv} population's
...
\ref{fig:org9150137}, is not at all surprising. The longer they spend in
...
a surprising degree of ruggedness, as we can see in figure \ref{fig:org1210297}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/wiwzuh_syscall_gaussian_3.pdf}
\caption{\label{fig:org1210297}
...
usage shown in figure \ref{fig:orge3125d9}. If the output registers
...
fitness sharing (see \S \ref{org433ba44}) provides a means for the evolutionary
...
and two classes (see figure \ref{fig:orgdcda489}).
...
\ref{fig:org3def212}). Its best specimen clearly relied on a single
...
\caption{\label{fig:org3def212}
The classification on the data displayed in figure \ref{fig:orgdcda489} performed by the best specimen in the \emph{Kathot} population, 35\(^{\text{th}}\) generation (execution trace inset).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kathot_fitness.pdf}
\caption{\label{fig:org39fcd48}
...
\caption{\label{fig:orgc79c13d}
Address visitation heatmap of the \emph{Fizwej} population, at season 4 on the left, and season 124, on the right, showing a dramatic decline in phenotypic diversity. Compare with the \emph{xeqcyv} and \emph{kurlig} heatmaps in figures \ref{fig:org2a6fd2c} and \ref{fig:org5be5988}.}
\end{figure}
...
of future directions for this project, in \S \ref{org0de89b6}. Incorporating
...
will look at more closely in \S \ref{org972aa3f}) , which I stumbled
...
20,000\(^{\text{th}}\) tournament iteration (figure \ref{fig:org705e763}). Due to the data that
was collected at the time being insufficiently coarse-grained, I cannot say
whether dynamics similar to those described in \S \ref{org2db27e4} played
...
In view of the phenomena described in \S \ref{org2db27e4}, however, this approach to regulating the crash rate may warrant revision. 

\paragraph{A proposed method for regulating malignant genes with a TTL field}
\label{sec:org900a86d}
\label{org24108e1}
...
Section \ref{org56e0406}. Its attribute-class mapping is plotted
in figure \ref{fig:org6460772}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_plot.pdf}
\caption{\label{fig:org6460772}
...
This is what we see happening in figure \ref{fig:org729bd0d}, for example,
...
depth in section \ref{org45f7bac}.
...
mechanic detailed in \S \ref{org4dacc08}, however: a sufficiently fragile gene
...
\ref{fig:org729bd0d} is a symptom of (premature) genetic convergence. 
...
\caption{\label{fig:org6204829}
A trial similar to the one documented in figure \ref{fig:org1902719}, with per-class difficulty recorded, but with the fitness sharing mechanism suspended (\emph{Cazmud} population). The filled curve surrounding each mean difficulty class, here being of visible breadth only in the case of the \emph{iris virginia}, represents the standard deviation of difficulty for each exemplar class.}
\end{figure}

For this reason, I introduced into \gls{roper} the \emph{fitness sharing} mechanism 
that I have outlined in \S \ref{org433ba44}. 
...
phylogenic generation of 91.3. Figure \ref{fig:org1902719} shows the course the
...
\caption{\label{fig:org1902719}
A very good run on the Iris classification task, employing the fitness sharing algorithm documented in \S \ref{org433ba44} (\emph{Ragweb} population). The filled curve surrounding each mean difficulty line again represents the standard deviation of difficulty for that exemplar class.}
\end{figure}



This run can be informatively compared with the one illustrated in figure
\ref{fig:org6204829}. Note the tight interbraiding of problem difficulties in
figure \ref{fig:org1902719}, as compared to their gaping -- but still, slowly,
fluctuating -- disparity in figure \ref{fig:org6204829}. The ballooning standard
deviation of difficulty by class in figure \ref{fig:org1902719} also suggests
...
The specifications of the game are described above, in \S \ref{org7356ffa}.
...
in \S \ref{org65ff6cf}), our ``in vitro'' populations have the advantage of a gently
...
discussed in \S\S \ref{org2452348} and \ref{org41e079f}, but
...
The results, presented in figures \ref{fig:org3d8d991} and
\ref{fig:org9dead05}, were found to corroborate the intron hypothesis,
...
appearing in figure \ref{fig:org3d8d991}, suggest that this \emph{may} be the
...
\label{sec:org9cb8153}
\label{org0de89b6}

\section{The devil in the details}
\label{sec:org3a8524c}
...
spend the majority of Chapter \ref{orgf534caa} labouring over peculiarities
...
\label{sec:org8b29cde}

The reader will have noticed that Chapter \ref{orgf534caa} raised at least
...
\label{sec:org73b2cb3}

This was touched on in \S \ref{org24108e1}, in response to an interesting case
...
\label{sec:org0614f33}

Frequently, in my analysis of various peculiar behaviours in \gls{roper}'s
populations, in Chapter \ref{orgf534caa}, my enquiry was cut short by a simple
...
\label{sec:org25700f4}

The experiment touched on in \ref{org2ca1774} remained largely tangential,
...
explains why we see it being given so great a preference in figure \ref{fig:orge3125d9}. 
...
\caption{\label{fig:org7d14641}
Performance of the \emph{Xysxim} population on the simple, linearly separable dataset first encountered in \S \ref{org56e0406}.}
\end{figure}
...
\label{sec:org2e3d020}
\label{orgf12737a}
\subsection{The trouble with crossover}
\label{sec:orga2f9f71}
...
used in \S \ref{org433ba44}, and demonstrate its impact on classification tasks in 
Section \ref{org97e2687}. Its applicability, as we have seen, relies
...
neither the syscall preparation task (cf. \S \ref{org41e079f}), which uses a
singleton problem set, nor the
Snake experiment (cf. \S \ref{org2ca1774}), which uses a continuously
...
throughout Chapter \ref{orgf534caa}, in the analysis of heatmap data. 
...
figure \ref{fig:orgc187771} -- whose evolutionary run bore the same configuration as the population
in which the malignant gene problem was first diagnosed (see \S \ref{org2db27e4}).  

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/xilvyb.pdf}
\caption{\label{fig:orgc187771}
...
and introducing a selective incentive for behavioural diversity (see \S \ref{orgca4cce0}) appears to yield
...
completing the syscall preparation task, outlined in \S \ref{org2452348}.
