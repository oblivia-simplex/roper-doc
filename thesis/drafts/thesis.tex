...
\label{sec:org2cbe10f}
\label{org4650ec5}
\section{What is the aim of this research?}
\label{sec:orgbdf5b4d}
\label{org8c14ded}
...
here could, with minor adaptations,\footnote{Discussed in \S \ref{org0796296}.} used
to test and train \glspl{ids}, or provide one more instrument in the penetration
tester's toolbox.

\section{Who is this for?}
\label{sec:orge9d58a8}
...
\section{Synopsis}
\label{sec:org7ebf22e}
\label{orgbeb7d77}

In Chapter \ref{org34de4d2}, I set up some of the conceptual
...
design, in Chapter \ref{org41145c2}.

Chapter \ref{org4a3a967} introduces my contribution to research in the field of
...
Chapter \ref{org465e5f4} goes over a handful of experimental studies with
\gls{roper}, and consequent modifications to the design. 

Chapter \ref{org0796296} lays out some directions for future work and study on this
...
This will become clear as we work through Chapter \ref{org465e5f4}.
...
\label{sec:orgcf85b1f}
\label{org54b0ad5}
\subsection{1969: Benford}
\label{sec:orgb492fe2}
...
somewhat, in \S \ref{orgb50f5c2}, but it remains a useful
...
The main loop, outlined in Algorithm \ref{alg:evolve} and figure \ref{fig:orgeb346ec},
...
implementing the algorithms surveyed in \S \ref{orgfa583b7}.
...
harvested from the target executable. 
  \footnote{See \S \ref{org7bd0185} for a sustained explanation of how return-oriented
...
in figure \ref{fig:orga3c944f}. Since we are
...
As explained in \S \ref{org27df17b}, it is often helpful to think of
...
harvested from the target binary (see \S \ref{orgec8918e}), and a
...
these consequences in Chapter \ref{org465e5f4}.


\subsection{Crossover}
\label{sec:org0ef24cf}
\label{org3c23bc4} 
...
\ref{org3864a75}. The details of the algorithm itself can be found in
\ref{alg:crossover}.
...
Section \ref{org0d4edfb}. If we abstract away from the information loss
...
Section \ref{org0d4edfb} commute with and distribute over crossover, if we consider
...
\label{sec:orgc2e493b}
\label{orge56b2b6}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/evaluation_model.png}
\caption{\label{fig:orgaef27ca}
High-level overview of the ontogenesis and evaluation process. The ``path diversity'' component will not be introduced until we reach \S \ref{org4a9c21b}.}
\end{figure}
...
penalty to fitness, the details of which will be discussed in \S \ref{org1e192f7}.
...
detail in Chapter \ref{org465e5f4}. For the time being, the matter of \gls{cpu}
exceptions deserves closer comment.

\subsubsection{Failure modes and crash rates}
\label{sec:orgbee1a82}
...
of fitness function, listed in \S \ref{org1e192f7} -- one way that diversity depletion often
...
binary. (In all of the experiments documented in Chapter \ref{org465e5f4},
...
binary's \texttt{.text} section are plotted in figure \ref{fig:org8092418}, to which
I will make frequent reference in the subsequent heatmap overlay images.

In the following subsections (\ref{org826c2cf}, \ref{orgfe5f60b},
\ref{orgbe45120}, and \ref{orgd09a8b3}), I will outline each task set that
\gls{roper} was given to perform, and specify the way in which each was
incorporated into \gls{roper}'s fitness function.

In \S \ref{org89c9cc9}, I will walk through some of the more interesting results
gleaned from each experiment, and in \S \ref{orge2344d6}, I will lay out certain
...
\label{sec:org02f748b}
\label{org826c2cf}

As discussed in \S\S \ref{org0d4edfb} and \ref{org3c23bc4}, when designing a genetic
...
as a component of the fitness function (see \S \ref{org1e192f7} for details),
...
\gls{roper}, so long as the fitness sharing mechanism discussed in \ref{orga916dfa}
...
In figure \ref{fig:org8452c60}, we see heatmaps of the the \emph{Xeqcyv} population's
...
\ref{fig:org6bbf137}, is not at all surprising. The longer they spend in
...
a surprising degree of ruggedness, as we can see in figure \ref{fig:org7162017}. 

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/wiwzuh_syscall_gaussian_3.pdf}
\caption{\label{fig:org7162017}
...
usage shown in figure \ref{fig:org9b51390}. If the output registers
...
fitness sharing (see \S \ref{orga916dfa}) provides a means for the evolutionary
...
and two classes (see figure \ref{fig:org3051ee9}).
...
\ref{fig:orgf144c1b}). Its best specimen clearly relied on a single
...
\caption{\label{fig:orgf144c1b}
The classification on the data displayed in figure \ref{fig:org3051ee9} performed by the best specimen in the \emph{Kathot} population, 35\(^{\text{th}}\) generation (execution trace inset).}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/kathot_fitness.pdf}
\caption{\label{fig:org4644734}
...
\caption{\label{fig:orgf1b6bd2}
Address visitation heatmap of the \emph{Fizwej} population, at season 4 on the left, and season 124, on the right, showing a dramatic decline in phenotypic diversity. Compare with the \emph{xeqcyv} and \emph{kurlig} heatmaps in figures \ref{fig:org8452c60} and \ref{fig:orgc34c7ed}.}
\end{figure}
...
of future directions for this project, in \S \ref{org0796296}. Incorporating
...
will look at more closely in \S \ref{org92b4ebb}) , which I stumbled
...
20,000\(^{\text{th}}\) tournament iteration (figure \ref{fig:org4fa0b66}). Due to the data that
was collected at the time being insufficiently coarse-grained, I cannot say
whether dynamics similar to those described in \S \ref{orgf6d19fa} played
...
In view of the phenomena described in \S \ref{orgf6d19fa}, however, this approach to regulating the crash rate may warrant revision. 

\paragraph{A proposed method for regulating malignant genes with a TTL field}
\label{sec:orgd70d9cf}
\label{orgfbc7400}
...
Section \ref{org0cb46bf}. Its attribute-class mapping is plotted
in figure \ref{fig:org53d750b}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/iris_plot.pdf}
\caption{\label{fig:org53d750b}
...
This is what we see happening in figure \ref{fig:org5e838dd}, for example,
...
depth in section \ref{org915aca8}.
...
mechanic detailed in \S \ref{org3864a75}, however: a sufficiently fragile gene
...
\ref{fig:org5e838dd} is a symptom of (premature) genetic convergence. 
...
\caption{\label{fig:orga28fefe}
A trial similar to the one documented in figure \ref{fig:org79895f2}, with per-class difficulty recorded, but with the fitness sharing mechanism suspended (\emph{Cazmud} population). The filled curve surrounding each mean difficulty class, here being of visible breadth only in the case of the \emph{iris virginia}, represents the standard deviation of difficulty for each exemplar class.}
\end{figure}

For this reason, I introduced into \gls{roper} the \emph{fitness sharing} mechanism 
that I have outlined in \S \ref{orga916dfa}. 
...
phylogenic generation of 91.3. Figure \ref{fig:org79895f2} shows the course the
...
\caption{\label{fig:org79895f2}
A very good run on the Iris classification task, employing the fitness sharing algorithm documented in \S \ref{orga916dfa} (\emph{Ragweb} population). The filled curve surrounding each mean difficulty line again represents the standard deviation of difficulty for that exemplar class.}
\end{figure}



This run can be informatively compared with the one illustrated in figure
\ref{fig:orga28fefe}. Note the tight interbraiding of problem difficulties in
figure \ref{fig:org79895f2}, as compared to their gaping -- but still, slowly,
fluctuating -- disparity in figure \ref{fig:orga28fefe}. The ballooning standard
deviation of difficulty by class in figure \ref{fig:org79895f2} also suggests
...
The specifications of the game are described above, in \S \ref{orgd09a8b3}.
...
in \S \ref{org54b0ad5}), our ``in vitro'' populations have the advantage of a gently
...
exploited process.\footnote{See the semantic correction noted in \S \ref{orgdc5d98b}, which 
...
discussed in \S\S \ref{orgfe5f60b} and \ref{orgdc5d98b}, but
...
The results, presented in figures \ref{fig:orgb85c5eb} and
\ref{fig:org49f69a5}, were found to corroborate the intron hypothesis,
...
appearing in figure \ref{fig:orgb85c5eb}, suggest that this \emph{may} be the
...
\label{sec:orgbcfdec2}
\label{org0796296}

\section{The devil in the details}
\label{sec:org66811c3}
...
spend the majority of Chapter \ref{org465e5f4} labouring over peculiarities
...
\label{sec:org72a89c2}

The reader will have noticed that Chapter \ref{org465e5f4} raised at least
...
\label{sec:org345128d}

This was touched on in \S \ref{orgfbc7400}, in response to an interesting case
...
\label{sec:orgc230f57}

Frequently, in my analysis of various peculiar behaviours in \gls{roper}'s
populations, in Chapter \ref{org465e5f4}, my enquiry was cut short by a simple
...
\label{sec:org451b724}

The experiment touched on in \ref{orgee28f48} remained largely tangential,
...
explains why we see it being given so great a preference in figure \ref{fig:org9b51390}. 
...
\caption{\label{fig:org30a238e}
Performance of the \emph{Xysxim} population on the simple, linearly separable dataset first encountered in \S \ref{org0cb46bf}.}
\end{figure}
...
\label{sec:orgfbc585c}
\label{org783601d}
\subsection{The trouble with crossover}
\label{sec:org6f3d2c1}
...
used in \S \ref{orga916dfa}, and demonstrate its impact on classification tasks in 
Section \ref{org1da07bf}. Its applicability, as we have seen, relies
...
neither the syscall preparation task (cf. \S \ref{orgdc5d98b}), which uses a
singleton problem set, nor the
Snake experiment (cf. \S \ref{orgee28f48}), which uses a continuously
...
throughout Chapter \ref{org465e5f4}, in the analysis of heatmap data. 
...
figure \ref{fig:orga505c6a} -- whose evolutionary run bore the same configuration as the population
in which the malignant gene problem was first diagnosed (see \S \ref{orgf6d19fa}).  

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{../images/plots/xilvyb.pdf}
\caption{\label{fig:orga505c6a}
...
and introducing a selective incentive for behavioural diversity (see \S \ref{org4a9c21b}) appears to yield
...
completing the syscall preparation task, outlined in \S \ref{orgfe5f60b}.
...
\ref{fig:org79895f2} was never equaled in subsequent trials. 
...
\ref{fig:org79895f2}, but the results so far have shown an extraordinary improvement over
their predecessors. The performance curves for this run are shown in figure \ref{fig:orgc77c513}. 
...
As we can see in figure \ref{fig:org0df3f7f}, \emph{even the linearly inseparable} 
...
\label{sec:org9ff8312}

\section{Note}
\label{sec:org8ab7829}
